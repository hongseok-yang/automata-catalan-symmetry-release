...
`coarea`, `zetaMap`, `wrappedFlat`, `firstAscent`, `tailU`).  The worked
examples are checked by `decide`/`native_decide` in `Examples.lean`.
...
* `thm:wrp-logspace` — multihead model: `wrp_isLogspaceMH` +
  ...
  (`WRPWorktape.lean`, through the proved simulation
  `MHCOneHead.lean` + `MHCToTM.lean` → `LogspaceTM.lean`).
...
* `thm:wrp-not-closed` — `wrp_not_closed_preimage_comp` (regular preimage
  fails; `WRPCompWitness.lean`) and `wrp_not_closed_composition` (with the
  genuine 2DFT `S`; `WRPNotClosedComp.lean`, `SMapWRP.lean`).
...
(`SliceFasAssemblyGA.lean` and the `Slice*` files), and verbatim over an
arbitrary slice `u·vⁿ·z` in `OneLoopSlice.lean`
(`one_loop_finite_state`, `one_loop_rank_graph`,
`one_loop_presburger_sel`/`_label`/`_rankLt`/`_rankEq`/`_tie`/`_wrpOrd`).
`lem:presburger-counting` is the admitted axiom
`isSliceFamilySemilinear2_count`.  `lem:semilinear-envelope` —
`semilinear_envelope_dichotomy` (`Semilinearity.lean`) and
`EnvelopeMin.semilinear_envelope_min`.  `thm:wrp-slice-semilinearity` —
`wrp_slice_profile_semilinear` (`NoSwapWRP.lean`).
...
(`S_tri_not_semilinear`, `Semilinearity.lean`), and the headline
`thm:wrp-no-swap` — `wrp_no_area_dinv_swap` (`NoSwapWRP.lean`).
...
`heightSweep_isSRR1` (`SRR1.lean`).

## Modelling conventions

1. **Two renderings of `def:wrp`.**  The working class `WRP.IsWRP` asks the
   tie-order χ only to totally order the *selected* atoms of each word (the
   paper's remark notes χ-totality implies this).  It is a superset of the
   paper's class, so negative theorems over it are formally stronger.  The
   verbatim class `WRP.IsWRPPaper` (χ a strict total order on all atom pairs,
   and every arity positive) is also formalised (`WRPTieTotal.lean`), the
   inclusion is proved, and every headline theorem is restated over it
...
3. **The no-swap statement drops bijectivity.**  `wrp_no_area_dinv_swap`
   refutes any WRP transduction realising *some map* `F : D_n → D_n` with
   `area(F P) = dinv P` and `dinv(F P) = area P` — bijectivity of `F` is not
   assumed, which strengthens the paper's statement.

4. **The 2DFT reading of `cor:zeta-not-regular`.**  "Regular" is rendered as
   `Polyreg.IsRegular`, the arity-1 (dimension-1) MSO string transductions —
   by Engelfriet–Hoogeboom exactly the deterministic 2DFT class, which is the
   equivalence the paper itself invokes.  No literal two-way machine is
   re-proved equivalent; the lower bound uses the single admitted closure
   `polyreg_regular_preimage` applied to the hypothetical realiser and to a
   constructed counting encoder.

5. **`lem:one-loop-rank-affine` in graph form.**  `OneLoopSlice.lean` states
...
6. **`lem:semilinear-envelope` constants are rational.**  The
...
7. **`cor:srr-quadratic` lives in the multihead model.**  Its quadratic step
...
8. **Where the axioms are used.**  `SliceMSO.buchi` enters wherever a
...
9. **Decidable examples.**  The paper's worked examples are verified by
   ...
   plus spot checks in `NarayanaSweep.lean` and `ZetaClassification.lean`).
