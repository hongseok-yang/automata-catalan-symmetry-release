...
below.  The numerical worked examples are instead discharged by compiled
evaluation (\texttt{native\_decide}) and therefore additionally trust the Lean
compiler; none of the results in Table~\ref{tab:lean} relies on compiled
evaluation.  This section describes the scope of the formalisation, its trust
base, and the places where its statements differ from those in the paper.
...
(Definitions~\ref{def:rank-source}--\ref{def:wrp}); and the two machine models
of Section~\ref{sec:wrp-evaluation}.
...
handled in three ways: exactly three are admitted as axioms, some are proved in
...
The numerical examples involving $\area$, $\dinv$, $\zeta$, and the sweep
maps are certified by Lean's executable decision procedures.  Structural
examples, such as the two presentations of reverse-complement as a 2DFT and an
$\MSO$ string transduction, are not formalised.  The repository documentation
maps the numbered statements of the paper to their formal counterparts,
section by section.
...
Section~\ref{sec:slice-semilinearity} derives
Lemma~\ref{lem:presburger-counting} from them; the development proves that
lemma instead, in the single-count case, which is all its arguments need, and
by a route that uses neither.  It first decomposes the relation $A$ into pieces
with linearly independent period vectors, so the points of each piece have
unique coefficient tuples.  On one such piece, the bound
$|A_x|\le C(\|x\|_\infty+1)$ forces the kernel of the parameter map to
have rank at most one: two independent kernel directions would create
quadratically many points above a parameter of linear size.  Each fibre is
therefore an arithmetic progression with a parameter-independent step.
Inclusion--exclusion then counts finite unions of these progressions.  Thus,
the proof uses no quasi-polynomial machinery, and the entire counting layer is
axiom-free.
...
follows from (ii) and the two proved ingredients above.  Indeed, a
prefix-additive rank function is a fixed signed combination of prefix ranks and
bounded local corrections.  Each prefix rank is a weighted sum of counts of
earlier positions, grouped by source state and input letter.  These groups are
$\MSO$-definable by the proved automata-to-logic direction, their position sets
are semilinear along the family by~(ii), and the counting lemma makes their
counts semilinear as well.
...
Proposition~\ref{prop:two-pyramid-criterion}.  The formal proof of that
proposition applies (iii) twice rather than composing and collapsing, and so
needs neither; see the discussion of statement fidelity below.
Theorem~\ref{thm:eh} is excluded for the same kind of reason, discussed under
Scope above.
\end{itemize}

Table~\ref{tab:lean} records which axioms each headline result uses, and that
record can be audited mechanically with Lean's \texttt{\#print axioms}
command.  Several substantial parts of the development are axiom-free: the
basic closure and bounded-rank collapse theorems; the memberships of $\zeta$ and
$H$; Theorem~\ref{thm:narayana-sweep}; the two-pyramid closed forms
(Lemmas~\ref{lem:zeta-two-pyramid} and~\ref{lem:H-two-pyramid}); and the
deficit-zero lemmas of Section~\ref{sec:main}, including the model-free
obstruction (Corollary~\ref{cor:model-free-obstruction}).  The two simulations
that transfer the logspace evaluator between the machine models are also
axiom-free.  The evaluator theorem itself uses~(i) to turn the $\MSO$ data of a
presentation into finite automata.

The headline no-swap theorem and the arity-one case of
...
A machine-checked proof certifies exactly its formal statement.  Most of the
formal statements follow the paper directly; the following differences matter
when interpreting the headline results.
\begin{itemize}[leftmargin=*,itemsep=0.5em]
\item \emph{Partial maps and realisation.}
Transductions are modelled as total functions from input words to optional
output words; an absent value means ``undefined''.  Realisation
(Definition~\ref{def:relative}) constrains a transduction only on the stated
domain.

\item \emph{Two versions of $\WRP$.}
The verbatim class follows Definition~\ref{def:wrp}: copy names have positive
arity, and $\chi$ is a strict total order on selected atoms.  The working class
permits arity zero and requires only the combined output order to be total on
selected atoms.  It is a syntactic superset, so its negative theorems are
stronger; each negative headline theorem is also restated for the verbatim
class with the same trust base.  The positive memberships of $\zeta$, $H$, and
the additive level sorts are proved directly for the verbatim class.

\item \emph{Closure properties.}
Theorem~\ref{thm:wrp-closures} is the least completely rendered headline
result.  The formal clauses omit the arity bound, definition by cases, and
letter-deleting relabellings; restriction is given by an $\MSO$ sentence; and
tagging and concatenation treat two maps with a one-letter separator rather
than $r$ maps with separator words.  Since these are positive statements,
working-class proofs do not imply verbatim-class closure.  Verbatim-class
versions are currently available only for relabelling and concatenation.

\item \emph{Lower-bound statements.}
Theorem~\ref{thm:wrp-no-swap} has the paper's map form, with no bijectivity
assumption.
Corollary~\ref{cor:inverse-zeta-not-wrp} is stated as the nonexistence of a
$\WRP$ left inverse: no $\WRP$ transduction outputs $P$ on input $\zeta(P)$
for every Dyck path $P$.  This implies the paper's corollary without formalising
the classical bijectivity of $\zeta$.
The two-pyramid criterion (Proposition~\ref{prop:two-pyramid-criterion}) is
stronger in Lean: it omits the growth hypothesis
$|f(P_{m,n})|=O(|P_{m,n}|)$ because its proof applies axiom~(iii) directly.

\item \emph{Machine-model statements.}
In Corollary~\ref{cor:zeta-not-regular} and the 2DFT clause of
Theorem~\ref{thm:H-not-polyregular}, a ``deterministic two-way transducer'' is
rendered in Lean as a deterministic $\MSO$ string transduction.  Lean checks
this $\MSO$ formulation; the corresponding 2DFT conclusions in the paper then
use Theorem~\ref{thm:eh} externally, since that equivalence is not formalised.
Theorem~\ref{thm:wrp-logspace} is proved for both machine models of
Section~\ref{sec:wrp-evaluation}, but only over input alphabet $\{U,D\}$ and
without the output-length clause $|T(w)|=O(n^k)$.  Its polynomial step bound is
verified.  Corollary~\ref{cor:srr-quadratic}, including its quadratic step
bound, is verified in the multihead model whose unit-cost comparisons match
the paper's cost accounting.
\end{itemize}

The repository documentation records the remaining statement-level
conventions, including the graph form used for
Lemma~\ref{lem:one-loop-rank-affine}.
...
writing of the paper was assisted by Claude and Codex.  The human authors are
