...
With the exception of the new height-sweep lower-bound chain
(Lemmas~\ref{lem:H-two-pyramid} and~\ref{lem:H-probe} and
Theorem~\ref{thm:H-not-polyregular}), the results of this paper have been
formalised and machine-checked in the Lean~4 proof assistant
\cite{DeMouraUllrich2021}, building on the Mathlib library
\cite{MathlibCommunity2020}.  The development was produced
...
formalised results of the paper: Table~\ref{tab:lean} lists the headline
theorems with their Lean names or current status, and the intermediate lemmas
supporting those results are formalised as well.  In
particular, the deficit-zero analysis of Section~\ref{sec:main}, the
first-ascent computations of Section~\ref{sec:inverse-zeta}, and the
...
arbitrary slice $u\,v^n z$.  Background results quoted from the literature, such as
...
files in the repository map the formalised numbered statements of the paper to
...
\bottomrule
\end{tabularx}
\caption{The headline results and their Lean status.  The last column lists
the admitted axioms (i)--(vi), described in the text, on which each completed
Lean proof depends beyond the kernel; ``none'' marks results proved with no
admitted axiom, and ``pending'' marks the new theorem not yet added to the
development.  Lean names are cited without namespace prefixes; the repository
documentation maps the formalised statements of the paper, including their
intermediate lemmas, to their formal counterparts.}
\label{tab:lean}
\end{table}

\paragraph{Trust base.}
The completed development is built from Lean's kernel primitives plus exactly
six explicitly admitted axioms.  These axioms package standard external
results or their consequences concerning $\MSO$, finite automata, semilinear
sets, prefix-additive rank functions, and polyregular maps.  Some are stated
using the development's own abstractions so that they can be applied directly
to the formalised arguments below.
\begin{enumerate}[label=(\roman*)]
...
Section~\ref{sec:slice-semilinearity} from Woods' Presburger counting theorem
\cite[Theorem~1.10]{Woods2015}, and the formalisation admits the two
