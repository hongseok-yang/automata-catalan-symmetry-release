...
It comprises roughly $100{,}000$ lines of Lean in $159$ source files, pins an
...
Thm.~\ref{thm:wrp-strict-over-poly} (above $\polyreg$) &
  \texttt{polyreg\_strict\_subset\_wrp} & (v) \\
...
Thm.~\ref{thm:H-not-polyregular} (height sweep beyond $\polyreg$) &
  ...
  \texttt{heightSweep\_not\_regular} & (v) \\
...
lists the admitted axioms (i)--(v), described in the text, on which each
...
five explicitly admitted axioms.  These axioms package standard external
...
\item \texttt{count\_graph\_semilinear}: Lemma~\ref{lem:presburger-counting}
itself, in its single-count case.  Unlike (ii) and (iii), it is stated
entirely in the vocabulary of the Lean standard library --- semilinear sets
and cardinalities --- with no notion from this development, so that it can be
compared directly against the literature.
...
\end{enumerate}
Axiom (iv) is the point at which the formalisation consumes the same external
input as the informal development: Lemma~\ref{lem:presburger-counting} is
derived in Section~\ref{sec:slice-semilinearity} from Woods' Presburger
counting theorem~\cite[Theorem~1.10]{Woods2015}, and the formalisation admits
the lemma rather than re-proving that counting machinery.  The two counting
statements the development actually uses --- the joint count graph of a
two-parameter family, and the row-uniform form in which each row of that
graph is eventually affine on the residue classes of a single period --- are
both \emph{proved} from (iv); the passage from the joint graph to the
row-uniform form is axiom-free.  Which
...
and~\ref{thm:H-not-polyregular}) use only the preimage axiom (v).
...
growth hypothesis: the formal proof applies the preimage axiom (v) to the
