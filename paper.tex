...
It comprises roughly $100{,}000$ lines of Lean in $156$ source files, pins an
...
Thm.~\ref{thm:wrp-strict-over-poly} (above $\polyreg$) &
  \texttt{polyreg\_strict\_subset\_wrp} & (vi) \\
...
Prop.~\ref{prop:two-pyramid-criterion} (two-pyramid criterion) &
  \texttt{two\_pyramid\_criterion} & (vi) \\
...
Thm.~\ref{thm:H-not-polyregular} (height sweep beyond $\polyreg$) &
  ...
  \texttt{heightSweep\_not\_regular} & (vi) \\
...
lists the admitted axioms (i)--(vi), described in the text, on which each
...
and~\ref{thm:H-not-polyregular}) use only the preimage axiom (vi).
...
growth hypothesis: the formal proof applies the preimage axiom (vi) to the
