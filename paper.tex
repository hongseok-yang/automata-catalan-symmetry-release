...
It comprises roughly $107{,}000$ lines of Lean in $170$ source files, and it
...
from the statements ofthis paper.
...
in the general form of the paper, over an arbitrary slice $u\,v^n z$.  The single-count case of
Lemma~\ref{lem:presburger-counting} is proved by a route
different from the one taken in Section~\ref{sec:slice-semilinearity}; see the
discussion of the trust base below.  Background results quoted from the literature are not re-proved; where the
arguments need them, they enter through the admitted axioms below.
Theorem~\ref{thm:eh} is the exception: it is not one of those axioms, because
the formal statements quantify directly over deterministic $\MSO$
transductions, and it is used only to read them as statements about two-way
machines; see the discussion of statement fidelity below.  The paper's numeric worked
...
Every mathematical result in the completed development is built from Lean's
kernel and its three standard axioms (\texttt{propext},
\texttt{Classical.choice}, and \texttt{Quot.sound}), together with exactly
three explicitly admitted axioms of our own, each a standard external result
about $\MSO$, finite automata, semilinear sets, or polyregular maps.  The
second is stated using the development's own abstractions so that it can be
applied directly to the formalised arguments that follow.
\begin{enumerate}[label=(\roman*)]
...
development.
\item \texttt{msoDefinableRel2\_semilinear\_general}: for a two-parameter
family of words assembled from fixed blocks that repeat affinely in the two
parameters, as in the family $W_{m,n}$ of
Definition~\ref{def:two-parameter-family}, the set of tuples consisting of the
two parameters together with a $k$-tuple of positions satisfying a fixed
$\MSO$ formula is a semilinear subset of $\N^{k+2}$.  This combines (i) with
the Ginsburg--Spanier theorem~\cite{GinsburgSpanier1966}.
...
\end{enumerate}
One statement close to (ii) is deliberately not assumed.  The $\Z^d$-valued
companion of (ii) --- semilinearity of the value graph of a prefix-additive
rank function read along such a family --- is proved instead, from (i), (ii)
and counting.  A rank function unfolds into a fixed signed combination of
prefix ranks and bounded local corrections.  A prefix rank is the weighted
sum, over the finitely many pairs of source state and letter, of the number of
positions below the queried one that carry that pair.  Those position sets are
$\MSO$-definable by the converse direction of (i), which the development
proves; (ii) turns that definability into semilinearity along the family; and
their counts are then semilinear by the counting result described next, the
fibres being sets of positions in a slice word whose length is affine in the
two parameters.  The companion is therefore a theorem rather than a fourth
axiom, but it does rest on (ii).

The counting layer rests on no axiom.  Section~\ref{sec:slice-semilinearity}
...
a piece, the hypothesis that the number of points above a parameter grows at
most linearly forces the kernel of the map from coefficient tuples to
parameters to have rank at most one: two independent directions in that
kernel would place quadratically many points above a single parameter of
linear size.  Every fibre is therefore an arithmetic progression whose
direction does not depend on the parameter.  Counting the
union of finitely many such progressions is then elementary.  Both counting
statements that the development uses follow from this, and are likewise free
of any axiom: the joint count graph of a two-parameter family, and the
row-uniform form of that graph, in which the counts for each fixed value of
the first parameter are eventually affine on the residue classes of a single
period.
...
collapse, so the bound $|f(P_{m,n})|=O(|P_{m,n}|)$ is never needed.  The
formal criterion is therefore more general than
Proposition~\ref{prop:two-pyramid-criterion}, and this is why neither the
linear-growth collapse (Theorem~\ref{thm:polyregular-linear-collapse}) nor the
regular-preimage property of two-way transducers appears among the admitted
axioms.
