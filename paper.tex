...
machine-checked, the guarantees described in this section do not depend on how
those proofs were found.  The formalisation is distributed with
the paper.\footnote{\url{https://github.com/hongseok-yang/automata-catalan-symmetry-release}}
It comprises roughly $82{,}000$ lines of Lean in $162$ source files, and it
pins an exact toolchain (Lean~4 v4.33.1 and the matching Mathlib release) so
that the build is reproducible.  It contains no \texttt{sorry}, Lean's
placeholder for an unproved statement.  Every mathematical result in the
development is proved from Lean's kernel rules together with the short list of
admitted axioms described below; the paper's numeric worked examples are
instead discharged by compiled evaluation (\texttt{native\_decide}), so each of
those additionally trusts the Lean compiler.  No result in
Table~\ref{tab:lean} below depends on one.  This section summarises what is formalised,
on which axiomatic base, and where the formal statements deliberately differ
from the statements of this paper.

\paragraph{Scope.}
The development formalises the theory of the paper from the ground up rather than verifying an
isolated statement.  On the combinatorial side, it defines Dyck paths, the statistics $\area$,
$\dinv$, and $\coarea$, area sequences, the zeta map of
Definition~\ref{def:zeta}, the height sweep $H$ of
Section~\ref{sec:narayana-sweep}, and the families $W_n$, $W_{m,n}$, and
$P_{m,n}$.  On the machine side, it defines $\MSO$ on words together with the
...
of Section~\ref{sec:wrp-evaluation}.  On this base, it proves the
results of the paper.  Table~\ref{tab:lean} lists the headline theorems with
their Lean names; the intermediate lemmas that support them are formalised as
well.  In
...
proofs, the last of these in two forms: instantiated at the family $W_n$, and
in the general form of the paper, over an arbitrary slice $u\,v^n z$.
Background results quoted from the literature enter either through the three
admitted axioms listed below or as results the development proves for itself;
the two paragraphs on the trust base say which is which.
Theorem~\ref{thm:eh} is neither: the formal statements quantify directly over
deterministic $\MSO$ transductions, and the theorem is used only to read them
as statements about two-way machines; see the discussion of statement fidelity
below.  The paper's numeric worked
examples, such as the $\area$, $\dinv$, and $\zeta$ computations, and the small
sweep computations, are certified by Lean's executable decision procedures;
the paper's structural examples, such as the reverse-complement presented both
as a 2DFT and as an $\MSO$ transduction, are not formalised.  Two
documentation files in the repository map the numbered statements of the paper
to their formal counterparts, section by section.
...
state all three in full.  Below, $w\models\varphi(p_1,\ldots,p_k)$ means that
the word $w$ satisfies the $\MSO$ formula $\varphi$ when its free first-order
variables are interpreted by the positions $p_1,\ldots,p_k$.
\begin{enumerate}[label=(\roman*)]
...
\[
   \bigl\{(m,n,p_1,\ldots,p_k)\in\N^{k+2}\;:\;
          F(m,n)\models\varphi(p_1,\ldots,p_k)\bigr\}
\]
...
$W_n$ of Section~\ref{sec:wrapped-flat} is its row $m=1$.  The axiom composes
(i) with the Ginsburg--Spanier theorem~\cite{GinsburgSpanier1966}: marking the
$k$ positions turns $\varphi$ into a sentence over $\Sigma\times\{0,1\}^k$, the
marked words obtained from $F$ form a regular language read along two nested
loops, and the Parikh image of a regular language is semilinear.  Both
hypotheses are load-bearing: finiteness of $\Sigma$ for the first step, and the
block-linear shape of $F$ for the second, since an unrestricted
$F\colon\N^2\to\Sigma^*$ can encode a non-semilinear set of positions.
...
\paragraph{What is proved rather than assumed.}
The arguments in the body of this paper quote several further results from the
literature, and the slice analysis needs one statement close to (ii) that is
not among the axioms either.  All of them are proved, so the formalisation
assumes strictly less than the arguments given here.
\begin{itemize}
\item \emph{The converse direction of B\"uchi--Elgot--Trakhtenbrot}, that the
run of a DFA is $\MSO$-definable.  The paper uses it for the restriction clause
...
by a route that uses neither.  Decompose the relation $A$ into pieces whose period vectors are
linearly independent, so that the points of a piece correspond bijectively to
coefficient tuples and none is counted twice.  On such a piece the hypothesis
$|A_x|\le C(\lVert x\rVert_\infty+1)$ forces the kernel of the map from
coefficient tuples to parameters to have rank at most one, since two
independent kernel directions would place quadratically many points over a
single parameter of linear size.  Every fibre is then an arithmetic progression
whose step is independent of the parameter, and finite unions of such
progressions are counted by inclusion--exclusion.  No quasi-polynomial appears,
and the whole counting layer of the development is axiom-free.
...
analogous statement for the $\Z^d$-valued rank $\kappa$ of an atom: that
\[
   \bigl\{(m,n,p_1,\ldots,p_k,v)\;:\;
          \kappa\bigl(F(m,n),p_1,\ldots,p_k\bigr)=v\bigr\}
\]
is semilinear, with $v\in\Z^d$ encoded by a pair of vectors in $\N^d$.  This
follows from (ii) and the two proved ingredients above.  A prefix-additive rank
function is a fixed signed combination of prefix ranks and bounded local
corrections; a prefix rank is a weighted sum, over the finitely many pairs of
source state and letter, of the number of earlier positions carrying that pair;
those position sets are $\MSO$-definable by the converse direction of (i) and
hence semilinear along the family by (ii), and their counts are then semilinear
...
behind Theorem~\ref{thm:wrp-logspace} are all axiom-free, although the
theorem itself uses~(i).  The headline
...
formalised twice.  The verbatim class renders the definition as stated: the
tie-order $\chi$ is itself a strict total order on the selected atoms, and
every copy name has positive arity.  The working class asks only that the
combined output order $\prec$ --- atom rank refined by $\chi$ --- should
totally order the selected atoms of each input word.  The working class allows more
presentations, so the negative theorems proved for it are formally stronger.
...
Theorem~\ref{thm:wrp-closures} is the least
completely rendered result: the formal clauses track no arity, restriction is
stated for an $\MSO$-definable rather than a regular input language,
definition by cases and letter-deleting relabellings are not formalised, and
source tagging and concatenation are proved for two maps with a one-letter
separator rather than for $r$ maps with separator words.  Because closure
statements are positive, proving them over the working class does not yield
the verbatim-class statement, which is available only for relabelling and
concatenation.
Theorem~\ref{thm:wrp-no-swap} is formalised in its map form: no bijectivity
is assumed of the hypothetical swap.
Corollary~\ref{cor:inverse-zeta-not-wrp} is rendered as left inversion: there
is no $\WRP$ transduction that outputs $P$ on input $\zeta(P)$ for every Dyck
path $P$.
...
``deterministic two-way transducer'' is rendered as the class of arity-one
deterministic $\MSO$ transductions, which Theorem~\ref{thm:eh} identifies with
the two-way machines; the identification itself is quoted,
...
collapse, so the bound $|f(P_{m,n})|=O(|P_{m,n}|)$ is never needed.  The formal
criterion is therefore more general than
Proposition~\ref{prop:two-pyramid-criterion}; it is also what removes the
collapse and the composition closure from the trust base, as noted above.
Theorem~\ref{thm:wrp-logspace} is proved for both machine models of
Section~\ref{sec:wrp-evaluation}, with the input alphabet fixed to $\{U,D\}$
and apart from its output-length clause $|T(w)|=O(n^k)$, which is not
formalised.  The evaluator is constructed as an explicit multihead
bounded-counter transducer and verified together with its polynomial step
bound.  A proved two-stage simulation --- head elimination, followed by binary
counters on worktape tracks --- then transports the result to worktape
machines.  Corollary~\ref{cor:srr-quadratic} is proved in
the multihead model, with a verified quadratic bound on the number of machine
steps; the unit-cost word comparisons of that model match the cost accounting
of the proof given in the paper.  The remaining statement-level conventions, such as the
