...
\[
  ...
  \iff h<m.
\]
The step $U_2$ exists exactly when $h\le n-1$, and $U_1$ exactly when
$h\le m-1$.  The first inequality therefore holds whenever $U_2$ exists, the
second for every $h\ge1$, and the third whenever $U_1$ exists.  The three
remaining comparisons are immediate from the position differences
$D_2-D_1=2n$, $U_2-U_1=2m$ and $D_2-U_1=2m+2n-2h>0$.  Thus, whichever members
are absent, the surviving letters at each positive level occur in the relative
order $D,U,D,U$.
