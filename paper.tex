...
$\D_n$~\cite{ThomasWilliams2018}; its exchange of the two statistics defined
below follows from the corresponding tree and zeta descriptions
in~\cite{CeballosFangMuhle2020,Fang2024,SulzgruberThiel2018}.  What is new is
...
Two statistics of a Dyck word $w=w_1\cdots w_{2n}$ are the number of
\emph{valleys}
$\val(w)=|\{i:1\le i<2n,\ w_iw_{i+1}=DU\}|$ and the number of
\emph{double rises}
$\dr(w)=|\{i:1\le i<2n,\ w_iw_{i+1}=UU\}|$.  Each counts occurrences of a
fixed length-two factor, so each is a \emph{local} statistic, in contrast to
the global $\area$ and $\dinv$.  For $n\ge 1$, writing $w$ in its maximal runs
