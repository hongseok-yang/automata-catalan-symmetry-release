...
point, but we prove that $\zeta$ is not polyregular.  The missing mechanism is a
global ordering by numerical levels whose range grows with the input.  We call
its general form a \emph{rank sort} and introduce \emph{weighted-rank
polyregular maps} ($\WRP$), which extend polyregular maps by one such sort.
The class contains $\zeta$ and the classical height-sweep bijection witnessing
the Narayana symmetry, yet is a proper subclass of deterministic logspace.
We prove that $\zeta^{-1}$ lies outside $\WRP$ and that no $\WRP$ map can
realise a semilength-preserving area--dinv swap.  Thus, the rank-sorting
strategy behind $\zeta$ cannot be extended within $\WRP$ to a full exchange of
the two statistics.
\end{abstract}
...
A second positive result places a classical height-sweep bijection inside the
model.  This bijection is known to exchange two natural local statistics on
Dyck paths, the numbers of valleys and double rises.  Our contribution here is
to show that the same map is realised by the particularly simple fragment
$\SWR$ of $\WRP$.
...
These positive results show that $\WRP$ captures a recurring Catalan mechanism
rather than a single isolated construction.  We next ask whether its
rank-sorting mechanism is powerful enough to produce a uniform bijection
explaining the full $q,t$-Catalan symmetry.  Our central theorem gives a
negative answer.
...
realisation of an area--dinv swap.  The second shows that $\WRP$ is not closed
under inversion: $\zeta$ belongs to $\WRP$, whereas $\zeta^{-1}$, for which
Thomas and Williams~\cite{ThomasWilliams2018} gave an explicit construction,
does not.
...
$H$, which realises the bivariate Narayana symmetry, inside $\SWR$ and hence
inside $\WRP$.  A self-contained proof of its known combinatorial properties
...
Section~\ref{sec:lean} describes a machine-checked formalisation of the
paper's results in the Lean proof assistant, and the small axiomatic base on
which it rests.
...
\begin{example}[Peaks and parity in $\MSO$]\label{ex:mso-peak}
...
\subsection{Polyregular maps}\label{sec:polyreg}
...
\begin{enumerate}[label=(\roman*)]
\item a finite set $C$ of \emph{copy names}, each with an \emph{arity}
$k_c\ge 1$;
...
\begin{example}[A quadratic Dyck-path map]\label{ex:dyck-squaring}
...
\begin{remark}[The scalar scan-order case]\label{rem:swr}
...
$O(n^k)$ potential atoms once while retaining the best candidate seen so far,
...
an accidental defect of the definition.  Closure under functional inversion
is a separate question: later we prove that $\zeta\in\WRP$ but
...
The map $\zeta$ admits no $\WRP$ presentation in which every rank source is
uniformly bounded.  Equivalently, every $\WRP$ presentation of $\zeta$ uses at
least one rank source whose prefix ranks are unbounded on Dyck inputs.
\end{corollary}

Indeed, a presentation in which every rank source were uniformly bounded
would be polyregular by the theorem, contradicting
...
any $\WRP$ presentation of $\zeta$ must have access to an unbounded numerical
source.
...
Zeta $\zeta$ &
  ...
  $\area(\zeta(P))=\dinv(P)$. \\
Height sweep $H$ &
  $H\in\SWR\subseteq\WRP$; its $\polyreg$ status is not determined here. &
  Section~\ref{sec:narayana-sweep}
  (Prop.~\ref{prop:alw-sweep-swr} and
  Thm.~\ref{thm:narayana-sweep}) &
...
\item Zeta and $H$ are the positive rank-sort examples.  Both
belong to the particularly simple fragment $\SWR$: they sort atoms by one
additive integer atom rank and use a scan order to break ties.  For $\zeta$, the
lower bound $\zeta\notin\polyreg$ shows that this rank sort cannot be replaced
by any polyregular mechanism.  Together with the $\SWR$ presentation, this
places $\zeta$ in $\WRP\setminus\polyreg$ and witnesses the strict inclusion
$\polyreg\subsetneq\WRP$.  We do not prove a corresponding lower bound for
$H$; its membership in $\SWR$ is an upper bound, not a claim that $H$ lies
outside $\polyreg$.  The bijectivity and statistic exchange of $H$ are
classical; the classification supplied here is its membership in $\SWR$.
...
{Upper bound: $\zeta$ orders selected atoms by height}\label{sec:zeta-positive}
...
Then, $R$ is regular. Also, for all $m,n\ge 1$,
...
Definition~\ref{def:regular-language}.  By
...
\begin{proof}
Suppose, for contradiction, that some polyregular map $T$ realises $\zeta$ on
$\D$.  Let $a,b,\#$ be three distinct input symbols, with $\#$ serving as a
separator, and let
\[
   E\colon\{a,b,\#\}^*\longrightarrow\{U,D\}^*
\]
be the elementary 2DFT whose action on inputs of the form $a^m\#b^n$ is
\[
   E(a^m\#b^n)=U^mD^mU^nD^n=P_{m,n},\qquad m,n\ge 1.
\]
Its behaviour on other inputs is immaterial.  Such an $E$ is obtained by four
finite-state passes: output the $a$-block as $U$'s, output it again as $D$'s,
then do the same for the $b$-block.  Each $P_{m,n}$ is a Dyck path, so $T$
agrees with $\zeta$ on the image of $E$ that we use below.
...
\]
We use the standard facts that $S$ is regular, that regular languages are
closed under intersection, and that $L$ is not regular; see, for example,
Sipser~\cite[Chapter~1]{Sipser2013}.

A 2DFT is polyregular and $\polyreg$ is closed under composition
\cite{Bojanczyk2018}, so $T\circ E$ is polyregular.  Since $S$ is regular, its
membership condition is $\MSO$-definable.  Conjoining that condition with the
domain sentence in a polyregular presentation therefore gives a polyregular
map $g$ with domain $S$ and
\[
   g(a^m\#b^n)=T(P_{m,n})=\zeta(P_{m,n}),\qquad
   |g(a^m\#b^n)|=2(m+n)\le 2\,|a^m\#b^n|.
\]
Thus, $g$ has linear output growth \emph{on its domain}, so the linear-growth
collapse (Theorem~\ref{thm:polyregular-linear-collapse}) applies to $g$: it is
a deterministic $\MSO$ string transduction, equivalently computed by a
deterministic 2DFT\@.  A 2DFT has regular inverse images of regular languages
\cite{EngelfrietHoogeboom2001}, so $g^{-1}(R)$ is regular for the language $R$
of Lemma~\ref{lem:zeta-probe}.  But
\[
   g^{-1}(R)=\{a^m\#b^n:m,n\ge 1,\ \zeta(P_{m,n})\in R\}=L,
\]
contradicting the nonregularity of $L$.
\end{proof}
...
$\mathrm{dom}(T)$.  The proof of Theorem~\ref{thm:zeta-not-polyregular}
sidesteps this by collapsing the linear-growth restriction $g$, whose
domain is the regular language $\{a^m\#b^n:m,n\ge 1\}$.  On this domain, the
linear-growth hypothesis genuinely holds, so the conclusion is correspondingly
the strong one, that \emph{no} polyregular map realises $\zeta$.
\end{remark}
...
ability to sort by an unbounded integer rank.
\end{remark}

\section{The height sweep and the Narayana symmetry}
\label{sec:narayana-sweep}

As a second positive example for $\WRP$, consider the starting-height,
...
statistic exchange are known; our computational result is that
Proposition~\ref{prop:alw-sweep-swr} places it in $\SWR\subseteq\WRP$.
...
rises and no valley, is sent to the zigzag $UDUDUD$, with $\val=2$ and $\dr=0$:
...
Appendix~\ref{app:narayana-sweep-proof}.  Combined with
Proposition~\ref{prop:alw-sweep-swr}, Theorem~\ref{thm:narayana-sweep}
establishes the introduction-level statement,
...
connectives and quantifiers.  A set of integer tuples is called
\emph{Presburger-definable} if membership in the set can be expressed by such a
formula.  A condition on integer variables is called
\emph{Presburger-definable} if it can be expressed by such a formula.  The
Presburger-definable sets are precisely the semilinear sets.  We
use two technical ingredients.  First, on a repeated-block slice, the
...
parametric Presburger counting theorem of Woods~\cite{Woods2015}, which
...
The theorem below asserts that, if $T\in\WRP$ and $|T(W_n)|=O(n)$, these
first-ascent pairs form a semilinear set as $n\ge1$ varies.
...
Consider $T\in\WRP$.  Assume $|T(W_n)|=O(n)$.  Then, the set
...
(Lemma~\ref{lem:deficit-zero-targets}), so its first-ascent pair $(\fas,\tailU)$ is
confined to one explicit set $S_{\mathrm{tri}}\subseteq\N^2$, the ``triangular''
region defined in Corollary~\ref{cor:forced-triangular-pairs}.  As $n$ grows,
...
The no-swap theorem now follows.
...
is $F(W_n)\in\D_{n+1}$, so $|T(W_n)|=2(n+1)=O(n)$.  By
Theorem~\ref{thm:wrp-slice-semilinearity}, the set of first-ascent pairs
\[
   \{(\fas(T(W_n)),\tailU(T(W_n))):n\ge 1\}
\]
is semilinear.  But $T(W_n)=F(W_n)$, and
Corollary~\ref{cor:forced-triangular-pairs} identifies the same set
with $S_{\mathrm{tri}}$, which is not semilinear by
Lemma~\ref{lem:triangular-not-semilinear}.  Contradiction.
\end{proof}
...
$q,t$-symmetry.  We now turn to a different structural question: is $\WRP$
closed under taking inverses?  The zeta map provides a counterexample: it is a
semilength-preserving bijection in $\WRP$ (Theorem~\ref{thm:zeta-wrp}), whereas we
prove below that $\zeta^{-1}$ does not belong to $\WRP$.  Thus, $\WRP$ is not
closed under taking inverses, even among semilength-preserving bijections of Dyck
paths.
...
$\zeta^{-1}\in\WRP$.  First, since $\zeta^{-1}$ is semilength-preserving,
...
for any $T\in\WRP$ with $|T(W_{m,n})|=O(m+n)$, its first-ascent pairs,
recorded together with $m$ and $n$, form a semilinear set
...
Let $T\in\WRP$ be fixed and suppose $|T(W_{m,n})|=O(m+n)$.  Then,
\[
...
$\zeta^{-1}\in\WRP$, then Theorem~\ref{thm:two-parameter-semilinearity}
makes
\[
...
$\WRP$ is not closed under taking inverses, even for bijections of Dyck paths
that preserve semilength.

\section{Related work}\label{sec:discussion}\label{sec:related-work}
...
bijections, but the lower bounds show that it cannot realise a full
area--dinv exchange and is not closed under taking inverses.
...
It comprises roughly $100{,}000$ lines of Lean in $154$ source files, pins an
...
and $\coarea$, area sequences, the zeta map of Definition~\ref{def:zeta}, and
the families $W_n$ and $W_{m,n}$; $\MSO$ on words together with the
...
their Lean names, and the intermediate lemmas are formalised as well.  In
particular, the deficit-zero analysis of Section~\ref{sec:main}, the
first-ascent computations of Section~\ref{sec:inverse-zeta}, and the one-loop
lemmas of Section~\ref{sec:slice-semilinearity} all have formal proofs, the
last both instantiated at the family $W_n$ and verbatim over an arbitrary
slice $u\,v^n z$.  Background results quoted from the literature, such as
...
files in the repository map every numbered statement of the paper to its
formal counterpart, section by section; Table~\ref{tab:lean} cites Lean names
without their namespace prefixes.
...
Thm.~\ref{thm:wrp-strict-over-poly} (above $\polyreg$) &
  \texttt{polyreg\_strict\_subset\_wrp} & (i), (vi) \\
...
Thm.~\ref{thm:wrp-not-closed} (closure failure) &
  \texttt{wrp\_not\_closed\_preimage\_comp},
...
Thm.~\ref{thm:zeta-not-polyregular} (zeta beyond $\polyreg$) &
  \texttt{zetaMap\_not\_polyregular} & (vi) \\
Cor.~\ref{cor:zeta-not-regular} (zeta beyond $\MSO$) &
  \texttt{zetaMap\_not\_regular} & (vi) \\
...
Cor.~\ref{cor:inverse-zeta-not-wrp} (general arity) &
  \texttt{inverse\_zeta\_not\_wrp} & (i)--(iv) \\
\bottomrule
\end{tabularx}
\caption{The headline results and their Lean counterparts.  The last column
lists the admitted axioms (i)--(vi), described in the text, on which the Lean
proof depends beyond the kernel; ``none'' marks results proved with no
admitted axiom.  Lean names are cited without namespace prefixes; the
repository documentation maps every numbered statement of the paper,
including the intermediate lemmas, to its formal counterpart.}
\label{tab:lean}
\end{table}

\paragraph{Trust base.}
Every result is proved from Lean's kernel primitives plus exactly six
explicitly admitted axioms.  Each axiom is a standard fact from the
literature, and none of them mentions a notion introduced in this paper: they
speak only about $\MSO$, finite automata, semilinear sets, and polyregular
maps.
\begin{enumerate}[label=(\roman*)]
...
\item \texttt{regularRankTerm\_value2\_graph\_semilinear}: the $\Z^d$-valued
sibling of (ii), semilinearity of the value graphs of prefix-additive rank
functions read along such a family.
\item \texttt{isSliceFamilySemilinear2\_count}: the bounded-counting
principle of Lemma~\ref{lem:presburger-counting} in a per-row form: for each
value of the first parameter, the fibre count is eventually affine in the
second parameter on residue classes, with a period uniform across the first
parameter.
\item \texttt{twoParamCountGraph\_admitted}: the joint count-graph form of
Lemma~\ref{lem:presburger-counting} for two-parameter families.
...
\end{enumerate}
Axioms (iv) and (v) are the point at which the informal development also
consumes an external input: Lemma~\ref{lem:presburger-counting} is derived in
Section~\ref{sec:slice-semilinearity} from Woods' parametric Presburger
counting theorem~\cite{Woods2015}, and the formalisation admits the two
instances it uses rather than re-proving that counting machinery.  Which
...
deficit-zero lemmas of Section~\ref{sec:main} together with
Lemma~\ref{lem:triangular-not-semilinear}, and the machine simulations behind
Theorem~\ref{thm:wrp-logspace} are all axiom-free.  The headline no-swap
theorem and the arity-one case of Corollary~\ref{cor:inverse-zeta-not-wrp}
use only the B\"uchi--Elgot--Trakhtenbrot axiom (i).
...
presentations, so the negative theorems, proved over it, are formally
stronger; every headline theorem is also restated over the verbatim class
with the same admitted axioms, and the positive memberships of $\zeta$, $H$,
and the additive level sorts are proved in the verbatim class directly.
...
$\zeta^{-1}$ is not in $\WRP$.  In Corollary~\ref{cor:zeta-not-regular},
...
not re-proved.  Theorem~\ref{thm:wrp-logspace} is proved for both machine
...
selection and label formulas on the input annotated with $\bar i$; a fixed
$\MSO$ formula is checkable by a finite-state pass, in constant space beyond
$\bar i$.
...
candidate has been recorded yet); when $B$ is updated, record its label as
well.  At the end of the scan, emit the recorded label of $B$ and set $L:=B$;
if the scan found no selected atom with $L\prec\alpha$, every selected atom
has been emitted and the machine halts.  Because $\prec$ is a strict total
...
the set $C$.  At the end of the scan emit the cached label of $B$ and set
$L:=B$; if no selected atom was $\succ L$, the output is complete.  Because
$\prec$ is a strict total order
...
records in its finite control whether the first letter is $G$, skips the rest
of block $0$ and the separator (advancing past the first $\#$), and then copies
the remaining block-$2$ letters to its output if and only if that bit is set.
Its finite control also records whether it is before or after the separator;
every defined head transition moves right, and it halts acceptingly at the
right end marker.  On
input $w$ the composite $S\circ D$ outputs $w$ when $w\in L_{\ge 0}$ (first
letter $G$, bit set, copy emitted) and $\varepsilon$ otherwise; that is,
...
\label{rem:closure-boundary}
Both $\TDFT/\MSO$ maps and $\polyreg$ maps have regular inverse
images of regular languages: a polyregular presentation
(Definition~\ref{def:polyregular}) is a tuple-based $\MSO$
interpretation, and $\MSO$ interpretations admit backward translation
of $\MSO$ formulas, so ``the output lies in $K$'' pulls back to an
$\MSO$ sentence on the input.  $\WRP$ adds a rank-sort layer whose
...
A distinct closure question, closure under functional \emph{inversion}, also
fails: $\zeta\in\WRP$ while $\zeta^{-1}\notin\WRP$
(Section~\ref{sec:inverse-zeta}).  Unlike the failure above, this is not the
loss of a property the lower classes possess.  Polyregular maps are not closed
under functional inverse either: the unary squaring map
$s(a^n)=a^{n^2}$ is polyregular, but its inverse has domain
$\{a^{n^2}:n\ge0\}$, which is not regular, whereas every partial polyregular
presentation has an $\MSO$-definable, hence regular, domain.  The failure for
$\WRP$ is a separate expression of the same directional nature of the
rank-sort mechanism.
\end{remark}
...
If every rank source in a $\WRP$ presentation of $\zeta$ were uniformly
bounded, Theorem~\ref{thm:bounded-rank-collapse} would exhibit $\zeta$ as a
polyregular map, contradicting Theorem~\ref{thm:zeta-not-polyregular}.
Therefore every such presentation contains at least one source with unbounded
prefix ranks on the Dyck domain.  In the usual presentation from
...
$c=0$.  Otherwise, put $A=r-1$, so the earlier entries $a_1,\ldots,a_r$ are
...
of the required form $(A,\ldots,A,A-1,\ldots,A-1)$, and the number $d$ of trailing
$A-1$'s is fixed by $c$.  The equality case is therefore unique.
\end{proof}
\end{document}
