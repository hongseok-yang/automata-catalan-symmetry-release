...
It comprises roughly $105{,}000$ lines of Lean in $166$ source files, pins an
...
Thm.~\ref{thm:wrp-strict-over-poly} (above $\polyreg$) &
  \texttt{polyreg\_strict\_subset\_wrp} & (iv) \\
...
Prop.~\ref{prop:two-pyramid-criterion} (two-pyramid criterion) &
  \texttt{two\_pyramid\_criterion} & (iv) \\
Thm.~\ref{thm:zeta-not-polyregular} (zeta beyond $\polyreg$) &
  \texttt{zetaMap\_not\_polyregular} & (iv) \\
Cor.~\ref{cor:zeta-not-regular} (zeta beyond $\MSO$) &
  \texttt{zetaMap\_not\_regular} & (iv) \\
...
Thm.~\ref{thm:H-not-polyregular} (height sweep beyond $\polyreg$) &
  ...
  \texttt{heightSweep\_not\_regular} & (iv) \\
...
Cor.~\ref{cor:inverse-zeta-not-wrp} (general arity) &
  \texttt{inverse\_zeta\_not\_wrp} & (i)--(iii) \\
\bottomrule
\end{tabularx}
\caption{The headline results and their Lean counterparts.  The last column
lists the admitted axioms (i)--(iv), described in the text, on which each
...
four explicitly admitted axioms.  These axioms package standard external
results or their consequences concerning $\MSO$, finite automata, semilinear
sets, and polyregular maps.  Some are stated
...
and~\ref{thm:H-not-polyregular}) use only the preimage axiom (iv).
...
growth hypothesis: the formal proof applies the preimage axiom (iv) to the
