...
is threaded for the sufIdx in-slice bound).  The `D`-atom coordinate becomes the full
`(rs i).posAt mS t n = cellTupleF rs mS t n i`.
-/
...
open WRP Step SliceFamilyCell SliceDstarGA MSOMarkN SliceMarkN SliceMSO SliceFasGatesGA
  CopiedLandmark CopiedCells
...
/-- **The FULL-cell sigma-marked clause formula** (FG4): `clauseFormulaMk` with the
...
/-- **The FULL-cell sigma-marked clause characterization** (FG4): `clauseFMk_sat`
with the `D`-atom at the FULL cell coordinate `(rs i).posAt mS t n`.  All position
reasoning delegates to `regionDecodeLF_sat`; `hv` supplies the per-coord validity. -/
theorem clauseFMk_satF (M mthr : ℕ) (Sv Fv Bv : Finset ℕ)
    (hS : ∀ r ∈ Sv, r < M)
    (rs : Fin (P.toPoly.arity c') → RegionSpecF Bh)
    (sigma : Finset (Fin (P.toPoly.arity c')))
    ...
    have hcell : xOfL pb = fun i => if i ∈ sigma then dMarkMk P c c' ī i
      ...
      · simp only [if_neg hi]
        ...
        rw [if_neg hi, SliceFasGates.sat_relabelFO, comp_decodeLMk, hyFval, hyLval,
          htz] at hd
        exact (regionDecodeLF_sat (rs i) mS t n (xOfL pb i) hm hn hBn htn
          (by rw [← hlen]; exact hpb _) (hv i)).mp hd
...
/-- **The FULL-cell sigma-marked clause DFA**: `clauseDFAMk` over a `RegionSpecF` cell. -/
noncomputable def clauseDFAMkF (M mthr : ℕ) (Sv Fv Bv : Finset ℕ)
    (hS : ∀ r ∈ Sv, r < M)
    (rs : Fin (P.toPoly.arity c') → RegionSpecF Bh)
    (sigma : Finset (Fin (P.toPoly.arity c'))) :
    SliceMSO.DetAuto (MarkedN (P.toPoly.arity c + P.toPoly.arity c')) :=
  (MSOMarkN.markedDFAN_exists (P.toPoly.arity c + P.toPoly.arity c')
    (clauseFormulaMkF P c c' M mthr Sv Fv Bv hS rs sigma)).choose
...
/-- **The fibred TIE gate** (option A, the `cfgPosL`-cell mirror of
`SliceFasGatesGA.fasU_atomOrd_cellCfg_gate`): the atom of copy `c` at any valid tuple `ī` is
selected, `U`-labelled, and `atomOrd`-precedes every selected `D`-atom in full cell form
(`cfgCellGAFL`) — recognised by a single `markedDFAN`.  The `cfgPos↔cfgPosL` bridge to the
selector's `cfgCellGAF` is supplied by the TIE point bridge layer. -/
theorem fasU_atomOrd_cellCfg_gate_fibred (B Bh M mthr : ℕ)
    (S₁ F₁ K₁ : (c' : Fin P.toPoly.K) →
      (Fin (P.toPoly.arity c') → CopiedCells.RegionSpecF B) → Finset ℕ)
    (S₂ F₂ K₂ : (c' : Fin P.toPoly.K) →
      (Fin (P.toPoly.arity c') → CopiedCells.RegionSpecF Bh) → Finset ℕ)
      ...
      ∀ (n : ℕ) (ī : Fin (P.toPoly.arity c) → ℕ), Bh ≤ n →
        ...
        ((P.toPoly.sel c (copiedSlice mS n) ī
          ...
          ∧ ∀ b : P.toPoly.Atom, P.toPoly.selectedAtom (copiedSlice mS n) b →
              ...
              cfgCellGAFL B Bh M mthr S₁ F₁ K₁ S₂ F₂ K₂ mS n b →
        ...
        (MSO.Formula.and
          (MSOMarkN.andList ((List.finRange P.toPoly.K).flatMap (fun c' =>
            ((CopiedRank.regionTuplesF B (P.toPoly.arity c') mS).toList).map
              (fun rs => cellClauseF P c c' M mthr (S₁ c' rs) (F₁ c' rs) (K₁ c' rs)
  ...
  · rintro ⟨hsel, hlab, hord⟩
    refine ⟨((P.toPoly.selDef c).choose_spec (copiedSlice mS n) ī).mp hsel,
      ((P.toPoly.labelDef c U).choose_spec (copiedSlice mS n) ī).mp hlab, ?_, ?_⟩
    · rw [MSOMarkN.sat_andList]
      intro ψ hψ
      ...
      rw [cellClauseF_sat P c c' M mthr (S₁ c' rs) (F₁ c' rs) (K₁ c' rs)
  ...
  · rintro ⟨hsel, hlab, hord₁, hord₂⟩
    refine ⟨((P.toPoly.selDef c).choose_spec (copiedSlice mS n) ī).mpr hsel,
      ((P.toPoly.labelDef c U).choose_spec (copiedSlice mS n) ī).mpr hlab, ?_⟩
    rintro ⟨c', xb⟩ hbsel hbD (⟨rs, t, hrsv, hz, hbcell, hbcfg⟩ | ⟨rs, t, hrsv, hz, hbcell, hbcfg⟩)
    · rw [MSOMarkN.sat_andList] at hord₁
      have hclause := hord₁ (cellClauseF P c c' M mthr (S₁ c' rs) (F₁ c' rs) (K₁ c' rs)
          (fun r hr => (hS₁ c' rs r hr).1) rs)
