...
/-- **Residue-restricted lex-below count.**  Counting `j < N n` with `j ≡ a [mod m]` whose
rank line is `lexLtL`-below `T n` is `AffineOnResidues` in `n` — by reparametrising
`j = a + m·t` (a bijection onto the residue class) back to `countLexL` on the line
`(base + a·slope, m·slope)` with bound `⌈(N n − a)/m⌉`. -/
theorem countLexL_residue (a m : ℕ) (ha : a < m) (L : List (ℤ × ℤ)) (T : List (ℕ → ℤ))
    (hT : ∀ f ∈ T, AffineOnResiduesZ f) (N : ℕ → ℕ) (hN : AffineOnResidues N) :
    AffineOnResidues (fun n => ((Finset.range (N n)).filter
      (fun j : ℕ => j % m = a ∧ lexLtL (L.map (fun bp => bp.1 + (j : ℤ) * bp.2))
        (T.map (fun f => f n)))).card) := by
  have hm : 1 ≤ m := by omega
  have hmZ : (m : ℤ) ≠ 0 := by exact_mod_cast (by omega : m ≠ 0)
  set L' : List (ℤ × ℤ) := L.map (fun bp => (bp.1 + (a : ℤ) * bp.2, (m : ℤ) * bp.2)) with hL'
  set U' : ℕ → ℕ := fun n => (((N n : ℤ) - (a : ℤ) + (m : ℤ) - 1) / (m : ℤ)).toNat with hU'
  ...
  -- line reparametrisation
  have hline : ∀ t : ℕ, L'.map (fun bp => bp.1 + (t : ℤ) * bp.2)
      = L.map (fun bp => bp.1 + ((a + m * t : ℕ) : ℤ) * bp.2) := by
    intro t
    rw [hL', List.map_map]
  ...
  have hbound : ∀ t : ℕ, t < U' n ↔ a + m * t < N n := by
    intro t
    rw [hU', Int.lt_toNat]
    rw [show (((N n : ℤ) - (a : ℤ) + (m : ℤ) - 1) / (m : ℤ))
          = (((N n : ℤ) - (a : ℤ) + (m : ℤ) - 1) / (m : ℤ)) from rfl]
    constructor
    · intro h
      have h2 : ((t : ℤ) + 1) * (m : ℤ) ≤ (N n : ℤ) - (a : ℤ) + (m : ℤ) - 1 :=
        (Int.le_ediv_iff_mul_le (by omega : (0 : ℤ) < m)).mp (by omega)
      have e : ((t : ℤ) + 1) * (m : ℤ) = (m : ℤ) * (t : ℤ) + (m : ℤ) := by ring
      rw [e] at h2
      have : (m : ℤ) * (t : ℤ) = ((m * t : ℕ) : ℤ) := by push_cast; ring
      rw [this] at h2
      have h3 : ((a + m * t : ℕ) : ℤ) < (N n : ℤ) := by push_cast; push_cast at h2; omega
      exact_mod_cast h3
    · intro h
