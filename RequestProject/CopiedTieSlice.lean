/-
# The budgeted semantic bridge `tie_point_bridge_fibred_clause_budgeted_indexed`
  (§9 d4c, item 4 — the capstone integration)

The row-indexed gate family `GdfaF idx (n % pG)` whose acceptance (∧ rank = d*) characterizes the TIE
membership condition consumed by `CopiedCountTie.tie_count_fibred_of_gate_budgeted_indexed`.  Built from
the BANDED gate `CopiedBoundedGateBand.fasU_atomOrd_full_gate_bounded_band`, instantiated with the
budgeted selector emitters and finite activation sets recorded in `BridgeRowIndex`.
...
/-- Any scalarized tuple agrees with the update tuple at its distinguished
coordinate. -/
theorem tuple_eq_update_of_tupleFiberScalarization
    (P : WRP.Presentation Step Step) (hTuple : TupleFiberScalarization P)
    (c : Fin P.toPoly.K) (j0 : Fin (P.toPoly.arity c))
    (ī0 xb : Fin (P.toPoly.arity c) → ℕ) :
    xb = Function.update ī0 j0 (xb j0) :=
  (hTuple c j0 xb).trans
    (update_eq_const_of_tupleFiberScalarization P hTuple c j0 ī0 (xb j0)).symm

/-- The suffix cell representative stored in a row is the matching update-table
representative once tuple fibres scalarize. -/
theorem cellTupleF_sufIdx_eq_update_of_tupleFiberScalarization
    (P : WRP.Presentation Step Step) (hTuple : TupleFiberScalarization P)
    (c : Fin P.toPoly.K) (j0 : Fin (P.toPoly.arity c))
    {B : ℕ} (ī0 : Fin (P.toPoly.arity c) → ℕ) (mS n v : ℕ) :
    CopiedDstar.cellTupleF
        (fun _ : Fin (P.toPoly.arity c) =>
          (CopiedCells.RegionSpecF.sufIdx v : CopiedCells.RegionSpecF B))
        mS (B + 1) n =
      Function.update ī0 j0 (mS + 2 * n + 1 + v) := by
  calc
    CopiedDstar.cellTupleF
        (fun _ : Fin (P.toPoly.arity c) =>
          (CopiedCells.RegionSpecF.sufIdx v : CopiedCells.RegionSpecF B))
        ...
        = (fun _ : Fin (P.toPoly.arity c) =>
            (CopiedDstar.cellTupleF
              (fun _ : Fin (P.toPoly.arity c) =>
                (CopiedCells.RegionSpecF.sufIdx v : CopiedCells.RegionSpecF B))
              mS (B + 1) n) j0) := hTuple c j0 _
    _ = (fun _ : Fin (P.toPoly.arity c) => mS + 2 * n + 1 + v) := by
      funext i
      simp [CopiedDstar.cellTupleF, CopiedCells.RegionSpecF.posAt]
    _ = Function.update ī0 j0 (mS + 2 * n + 1 + v) :=
      (update_eq_const_of_tupleFiberScalarization P hTuple c j0 ī0
        (mS + 2 * n + 1 + v)).symm

/-- Prefix twin of `cellTupleF_sufIdx_eq_update_of_tupleFiberScalarization`. -/
theorem cellTupleF_prefIdx_eq_update_of_tupleFiberScalarization
    (P : WRP.Presentation Step Step) (hTuple : TupleFiberScalarization P)
    (c : Fin P.toPoly.K) (j0 : Fin (P.toPoly.arity c))
    {B : ℕ} (ī0 : Fin (P.toPoly.arity c) → ℕ) (mS n v : ℕ) :
    CopiedDstar.cellTupleF
        (fun _ : Fin (P.toPoly.arity c) =>
          (CopiedCells.RegionSpecF.prefIdx v : CopiedCells.RegionSpecF B))
        mS (B + 1) n =
      Function.update ī0 j0 v := by
  calc
    CopiedDstar.cellTupleF
        (fun _ : Fin (P.toPoly.arity c) =>
          (CopiedCells.RegionSpecF.prefIdx v : CopiedCells.RegionSpecF B))
        ...
        = (fun _ : Fin (P.toPoly.arity c) =>
            (CopiedDstar.cellTupleF
              (fun _ : Fin (P.toPoly.arity c) =>
                (CopiedCells.RegionSpecF.prefIdx v : CopiedCells.RegionSpecF B))
              mS (B + 1) n) j0) := hTuple c j0 _
    _ = (fun _ : Fin (P.toPoly.arity c) => v) := by
      funext i
      simp [CopiedDstar.cellTupleF, CopiedCells.RegionSpecF.posAt]
    _ = Function.update ī0 j0 v :=
      (update_eq_const_of_tupleFiberScalarization P hTuple c j0 ī0 v).symm

/-- Abstract coverage for the CORE/shallow/deep-boundary part of the semantic
split.  This is the first component of `tieSemanticSplitAt`; unlike
`TupleFiberScalarization`, it is a descriptor-level obligation that an
arbitrary-arity bridge can supply directly. -/
...
/-- The bulk cfg witness for an atom is bounded by the fixed row descriptor
window.  This is exactly the first callback consumed by the CORE branch. -/
def BulkCfgWitnessBounded
    {P : WRP.Presentation Step Step} {B M mthr qB mS n : ℕ}
    (S1 : (c' : Fin P.toPoly.K) →
      (Fin (P.toPoly.arity c') → CopiedCells.RegionSpecF B) → Finset ℕ)
    (b : P.toPoly.Atom) : Prop :=
    ∀ (rs : Fin (P.toPoly.arity b.1) → CopiedCells.RegionSpecF B) (t : ℕ),
      (∀ i, (rs i).valid mS) → t < n →
      b.2 = CopiedDstar.cellTupleF rs mS t n →
      CopiedTieGate.cfgPosL M mthr (S1 b.1 rs) ∅ ∅
...
/-- Frozen half of `BulkCfgWitnessBounded`. -/
def FrozenCfgWitnessBounded
    {P : WRP.Presentation Step Step} {Bh M mthr qB mS n : ℕ}
    (F2 : (c' : Fin P.toPoly.K) →
      (Fin (P.toPoly.arity c') → CopiedCells.RegionSpecF Bh) → Finset ℕ)
    (b : P.toPoly.Atom) : Prop :=
    ∀ (rs : Fin (P.toPoly.arity b.1) → CopiedCells.RegionSpecF Bh) (t : ℕ),
...
/-- Positive witness that the bulk cfg branch left the fixed descriptor
window.  This is the mixed/unbounded branch that remains after the bounded
CORE callback is used. -/
def BulkCfgUnboundedWitness
    {P : WRP.Presentation Step Step} {B M mthr qB mS n : ℕ}
    (S1 : (c' : Fin P.toPoly.K) →
      (Fin (P.toPoly.arity c') → CopiedCells.RegionSpecF B) → Finset ℕ)
    (b : P.toPoly.Atom) : Prop :=
...
/-- Frozen-half unbounded cfg witness. -/
def FrozenCfgUnboundedWitness
    {P : WRP.Presentation Step Step} {Bh M mthr qB mS n : ℕ}
    (F2 : (c' : Fin P.toPoly.K) →
      (Fin (P.toPoly.arity c') → CopiedCells.RegionSpecF Bh) → Finset ℕ)
    (b : P.toPoly.Atom) : Prop :=
...
/-- A descriptor tuple leaves the fixed bounded window in either the prefix or
suffix stretch. -/
def DescriptorOutOfBoundWitness {B k : ℕ} (q_U q_D : ℕ)
    (rs : Fin k → CopiedCells.RegionSpecF B) : Prop :=
    (∃ (i : Fin k) (q : ℕ), rs i = CopiedCells.RegionSpecF.prefIdx q ∧ q_U ≤ q)
      ∨ ∃ (i : Fin k) (l : ℕ), rs i = CopiedCells.RegionSpecF.sufIdx l ∧ q_D ≤ l

/-- Non-membership in `boundedTuplesF` is exactly caused by an explicit
out-of-bound prefix/suffix descriptor coordinate. -/
theorem descriptorOutOfBoundWitness_of_not_boundedTuplesF
    {B k q_U q_D : ℕ} {rs : Fin k → CopiedCells.RegionSpecF B}
    (hnot : rs ∉ CopiedBoundedGate.boundedTuplesF B k q_U q_D) :
    DescriptorOutOfBoundWitness q_U q_D rs := by
  rw [CopiedBoundedGate.mem_boundedTuplesF] at hnot
  push Not at hnot
  ...
  | prefIdx q =>
      left
      exact ⟨i, q, hrs, by simpa [hrs] using hi⟩
  | sufIdx l =>
      right
      exact ⟨i, l, hrs, by simpa [hrs] using hi⟩

/-- A tuple has an actual deep prefix coordinate. -/
def TupleDeepPrefixCoordinate {k : ℕ} (q_U mS : ℕ) (xb : Fin k → ℕ) : Prop :=
    ∃ (i : Fin k) (q : ℕ), xb i = q ∧ q_U ≤ q ∧ q < mS - 1

/-- A tuple has an actual deep suffix coordinate. -/
def TupleDeepSuffixCoordinate {k : ℕ} (q_D mS n : ℕ) (xb : Fin k → ℕ) : Prop :=
    ∃ (i : Fin k) (l : ℕ),
      xb i = mS + 2 * n + 1 + l ∧ q_D ≤ l ∧ l < mS - 1

/-- A tuple is exactly the zero-base distinguished-coordinate update at a deep
prefix coordinate.  This is the canonical shape consumed by the zero-base
update bridge callbacks. -/
def TupleDeepPrefixChoiceCoordinate {k : ℕ} (j0 : Fin k) (q_U mS : ℕ)
    (xb : Fin k → ℕ) : Prop :=
    ∃ q : ℕ, xb = Function.update (fun _ : Fin k => 0) j0 q ∧ q_U ≤ q ∧ q < mS - 1

/-- A tuple is exactly the zero-base distinguished-coordinate update at a deep
suffix coordinate. -/
def TupleDeepSuffixChoiceCoordinate {k : ℕ} (j0 : Fin k) (q_D mS n : ℕ)
    (xb : Fin k → ℕ) : Prop :=
    ∃ l : ℕ, xb = Function.update (fun _ : Fin k => 0) j0 (mS + 2 * n + 1 + l) ∧
      q_D ≤ l ∧ l < mS - 1

/-- The concrete tuple coordinate produced by an out-of-bound descriptor: an
actual prefix position `q`, or an actual suffix position `mS + 2*n + 1 + l`. -/
def TupleDeepBoundaryCoordinate {k : ℕ} (q_U q_D mS n : ℕ)
    (xb : Fin k → ℕ) : Prop :=
    TupleDeepPrefixCoordinate q_U mS xb ∨ TupleDeepSuffixCoordinate q_D mS n xb

/-- A tuple is a deep boundary tuple at the chosen zero-base update coordinate. -/
def TupleDeepChoiceBoundaryCoordinate {k : ℕ} (j0 : Fin k) (q_U q_D mS n : ℕ)
    (xb : Fin k → ℕ) : Prop :=
    TupleDeepPrefixChoiceCoordinate j0 q_U mS xb ∨
      TupleDeepSuffixChoiceCoordinate j0 q_D mS n xb

/-- A global distinguished-coordinate choice, one coordinate for each copy. -/
abbrev CoordChoice (P : WRP.Presentation Step Step) :=
  (c : Fin P.toPoly.K) → Fin (P.toPoly.arity c)

noncomputable instance coordChoiceFintype (P : WRP.Presentation Step Step) :
    Fintype (CoordChoice P) := by
  dsimp [CoordChoice]
  infer_instance

noncomputable instance coordChoiceDecidableEq (P : WRP.Presentation Step Step) :
    DecidableEq (CoordChoice P) := by
  classical
  dsimp [CoordChoice]
  infer_instance

/-- Evaluating a cell tuple turns an out-of-bound descriptor coordinate into
the corresponding concrete prefix/suffix coordinate of the atom tuple. -/
theorem tupleDeepBoundaryCoordinate_of_descriptorOutOfBound
    {B k q_U q_D mS n t : ℕ}
    {rs : Fin k → CopiedCells.RegionSpecF B} {xb : Fin k → ℕ}
    (hvalid : ∀ i, (rs i).valid mS)
    (hcell : xb = CopiedDstar.cellTupleF rs mS t n)
    (hout : DescriptorOutOfBoundWitness q_U q_D rs) :
    TupleDeepBoundaryCoordinate q_U q_D mS n xb := by
  rcases hout with ⟨i, q, hrs, hq⟩ | ⟨i, l, hrs, hl⟩
  · left
    refine ⟨i, q, ?_, hq, ?_⟩
    · have hcelli := congrFun hcell i
      simpa [CopiedDstar.cellTupleF, CopiedCells.RegionSpecF.posAt, hrs] using hcelli
    · have hvalidi := hvalid i
      simpa [CopiedCells.RegionSpecF.valid, hrs] using hvalidi
  · right
    refine ⟨i, l, ?_, hl, ?_⟩
    · have hcelli := congrFun hcell i
      simpa [CopiedDstar.cellTupleF, CopiedCells.RegionSpecF.posAt, hrs] using hcelli
...
/-- Bulk cfg witness with its out-of-bound descriptor coordinate exposed. -/
def BulkCfgDeepBoundaryWitness
    {P : WRP.Presentation Step Step} {B M mthr qB mS n : ℕ}
    (S1 : (c' : Fin P.toPoly.K) →
      (Fin (P.toPoly.arity c') → CopiedCells.RegionSpecF B) → Finset ℕ)
    (b : P.toPoly.Atom) : Prop :=
    ∃ (rs : Fin (P.toPoly.arity b.1) → CopiedCells.RegionSpecF B) (t : ℕ),
      ...
      CopiedTieGate.cfgPosL M mthr (S1 b.1 rs) ∅ ∅
        (mS + 1) (mS + 2 * (n - 1)) (mS + 2 * t) ∧
      DescriptorOutOfBoundWitness qB qB rs

/-- Frozen cfg witness with its out-of-bound descriptor coordinate exposed. -/
def FrozenCfgDeepBoundaryWitness
    {P : WRP.Presentation Step Step} {Bh M mthr qB mS n : ℕ}
    (F2 : (c' : Fin P.toPoly.K) →
      (Fin (P.toPoly.arity c') → CopiedCells.RegionSpecF Bh) → Finset ℕ)
    (b : P.toPoly.Atom) : Prop :=
...
/-- Bulk cfg witness with both the offending descriptor coordinate and its
concrete tuple-coordinate equation exposed. -/
def BulkCfgTupleDeepBoundaryWitness
    {P : WRP.Presentation Step Step} {B M mthr qB mS n : ℕ}
    (S1 : (c' : Fin P.toPoly.K) →
      (Fin (P.toPoly.arity c') → CopiedCells.RegionSpecF B) → Finset ℕ)
    (b : P.toPoly.Atom) : Prop :=
    ∃ (rs : Fin (P.toPoly.arity b.1) → CopiedCells.RegionSpecF B) (t : ℕ),
      ...
      CopiedTieGate.cfgPosL M mthr (S1 b.1 rs) ∅ ∅
        (mS + 1) (mS + 2 * (n - 1)) (mS + 2 * t) ∧
      DescriptorOutOfBoundWitness qB qB rs ∧
      TupleDeepBoundaryCoordinate qB qB mS n b.2

/-- Frozen cfg witness with both the offending descriptor coordinate and its
concrete tuple-coordinate equation exposed. -/
def FrozenCfgTupleDeepBoundaryWitness
    {P : WRP.Presentation Step Step} {Bh M mthr qB mS n : ℕ}
    (F2 : (c' : Fin P.toPoly.K) →
      (Fin (P.toPoly.arity c') → CopiedCells.RegionSpecF Bh) → Finset ℕ)
    (b : P.toPoly.Atom) : Prop :=
...
/-- A deep bulk witness canonically exposes the concrete tuple coordinate
specified by its descriptor coordinate. -/
theorem bulkCfgTupleDeepBoundaryWitness_of_deepBoundary
    {P : WRP.Presentation Step Step} {B M mthr qB mS n : ℕ}
    {S1 : (c' : Fin P.toPoly.K) →
      (Fin (P.toPoly.arity c') → CopiedCells.RegionSpecF B) → Finset ℕ}
    {b : P.toPoly.Atom}
    (h : BulkCfgDeepBoundaryWitness (P := P) (B := B) (M := M) (mthr := mthr)
      (qB := qB) (mS := mS) (n := n) S1 b) :
    BulkCfgTupleDeepBoundaryWitness (P := P) (B := B) (M := M) (mthr := mthr)
      (qB := qB) (mS := mS) (n := n) S1 b := by
  rcases h with ⟨rs, t, hvalid, htn, hcell, hpos, hout⟩
  exact ⟨rs, t, hvalid, htn, hcell, hpos, hout,
    tupleDeepBoundaryCoordinate_of_descriptorOutOfBound hvalid hcell hout⟩

/-- A deep frozen witness canonically exposes the concrete tuple coordinate
specified by its descriptor coordinate. -/
theorem frozenCfgTupleDeepBoundaryWitness_of_deepBoundary
    {P : WRP.Presentation Step Step} {Bh M mthr qB mS n : ℕ}
    {F2 : (c' : Fin P.toPoly.K) →
      (Fin (P.toPoly.arity c') → CopiedCells.RegionSpecF Bh) → Finset ℕ}
    {b : P.toPoly.Atom}
    (h : FrozenCfgDeepBoundaryWitness (P := P) (Bh := Bh) (M := M) (mthr := mthr)
      (qB := qB) (mS := mS) (n := n) F2 b) :
    FrozenCfgTupleDeepBoundaryWitness (P := P) (Bh := Bh) (M := M) (mthr := mthr)
      (qB := qB) (mS := mS) (n := n) F2 b := by
  rcases h with ⟨rs, t, hvalid, htn, hcell, hpos, hout⟩
  exact ⟨rs, t, hvalid, htn, hcell, hpos, hout,
    tupleDeepBoundaryCoordinate_of_descriptorOutOfBound hvalid hcell hout⟩

/-- Unbounded bulk cfg witnesses can be sharpened to expose the offending
descriptor coordinate. -/
theorem bulkCfgDeepBoundaryWitness_of_unbounded
    {P : WRP.Presentation Step Step} {B M mthr qB mS n : ℕ}
    {S1 : (c' : Fin P.toPoly.K) →
      (Fin (P.toPoly.arity c') → CopiedCells.RegionSpecF B) → Finset ℕ}
    {b : P.toPoly.Atom}
    (h : BulkCfgUnboundedWitness (P := P) (B := B) (M := M) (mthr := mthr)
      (qB := qB) (mS := mS) (n := n) S1 b) :
    BulkCfgDeepBoundaryWitness (P := P) (B := B) (M := M) (mthr := mthr)
      (qB := qB) (mS := mS) (n := n) S1 b := by
  rcases h with ⟨rs, t, hvalid, htn, hcell, hpos, hnotBound⟩
  exact ⟨rs, t, hvalid, htn, hcell, hpos,
    descriptorOutOfBoundWitness_of_not_boundedTuplesF hnotBound⟩

/-- Unbounded frozen cfg witnesses can be sharpened to expose the offending
descriptor coordinate. -/
theorem frozenCfgDeepBoundaryWitness_of_unbounded
    {P : WRP.Presentation Step Step} {Bh M mthr qB mS n : ℕ}
    {F2 : (c' : Fin P.toPoly.K) →
      (Fin (P.toPoly.arity c') → CopiedCells.RegionSpecF Bh) → Finset ℕ}
    {b : P.toPoly.Atom}
    (h : FrozenCfgUnboundedWitness (P := P) (Bh := Bh) (M := M) (mthr := mthr)
      (qB := qB) (mS := mS) (n := n) F2 b) :
    FrozenCfgDeepBoundaryWitness (P := P) (Bh := Bh) (M := M) (mthr := mthr)
      (qB := qB) (mS := mS) (n := n) F2 b := by
  rcases h with ⟨rs, t, hvalid, htn, hcell, hpos, hnotBound⟩
  exact ⟨rs, t, hvalid, htn, hcell, hpos,
    descriptorOutOfBoundWitness_of_not_boundedTuplesF hnotBound⟩

/-- Failure of the bulk bounded callback exposes an unbounded bulk cfg
witness. -/
theorem bulkCfgUnboundedWitness_of_not_bounded
    {P : WRP.Presentation Step Step} {B M mthr qB mS n : ℕ}
    {S1 : (c' : Fin P.toPoly.K) →
      (Fin (P.toPoly.arity c') → CopiedCells.RegionSpecF B) → Finset ℕ}
    {b : P.toPoly.Atom}
    (h : ¬ BulkCfgWitnessBounded (P := P) (B := B) (M := M)
      (mthr := mthr) (qB := qB) (mS := mS) (n := n) S1 b) :
    BulkCfgUnboundedWitness (P := P) (B := B) (M := M) (mthr := mthr)
      (qB := qB) (mS := mS) (n := n) S1 b := by
  classical
  rw [BulkCfgWitnessBounded] at h
  push Not at h
  rcases h with ⟨rs, t, hvalid, htn, hcell, hpos, hnotBound⟩
  exact ⟨rs, t, hvalid, htn, hcell, hpos, hnotBound⟩

/-- Failure of the frozen bounded callback exposes an unbounded frozen cfg
witness. -/
theorem frozenCfgUnboundedWitness_of_not_bounded
    {P : WRP.Presentation Step Step} {Bh M mthr qB mS n : ℕ}
    {F2 : (c' : Fin P.toPoly.K) →
      (Fin (P.toPoly.arity c') → CopiedCells.RegionSpecF Bh) → Finset ℕ}
    {b : P.toPoly.Atom}
    (h : ¬ FrozenCfgWitnessBounded (P := P) (Bh := Bh) (M := M)
      (mthr := mthr) (qB := qB) (mS := mS) (n := n) F2 b) :
    FrozenCfgUnboundedWitness (P := P) (Bh := Bh) (M := M) (mthr := mthr)
      (qB := qB) (mS := mS) (n := n) F2 b := by
  classical
  rw [FrozenCfgWitnessBounded] at h
  push Not at h
  rcases h with ⟨rs, t, hvalid, htn, hcell, hpos, hnotBound⟩
  exact ⟨rs, t, hvalid, htn, hcell, hpos, hnotBound⟩

/-- Supplier for the genuinely hard part of zero-base core-boundary coverage:
selected `D` achievers whose selector cfg witness leaves the fixed descriptor
window.  Bounded cfg witnesses are discharged separately by the existing CORE
callback. -/
def CoreBoundaryUpdateZeroUnboundedCfgCoverage
    ...
    ∀ {B Bh M mthr qB Q mS n Nc mx : ℕ}
      ...
      ∀ b : P.toPoly.Atom,
        ...
        CopiedTieGate.cfgCellGAFL B Bh M mthr S1
          (fun _ _ => ∅) (fun _ _ => ∅) (fun _ _ => ∅) F2 (fun _ _ => ∅)
          mS n b →
        (BulkCfgUnboundedWitness (P := P) (B := B) (M := M)
            (mthr := mthr) (qB := qB) (mS := mS) (n := n) S1 b ∨
          FrozenCfgUnboundedWitness (P := P) (Bh := Bh) (M := M)
            (mthr := mthr) (qB := qB) (mS := mS) (n := n) F2 b) →
        P.toPoly.atomOrd (copiedSlice mS n) ⟨c, ī⟩ b

/-- Sharper core-boundary supplier: the hard cfg branch is presented with an
explicit out-of-bound prefix/suffix descriptor coordinate. -/
def CoreBoundaryUpdateZeroDeepCfgCoverage
    (P : WRP.Presentation Step Step) (hV : P.Valid) : Prop :=
    ∀ {B Bh M mthr qB Q mS n Nc mx : ℕ}
      {pcF Ts Tp : Fin P.toPoly.K → ℕ}
      (j0F : (c : Fin P.toPoly.K) → Fin (P.toPoly.arity c))
      ...
      ∀ b : P.toPoly.Atom,
        ...
        CopiedTieGate.cfgCellGAFL B Bh M mthr S1
          (fun _ _ => ∅) (fun _ _ => ∅) (fun _ _ => ∅) F2 (fun _ _ => ∅)
          mS n b →
        (BulkCfgDeepBoundaryWitness (P := P) (B := B) (M := M)
            (mthr := mthr) (qB := qB) (mS := mS) (n := n) S1 b ∨
          FrozenCfgDeepBoundaryWitness (P := P) (Bh := Bh) (M := M)
            (mthr := mthr) (qB := qB) (mS := mS) (n := n) F2 b) →
        P.toPoly.atomOrd (copiedSlice mS n) ⟨c, ī⟩ b

/-- Sharpest current core-boundary supplier interface: the hard cfg branch
comes with both an out-of-bound descriptor coordinate and its concrete
prefix/suffix coordinate in the atom tuple. -/
def CoreBoundaryUpdateZeroTupleDeepCfgCoverage
    (P : WRP.Presentation Step Step) (hV : P.Valid) : Prop :=
    ∀ {B Bh M mthr qB Q mS n Nc mx : ℕ}
      {pcF Ts Tp : Fin P.toPoly.K → ℕ}
      (j0F : (c : Fin P.toPoly.K) → Fin (P.toPoly.arity c))
      ...
      ∀ b : P.toPoly.Atom,
        ...
        CopiedTieGate.cfgCellGAFL B Bh M mthr S1
          (fun _ _ => ∅) (fun _ _ => ∅) (fun _ _ => ∅) F2 (fun _ _ => ∅)
          mS n b →
        (BulkCfgTupleDeepBoundaryWitness (P := P) (B := B) (M := M)
            (mthr := mthr) (qB := qB) (mS := mS) (n := n) S1 b ∨
          FrozenCfgTupleDeepBoundaryWitness (P := P) (Bh := Bh) (M := M)
            (mthr := mthr) (qB := qB) (mS := mS) (n := n) F2 b) →
        P.toPoly.atomOrd (copiedSlice mS n) ⟨c, ī⟩ b

/-- Generic cfg-free semantic core-boundary supplier parameterized by the
coordinate predicate carried by the hard selected-`D` achiever. -/
def CoreBoundaryUpdateZeroTupleCoordinateCoverageFor
    (P : WRP.Presentation Step Step) (hV : P.Valid)
    (Coord : {k : ℕ} → ℕ → ℕ → ℕ → (Fin k → ℕ) → Prop) : Prop :=
    ∀ {qB Q mS n Nc mx : ℕ}
      {pcF Ts Tp : Fin P.toPoly.K → ℕ}
      (j0F : (c : Fin P.toPoly.K) → Fin (P.toPoly.arity c))
      {c : Fin P.toPoly.K} {ī : Fin (P.toPoly.arity c) → ℕ},
...
/-- Generic cfg-free semantic supplier whose coordinate predicate may depend on
the distinguished zero-base update coordinate chosen by `j0F`.  This isolates
the part of the arbitrary-arity problem already handled by the update bridge
callbacks: once the exposed deep coordinate is known to be the chosen update
coordinate, the canonical callbacks apply directly. -/
def CoreBoundaryUpdateZeroTupleChoiceCoordinateCoverageFor
    (P : WRP.Presentation Step Step) (hV : P.Valid)
    (Coord : {k : ℕ} → Fin k → ℕ → ℕ → ℕ → (Fin k → ℕ) → Prop) : Prop :=
    ∀ {qB Q mS n Nc mx : ℕ}
      {pcF Ts Tp : Fin P.toPoly.K → ℕ}
      (j0F : (c : Fin P.toPoly.K) → Fin (P.toPoly.arity c))
      {c : Fin P.toPoly.K} {ī : Fin (P.toPoly.arity c) → ℕ},
      ...
      ∀ b : P.toPoly.Atom,
        ...
        P.toPoly.labelOf (copiedSlice mS n) b = D →
        P.rankOf (copiedSlice mS n) b = CopiedDstar.dstarRankGA_m P hV mS n →
        Coord (j0F b.1) qB mS n b.2 →
        P.toPoly.atomOrd (copiedSlice mS n) ⟨c, ī⟩ b

/-- Prefix-coordinate half in the exact distinguished zero-update shape. -/
def CoreBoundaryUpdateZeroTuplePrefixChoiceCoverage
    (P : WRP.Presentation Step Step) (hV : P.Valid) : Prop :=
    CoreBoundaryUpdateZeroTupleChoiceCoordinateCoverageFor P hV
      (fun {k} j0 qB mS _n (xb : Fin k → ℕ) =>
        TupleDeepPrefixChoiceCoordinate j0 qB mS xb)

/-- Suffix-coordinate half in the exact distinguished zero-update shape. -/
def CoreBoundaryUpdateZeroTupleSuffixChoiceCoverage
    (P : WRP.Presentation Step Step) (hV : P.Valid) : Prop :=
    CoreBoundaryUpdateZeroTupleChoiceCoordinateCoverageFor P hV
      (fun {k} j0 qB mS n (xb : Fin k → ℕ) =>
        TupleDeepSuffixChoiceCoordinate j0 qB mS n xb)

/-- The explicit deep-coordinate supplier forgets to the older unbounded-cfg
supplier by extracting that coordinate from `rs ∉ boundedTuplesF`. -/
theorem coreBoundaryUpdateZeroUnboundedCfgCoverage_of_deepCfgCoverage
    (P : WRP.Presentation Step Step) (hV : P.Valid)
    (hDeep : CoreBoundaryUpdateZeroDeepCfgCoverage P hV) :
    CoreBoundaryUpdateZeroUnboundedCfgCoverage P hV := by
  intro B Bh M mthr qB Q mS n Nc mx pcF Ts Tp j0F S1 F2 c ī
    hm hqB_lt hQ1 hqB_def hpcF_dvd_Q hTs_le_mx hTp_le_mx hNcmx
    hdeep_suf_order hdeep_pre_order b hnotS hnotP hbsel hbD hbach hcfg
    hunbounded
  have hdeepWitness :
      BulkCfgDeepBoundaryWitness (P := P) (B := B) (M := M) (mthr := mthr)
          (qB := qB) (mS := mS) (n := n) S1 b ∨
        FrozenCfgDeepBoundaryWitness (P := P) (Bh := Bh) (M := M) (mthr := mthr)
          (qB := qB) (mS := mS) (n := n) F2 b := by
    rcases hunbounded with hbulk | hfrozen
    · exact Or.inl (bulkCfgDeepBoundaryWitness_of_unbounded hbulk)
    · exact Or.inr (frozenCfgDeepBoundaryWitness_of_unbounded hfrozen)
  exact hDeep j0F hm hqB_lt hQ1 hqB_def hpcF_dvd_Q hTs_le_mx hTp_le_mx hNcmx
    hdeep_suf_order hdeep_pre_order b hnotS hnotP hbsel hbD hbach hcfg hdeepWitness

/-- If the unbounded cfg-witness cases are supplied, the full zero-base
core-boundary package follows: bounded cfg witnesses use `hcore_from_cfg`, and
only genuinely unbounded/mixed witnesses are delegated. -/
theorem coreBoundaryUpdateZeroCoverage_of_unboundedCfgCoverage
    (P : WRP.Presentation Step Step) (hV : P.Valid)
    (hUnbounded : CoreBoundaryUpdateZeroUnboundedCfgCoverage P hV) :
  ...
  have hcfg := hcfg_of_rank b hselD.1 hselD.2 hbach
  by_cases hbulk :
      BulkCfgWitnessBounded (P := P) (B := B) (M := M) (mthr := mthr)
        (qB := qB) (mS := mS) (n := n) S1 b
  · by_cases hfrozen :
        FrozenCfgWitnessBounded (P := P) (Bh := Bh) (M := M) (mthr := mthr)
          (qB := qB) (mS := mS) (n := n) F2 b
    · exact hcore_from_cfg b hselD.1 hselD.2 hcfg hbulk hfrozen
    · exact hUnbounded j0F hm hqB_lt hQ1 hqB_def hpcF_dvd_Q hTs_le_mx hTp_le_mx
        hNcmx hdeep_suf_order hdeep_pre_order b hnotS hnotP hselD.1 hselD.2 hbach
        hcfg (Or.inr (frozenCfgUnboundedWitness_of_not_bounded hfrozen))
  · exact hUnbounded j0F hm hqB_lt hQ1 hqB_def hpcF_dvd_Q hTs_le_mx hTp_le_mx
      hNcmx hdeep_suf_order hdeep_pre_order b hnotS hnotP hselD.1 hselD.2 hbach
      hcfg (Or.inl (bulkCfgUnboundedWitness_of_not_bounded hbulk))
...
/-- Package the update-shaped core/boundary supplier in the exact CORE branch
shape required by `whole_order_of_update_split`.  This keeps the direct bridge
rewrite from re-spelling the long `CoreBoundaryUpdateCoverage` application at
the final semantic split. -/
theorem core_order_of_update_boundary
    (P : WRP.Presentation Step Step) (hV : P.Valid)
    ...
    ∀ b : P.toPoly.Atom,
      ...
      P.rankOf (copiedSlice mS n) b = CopiedDstar.dstarRankGA_m P hV mS n →
      P.toPoly.atomOrd (copiedSlice mS n) ⟨c0, ī0m⟩ b :=
  hCoreBoundary ī0F j0F hm hqB_lt hQ1 hqB_def hpcF_dvd_Q hTs_le_mx
    hTp_le_mx hNcmx hcore_from_cfg hdeep_suf_order hdeep_pre_order hcfg_of_rank

/-- Zero-base version of `core_order_of_update_boundary`, matching the
canonical bridge row index whose untouched coordinates are all zero. -/
...
/-- Update-shaped core-boundary coverage forgets to the legacy constant-tuple
interface under tuple-fibre scalarization. -/
theorem coreBoundaryCoverage_of_update_of_tupleFiberScalarization
    (P : WRP.Presentation Step Step) (hV : P.Valid)
    (hCore : CoreBoundaryUpdateCoverage P hV)
    (hTuple : TupleFiberScalarization P) :
    CoreBoundaryCoverage P hV := by
  intro B Bh M mthr qB Q mS n Nc mx pcF Ts Tp j0F S1 F2 c ī
    ...
    hcore_from_cfg hdeep_suf_order hdeep_pre_order hcfg_of_rank
    b hnotS hnotP hselD hbach
  let ī0F : (c : Fin P.toPoly.K) → Fin (P.toPoly.arity c) → ℕ := fun _ _ => 0
  exact hCore ī0F j0F hm hqB_lt hQ1 hqB_def hpcF_dvd_Q hTs_le_mx hTp_le_mx
    hNcmx hcore_from_cfg
    (by
      intro c' l hl hqBl hnotHi hselRun hachRun
      have hupd :=
        update_eq_const_of_tupleFiberScalarization P hTuple c' (j0F c') (ī0F c')
      ...
      have hachRunC :
          P.rank c' (copiedSlice mS n)
            ...
            = CopiedDstar.dstarRankGA_m P hV mS n := by
        simpa [hupd] using hachRun
      have hord := hdeep_suf_order c' l hl hqBl hnotHi hselRunC hachRunC
      simpa [hupd] using hord)
    (by
      intro c' l hl hqBl hnotHi hselRun hachRun
      have hupd :=
        update_eq_const_of_tupleFiberScalarization P hTuple c' (j0F c') (ī0F c') l
      have hselRunC :
          P.toPoly.sel c' (copiedSlice mS n) (fun _ : Fin (P.toPoly.arity c') => l)
            ∧ P.toPoly.label c' (copiedSlice mS n)
              (fun _ : Fin (P.toPoly.arity c') => l) = D := by
        simpa [hupd] using hselRun
      have hachRunC :
          P.rank c' (copiedSlice mS n) (fun _ : Fin (P.toPoly.arity c') => l)
            = CopiedDstar.dstarRankGA_m P hV mS n := by
        simpa [hupd] using hachRun
      have hord := hdeep_pre_order c' l hl hqBl hnotHi hselRunC hachRunC
      simpa [hupd] using hord)
    hcfg_of_rank b
    (by
      intro hsuf
...
/-- Abstract coverage for the suffix/prefix run-residue split branches.  The
current supplier is assembled from the arity-1 achiever-locus analysis plus
activation completeness; the arbitrary-arity route should replace this package
with descriptor-level run-residue coverage. -/
def RunResidueAffineCoverage (P : WRP.Presentation Step Step) (hV : P.Valid) : Prop :=
    CopiedAchieverLocus.DstarAchieverAffineLocus P hV ∧
    CopiedBoundedGateBand.BandedActivationCompleteness P hV
...
/-- Legacy bridge-facing run-residue coverage.  Its achiever branch is scalarized to constant
tuples; `RunResidueAffineCoverage` above records the table-valued half that the general route can
reuse without arity-one scalarization. -/
def RunResidueCoverage (P : WRP.Presentation Step Step) (hV : P.Valid) : Prop :=
    CopiedAchieverLocus.DstarAchieverLocus P hV ∧
    CopiedBoundedGateBand.BandedActivationCompleteness P hV

/-- Forget update-tuple run-class coverage to the legacy constant-tuple
interface under tuple-fibre scalarization. -/
theorem runClassUniformCoverage_of_update_of_tupleFiberScalarization
    (P : WRP.Presentation Step Step) (hV : P.Valid)
    (hUpdate : CopiedSelUniform.RunClassUpdateUniformCoverage P hV)
    (hTuple : TupleFiberScalarization P) :
    CopiedSelUniform.RunClassUniformCoverage P hV := by
  intro c j0 ī0 mS n pc M Nc F hpc hF posOf hposval hag hselconst hNcmS
    l_b hl_b_lo hl_bNc hselb hach l' hMl' hl'mS hres
  have hupdconst : ∀ v,
      Function.update ī0 j0 v = fun _ : Fin (P.toPoly.arity c) => v := by
    intro v
    exact update_eq_const_of_tupleFiberScalarization P hTuple c j0 ī0 v
  have hupdval :
      ∀ l0, l0 < mS - 1 →
        ∀ i, Function.update ī0 j0 (posOf l0) i < (copiedSlice mS n).length := by
    intro l0 hl0 i
    have hi := congrFun (hupdconst (posOf l0)) i
    rw [hi]
    exact hposval l0 hl0
  have hselconstU :
      ∀ l₁ l₂, M ≤ l₁ → l₁ < Nc → M ≤ l₂ → l₂ < Nc →
        (l₁ - M) % pc = (l₂ - M) % pc →
        ((P.toPoly.sel c (copiedSlice mS n) (Function.update ī0 j0 (posOf l₁))
          ...
          ↔ (P.toPoly.sel c (copiedSlice mS n) (Function.update ī0 j0 (posOf l₂))
            ∧ P.toPoly.label c (copiedSlice mS n)
              (Function.update ī0 j0 (posOf l₂)) = D)) := by
    intro l₁ l₂ hM₁ hNc₁ hM₂ hNc₂ hmod
    simpa [hupdconst (posOf l₁), hupdconst (posOf l₂)] using
      hselconst l₁ l₂ hM₁ hNc₁ hM₂ hNc₂ hmod
  have hselbU :
      P.toPoly.sel c (copiedSlice mS n) (Function.update ī0 j0 (posOf l_b))
        ∧ P.toPoly.label c (copiedSlice mS n)
          (Function.update ī0 j0 (posOf l_b)) = D := by
    simpa [hupdconst (posOf l_b)] using hselb
  have hagb_const :
      P.rank c (copiedSlice mS n) (fun _ : Fin (P.toPoly.arity c) => posOf l_b) =
        F l_b := by
    simpa [hupdconst (posOf l_b)] using hag l_b (by omega)
  have hachF : F l_b = CopiedDstar.dstarRankGA_m P hV mS n :=
    hagb_const.symm.trans hach
  have hFclass :=
    hUpdate c j0 ī0 mS n pc M Nc F hpc hF posOf hupdval hag hselconstU hNcmS
      l_b hl_b_lo hl_bNc hselbU hachF l' hMl' hl'mS hres
  have hagl_const :
      P.rank c (copiedSlice mS n) (fun _ : Fin (P.toPoly.arity c) => posOf l') =
        F l' := by
    simpa [hupdconst (posOf l')] using hag l' hl'mS
  exact hagl_const.trans hFclass

/-- Forget update-tuple d*-achiever coverage to the legacy bridge-facing
constant-tuple locus under tuple-fibre scalarization. -/
theorem dstarAchieverLocus_of_update_of_tupleFiberScalarization
    (P : WRP.Presentation Step Step) (hV : P.Valid)
    (hUpdate : CopiedAchieverLocus.DstarAchieverUpdateLocus P hV)
    (hTuple : TupleFiberScalarization P) :
    CopiedAchieverLocus.DstarAchieverLocus P hV := by
  constructor
  · intro c0 j0 ī0 mS n pc Ts Nc q_D mx FFs hpc hTq hF hag hselconst hNcmS hNcmx
      l hl hsel hach
    have hupdconst : ∀ v,
        Function.update ī0 j0 v = fun _ : Fin (P.toPoly.arity c0) => v := by
      intro v
      exact update_eq_const_of_tupleFiberScalarization P hTuple c0 j0 ī0 v
    have hupdval :
        ∀ l0, l0 < mS - 1 →
          ∀ i, Function.update ī0 j0 (mS + 2 * n + 1 + l0) i
            < (copiedSlice mS n).length := by
      intro l0 hl0 i
      have hi := congrFun (hupdconst (mS + 2 * n + 1 + l0)) i
      rw [hi, length_copiedSlice]
      omega
    have hselconstU :
        ∀ l₁ l₂, Ts ≤ l₁ → l₁ < Nc → Ts ≤ l₂ → l₂ < Nc →
          (l₁ - Ts) % pc = (l₂ - Ts) % pc →
      ...
      intro l₁ l₂ hT₁ hNc₁ hT₂ hNc₂ hmod
      simpa [hupdconst (mS + 2 * n + 1 + l₁),
        hupdconst (mS + 2 * n + 1 + l₂)] using
        hselconst l₁ l₂ hT₁ hNc₁ hT₂ hNc₂ hmod
    have hselU :
        P.toPoly.sel c0 (copiedSlice mS n)
            (Function.update ī0 j0 (mS + 2 * n + 1 + l))
          ∧ P.toPoly.label c0 (copiedSlice mS n)
            (Function.update ī0 j0 (mS + 2 * n + 1 + l)) = D := by
      simpa [hupdconst (mS + 2 * n + 1 + l)] using hsel
    have hagl_const :
        P.rank c0 (copiedSlice mS n)
            (fun _ : Fin (P.toPoly.arity c0) => mS + 2 * n + 1 + l) =
          FFs l := by
      simpa [hupdconst (mS + 2 * n + 1 + l)] using hag l hl
    have hachF : FFs l = CopiedDstar.dstarRankGA_m P hV mS n :=
      hagl_const.symm.trans hach
    exact hUpdate.1 c0 j0 ī0 mS n pc Ts Nc q_D mx FFs hpc hTq hF hupdval hag
      hselconstU hNcmS hNcmx l hl hselU hachF
  · intro c0 j0 ī0 mS n pc Tp Nc q_U mx FFp hpc hTq hF hag hselconst hNcmS hNcmx
      l hl hsel hach
    have hupdconst : ∀ v,
        Function.update ī0 j0 v = fun _ : Fin (P.toPoly.arity c0) => v := by
      intro v
    ...
    have hupdval :
        ∀ l0, l0 < mS - 1 →
          ∀ i, Function.update ī0 j0 l0 i < (copiedSlice mS n).length := by
      intro l0 hl0 i
      have hi := congrFun (hupdconst l0) i
      rw [hi, length_copiedSlice]
      omega
    have hselconstU :
        ∀ l₁ l₂, Tp ≤ l₁ → l₁ < Nc → Tp ≤ l₂ → l₂ < Nc →
          (l₁ - Tp) % pc = (l₂ - Tp) % pc →
          ((P.toPoly.sel c0 (copiedSlice mS n) (Function.update ī0 j0 l₁)
              ∧ P.toPoly.label c0 (copiedSlice mS n) (Function.update ī0 j0 l₁) = D)
            ↔ (P.toPoly.sel c0 (copiedSlice mS n) (Function.update ī0 j0 l₂)
              ∧ P.toPoly.label c0 (copiedSlice mS n) (Function.update ī0 j0 l₂) = D)) := by
      intro l₁ l₂ hT₁ hNc₁ hT₂ hNc₂ hmod
      simpa [hupdconst l₁, hupdconst l₂] using
        hselconst l₁ l₂ hT₁ hNc₁ hT₂ hNc₂ hmod
    have hselU :
        P.toPoly.sel c0 (copiedSlice mS n) (Function.update ī0 j0 l)
          ∧ P.toPoly.label c0 (copiedSlice mS n) (Function.update ī0 j0 l) = D := by
      simpa [hupdconst l] using hsel
    have hagl_const :
        P.rank c0 (copiedSlice mS n) (fun _ : Fin (P.toPoly.arity c0) => l) =
          FFp l := by
      simpa [hupdconst l] using hag l hl
    have hachF : FFp l = CopiedDstar.dstarRankGA_m P hV mS n :=
      hagl_const.symm.trans hach
    exact hUpdate.2 c0 j0 ī0 mS n pc Tp Nc q_U mx FFp hpc hTq hF hupdval hag
      hselconstU hNcmS hNcmx l hl hselU hachF

/-- Forget update-tuple band/deep soundness to the legacy bridge-facing
soundness package under tuple-fibre scalarization. -/
theorem bandedRankSoundness_of_update_of_tupleFiberScalarization
    (P : WRP.Presentation Step Step) (hV : P.Valid)
    (hUpdate : CopiedBoundedGateBand.BandedUpdateRankSoundness P hV)
    (hTuple : TupleFiberScalarization P) :
    CopiedBoundedGateBand.BandedRankSoundness P hV := by
  classical
  rcases hUpdate with ⟨hSuf, hPre, hDeepSuf, hDeepPre⟩
  refine ⟨?_, ?_, ?_, ?_⟩
  · intro c j0 ī0 B mS n pc T Q k F hm hn hpc hQ hpcQ hk hTk hT2 hF hag
      ρ ρact hρ hρact h0 h1 xb hp hband habs
    have hρact_lt : ρact < pc := by
      rw [hρact]
      exact Nat.mod_lt _ (Nat.lt_of_lt_of_le Nat.zero_lt_one hpc)
    have htuple0 :=
      cellTupleF_sufIdx_eq_update_of_tupleFiberScalarization P hTuple c j0 ī0 mS n
        (T + ρact) (B := B)
    have htuple1 :=
      cellTupleF_sufIdx_eq_update_of_tupleFiberScalarization P hTuple c j0 ī0 mS n
        (T + ρact + pc) (B := B)
    have h0F : F (T + ρact) = CopiedDstar.dstarRankGA_m P hV mS n := by
      rw [← hag (T + ρact) (by omega)]
      rw [← htuple0]
      exact h0
    have h1F : F (T + ρact + pc) = CopiedDstar.dstarRankGA_m P hV mS n := by
      rw [← hag (T + ρact + pc) (by omega)]
      rw [← htuple1]
      exact h1
    have hrank :=
      @hSuf c j0 ī0 B mS n pc T Q k F hm hn hpc hQ hpcQ hk hTk hT2 hF hag
        ρ ρact
        hρ hρact h0F h1F (xb j0) hp hband habs
    rw [tuple_eq_update_of_tupleFiberScalarization P hTuple c j0 ī0 xb]
    exact hrank
  · intro c j0 ī0 B mS n pc T Q k F hm hn hpc hQ hpcQ hTk hT2 hF hag
      ρ ρact hρ hρact h0 h1 xb hrun hband hpref habs
    have hρact_lt : ρact < pc := by
      rw [hρact]
      exact Nat.mod_lt _ (Nat.lt_of_lt_of_le Nat.zero_lt_one hpc)
    have htuple0 :=
      cellTupleF_prefIdx_eq_update_of_tupleFiberScalarization P hTuple c j0 ī0 mS n
        (T + ρact) (B := B)
    have htuple1 :=
      cellTupleF_prefIdx_eq_update_of_tupleFiberScalarization P hTuple c j0 ī0 mS n
        (T + ρact + pc) (B := B)
    ...
    have h1F : F (T + ρact + pc) = CopiedDstar.dstarRankGA_m P hV mS n := by
      ...
      exact h1
    have hrank :=
      @hPre c j0 ī0 B mS n pc T Q k F hm hn hpc hQ hpcQ hTk hT2 hF hag
        ρ ρact
        hρ hρact h0F h1F (xb j0) hrun hband hpref habs
    rw [tuple_eq_update_of_tupleFiberScalarization P hTuple c j0 ī0 xb]
    exact hrank
  · intro c j0 B mS n t k hk hactive xb hguard
    exact @hDeepSuf c j0 B mS n t k hk hactive xb hguard
  · intro c j0 B mS n t k hm hn hk hactive xb hguard
    exact @hDeepPre c j0 B mS n t k hm hn hk hactive xb hguard

/-- Forget update-shaped activation completeness to the legacy constant-tuple
activation package under tuple-fibre scalarization. -/
theorem bandedActivationCompleteness_of_update_of_tupleFiberScalarization
    (P : WRP.Presentation Step Step) (hV : P.Valid)
    (hUpdate : CopiedBoundedGateBand.BandedUpdateActivationCompleteness P hV)
    (hTuple : TupleFiberScalarization P) :
    CopiedBoundedGateBand.BandedActivationCompleteness P hV := by
  classical
  rcases hUpdate with ⟨hSuf, hPre⟩
  constructor
  · intro c j0 ī0 B mS n pc T ρact F hpc hρact_lt hT2 hag h0F h1F
    have htuple0 :=
      cellTupleF_sufIdx_eq_update_of_tupleFiberScalarization P hTuple c j0 ī0 mS n
        (T + ρact) (B := B)
    have htuple1 :=
      cellTupleF_sufIdx_eq_update_of_tupleFiberScalarization P hTuple c j0 ī0 mS n
        (T + ρact + pc) (B := B)
    have hactive := hSuf c j0 ī0 mS n pc T ρact F hpc hρact_lt hT2 hag h0F h1F
    constructor
    · rw [htuple0]
      exact hactive.1
    · rw [htuple1]
  ...
  · intro c j0 ī0 B mS n pc T ρact F hpc hρact_lt hT2 hag h0F h1F
    ...
    have htuple1 :=
      cellTupleF_prefIdx_eq_update_of_tupleFiberScalarization P hTuple c j0 ī0 mS n
        (T + ρact + pc) (B := B)
    have hactive := hPre c j0 ī0 mS n pc T ρact F hpc hρact_lt hT2 hag h0F h1F
    constructor
    · rw [htuple0]
      exact hactive.1
    · rw [htuple1]
...
/-- Coordinate/cycle data plus update run-class coverage produce the
update-shaped semantic split package whenever the tuple fibres scalarize.  The
selected-D constancy hypotheses are obtained by rewriting the update tuples to
the constant tuples used by the old clear-window argument. -/
theorem tieSemanticUpdateSplitData_of_coordData_of_tupleFiberScalarization
    (P : WRP.Presentation Step Step) (hV : P.Valid)
    (hCoord : CopiedSelUniform.TieSemanticCoordData P)
    (hRunClass : CopiedSelUniform.RunClassUpdateUniformCoverage P hV)
    (hTuple : TupleFiberScalarization P) :
    CopiedSelUniform.TieSemanticUpdateSplitData P hV := by
  classical
  ...
  have hupdvalp :
    ...
    have hi :=
      congrFun
        (update_eq_const_of_tupleFiberScalarization P hTuple c (j0F c) (ī0F c) l) i
    rw [hi, length_copiedSlice]
    omega
  have hselconsts :
    ...
    have hlegacy :=
      CopiedSelUniform.selconst_suf P c (Mc c) (hMc c) mS n (pcF c) (Ts c) Nc hm1
        (hNcs c) (fun κ b hb => hsufcyc c κ b hb)
        l₁ l₂ hT₁ hNc₁ hT₂ hNc₂ hmod
    simpa [
      update_eq_const_of_tupleFiberScalarization P hTuple c (j0F c) (ī0F c)
        (mS + 2 * n + 1 + l₁),
      update_eq_const_of_tupleFiberScalarization P hTuple c (j0F c) (ī0F c)
        (mS + 2 * n + 1 + l₂)] using hlegacy
  have hselconstp :
    ...
    intro c l₁ l₂ hT₁ hNc₁ hT₂ hNc₂ hmod
    have hlegacy :=
      CopiedSelUniform.selconst_pre P c (Mc c) (hMc c) mS n (pcF c) (Tp c) Nc hm1
        (hNcp c) (fun κ b hb => hprefcyc c κ b hb)
        l₁ l₂ hT₁ hNc₁ hT₂ hNc₂ hmod
    simpa [
      update_eq_const_of_tupleFiberScalarization P hTuple c (j0F c) (ī0F c) l₁,
      update_eq_const_of_tupleFiberScalarization P hTuple c (j0F c) (ī0F c) l₂] using
      hlegacy
  refine ⟨FFs, FFp, Nc, hNcmS, hNcmMbr, hNcs, hNcp, hFs, hFp, hags, hagp,
    hupdvals, hupdvalp, hselconsts, hselconstp, ?_⟩
  intro c0 ī0m
  exact CopiedSelUniform.tie_semantic_update_split_slice P hV hRunClass mS n c0 ī0m
    pcF Ts Tp hpcF Nc hNcmS FFs FFp ī0F j0F hFs hFp hupdvals hupdvalp hags hagp
    hselconsts hselconstp

/-- Current supplier bundle for the row-indexed tie bridge.  This packages the
arity-sensitive pieces which the arbitrary-arity route must replace or refine. -/
def BridgeSupplierBundle (P : WRP.Presentation Step Step) (hV : P.Valid) : Prop :=
    CopiedSelUniform.TieSemanticCoordData P ∧
    CopiedSelUniform.RunClassUniformCoverage P hV ∧
    (∃ CbudN : ℕ,
      ∀ (mS : ℕ), 1 ≤ mS →
    ...
    RunResidueCoverage P hV ∧
    CopiedBoundedGateBand.BandedTupleRankSoundness P hV ∧
    CoreBoundaryCoverage P hV

/-- Update-shaped supplier bundle for the row-indexed bridge.  This is the
descriptor-facing counterpart of `BridgeSupplierBundle`: the semantic split,
run-class collapse, achiever coverage, and run-band soundness are all stated
for distinguished-coordinate update tuples.  The scalarized conversion below is
only a compatibility bridge for the current arity-1 route. -/
def BridgeUpdateSupplierBundle (P : WRP.Presentation Step Step) (hV : P.Valid) : Prop :=
    CopiedSelUniform.TieSemanticUpdateSplitData P hV ∧
...
/-- Forget the zero-base witness from the canonical update supplier bundle. -/
theorem bridgeUpdateSupplierBundle_of_zero
    (P : WRP.Presentation Step Step) (hV : P.Valid)
    (hZero : BridgeUpdateZeroSupplierBundle P hV) :
    BridgeUpdateSupplierBundle P hV := by
  rcases hZero with
    ⟨hZeroSplit, hRunClassUpdate, hBudget, hRunResidueUpdate, hBandActiveUpdate,
      hCoreBoundaryUpdate⟩
  exact ⟨CopiedSelUniform.tieSemanticUpdateSplitData_of_zeroSplitData P hV hZeroSplit,
    hRunClassUpdate, hBudget, hRunResidueUpdate, hBandActiveUpdate, hCoreBoundaryUpdate⟩
...
/-- Compatibility bridge from the update-shaped supplier bundle to the current
legacy bridge bundle, isolating the remaining tuple-fibre scalarization
dependency. -/
theorem bridgeSupplierBundle_of_update_of_tupleFiberScalarization
    (P : WRP.Presentation Step Step) (hV : P.Valid)
    (hUpdate : BridgeUpdateSupplierBundle P hV)
    (hTuple : TupleFiberScalarization P) :
    BridgeSupplierBundle P hV := by
  rcases hUpdate with
    ⟨hUpdateSplit, hRunClassUpdate, ⟨CbudN, hbudCN⟩, hRunResidueUpdate, hBandActiveUpdate,
      hCoreBoundaryUpdate⟩
  refine ⟨
    CopiedSelUniform.tieSemanticCoordData_of_updateSplitData P hV hUpdateSplit,
    runClassUniformCoverage_of_update_of_tupleFiberScalarization P hV hRunClassUpdate hTuple,
    ⟨CbudN, hbudCN⟩,
    ?_,
    CopiedBoundedGateBand.bandedTupleRankSoundness_of_rankSoundness P hV
      (bandedRankSoundness_of_update_of_tupleFiberScalarization P hV
        hRunResidueUpdate.2 hTuple),
    coreBoundaryCoverage_of_update_of_tupleFiberScalarization P hV hCoreBoundaryUpdate hTuple⟩
  exact ⟨
    dstarAchieverLocus_of_update_of_tupleFiberScalarization P hV hRunResidueUpdate.1 hTuple,
    bandedActivationCompleteness_of_update_of_tupleFiberScalarization P hV
      hBandActiveUpdate hTuple⟩

/-- Build the update-shaped bridge supplier bundle from coordinate data.  The
run-class, run-residue, and band update suppliers are arity-free; scalarization
is used here only to construct the update semantic split from an already
packaged `TieSemanticCoordData`. -/
theorem bridgeUpdateSupplierBundle_of_coordData
    (P : WRP.Presentation Step Step) (hV : P.Valid)
    (hCoord : CopiedSelUniform.TieSemanticCoordData P)
    (CbudN : ℕ)
    (hbudCN : ∀ (mS : ℕ), 1 ≤ mS →
    ...
    (hCoreBoundary : CoreBoundaryUpdateCoverage P hV) :
    BridgeUpdateSupplierBundle P hV :=
  ⟨tieSemanticUpdateSplitData_of_coordData_of_tupleFiberScalarization P hV hCoord
      (CopiedSelUniform.runClassUpdateUniformCoverage P hV) hTuple,
    CopiedSelUniform.runClassUpdateUniformCoverage P hV,
    ⟨CbudN, hbudCN⟩,
    runResidueUpdateCoverage P hV,
    hBandActive,
...
coordinate in every copy.  Unlike `bridgeUpdateSupplierBundle_of_coordData`,
the semantic split component here is the general-arity zero-base update split;
no tuple-fibre scalarization is used for that part. -/
...
/-- Arity-1 supplier for `BridgeUpdateSupplierBundle`. -/
theorem bridgeUpdateSupplierBundle_of_arity_one
    (P : WRP.Presentation Step Step) (hV : P.Valid)
    (harity1 : ∀ c, P.toPoly.arity c = 1) :
    BridgeUpdateSupplierBundle P hV :=
  bridgeUpdateSupplierBundle_of_zero P hV
    (bridgeUpdateZeroSupplierBundle_of_arity_one P hV harity1)

/-- Arity-1 supplier for `BridgeSupplierBundle`. -/
theorem bridgeSupplierBundle_of_arity_one
    (P : WRP.Presentation Step Step) (hV : P.Valid)
    (harity1 : ∀ c, P.toPoly.arity c = 1) :
    BridgeSupplierBundle P hV :=
  bridgeSupplierBundle_of_update_of_tupleFiberScalarization P hV
    (bridgeUpdateSupplierBundle_of_arity_one P hV harity1)
    (tupleFiberScalarization_of_arity_one P harity1)
...
/-- Finite row data for the multi-coordinate bridge: one ordinary row index for
each global distinguished-coordinate choice. -/
abbrev ChoiceBridgeRowIndex (P : WRP.Presentation Step Step)
    (B Bh M pG q_U q_D Q : ℕ) (Ndeep : Fin P.toPoly.K → ℕ) :=
  CoordChoice P → BridgeRowIndex P B Bh M pG q_U q_D Q Ndeep

noncomputable instance choiceBridgeRowIndexFintype (P : WRP.Presentation Step Step)
    (B Bh M pG q_U q_D Q : ℕ) (Ndeep : Fin P.toPoly.K → ℕ) :
    Fintype (ChoiceBridgeRowIndex P B Bh M pG q_U q_D Q Ndeep) := by
  dsimp [ChoiceBridgeRowIndex]
  infer_instance

noncomputable instance choiceBridgeRowIndexDecidableEq (P : WRP.Presentation Step Step)
    (B Bh M pG q_U q_D Q : ℕ) (Ndeep : Fin P.toPoly.K → ℕ) :
    DecidableEq (ChoiceBridgeRowIndex P B Bh M pG q_U q_D Q Ndeep) := by
  classical
  dsimp [ChoiceBridgeRowIndex]
  infer_instance

/-- Canonical finite enumeration of all global distinguished-coordinate choices. -/
noncomputable def coordChoiceList (P : WRP.Presentation Step Step) : List (CoordChoice P) :=
  (Finset.univ : Finset (CoordChoice P)).toList

/-- Conjoin a scalar row-indexed automaton family over all global
distinguished-coordinate choices. -/
noncomputable def choiceBridgeGdfaF
    {P : WRP.Presentation Step Step} {B Bh M pG q_U q_D Q : ℕ}
    {Ndeep : Fin P.toPoly.K → ℕ}
    (GdfaF : (J : CoordChoice P) →
      BridgeRowIndex P B Bh M pG q_U q_D Q Ndeep → ℕ →
        (c : Fin P.toPoly.K) → SliceMSO.DetAuto (MarkedN (P.toPoly.arity c))) :
    ChoiceBridgeRowIndex P B Bh M pG q_U q_D Q Ndeep → ℕ →
      (c : Fin P.toPoly.K) → SliceMSO.DetAuto (MarkedN (P.toPoly.arity c)) :=
  fun idx rN c =>
    SliceMSO.DetAuto.all
      ((coordChoiceList P).map (fun J => GdfaF J (idx J) rN c))
...
/-- Deep-suffix row table for canonical update-row indices.  It stores the
deep offsets whose constant deep-suffix representative has rank `d*` at the
row representative. -/
noncomputable def updateDsufRowTable
    (P : WRP.Presentation Step Step) (hV : P.Valid) {pG B : ℕ}
    (Ndeep : Fin P.toPoly.K → ℕ) (mS : ℕ) (repN : Fin pG → ℕ) :
    Fin pG → (c' : Fin P.toPoly.K) → Finset ℕ :=
  fun r c' =>
...
/-- Deep-prefix row table for canonical update-row indices.  The stored gate
offset is shifted by `+3`, matching `deepPreOrdClauseAt`. -/
noncomputable def updateDpreRowTable
    (P : WRP.Presentation Step Step) (hV : P.Valid) {pG B : ℕ}
    (Ndeep : Fin P.toPoly.K → ℕ) (mS : ℕ) (repN : Fin pG → ℕ) :
    Fin pG → (c' : Fin P.toPoly.K) → Finset ℕ :=
  fun r c' =>
...
/-- Deep-suffix row table for genuine update-tuple deep clauses.  Unlike
`updateDsufRowTable`, the representative rank is measured on the tuple with
only the distinguished coordinate moved into the deep suffix position. -/
...
theorem updateBridgePeriodSuppliers_of_budget
  ...
  obtain ⟨NDomN, pDomN, hpDomN, hDomN⟩ := CopiedTie2b.domain_EP_n_uniform_floor P
  obtain ⟨NDpN, pDpN, hpDpN, hDpN⟩ := CopiedTie2b.Dpresent_EP_n_uniform_floor P
...
theorem rowS1Odd_mkBridgeRowIndex {P : WRP.Presentation Step Step} {B Bh M pG q_U q_D Q : ℕ}
    {Ndeep : Fin P.toPoly.K → ℕ} (hpG : 1 ≤ pG)
    (S1 : Fin pG → S1Key P B q_U q_D → Finset ℕ)
    (F2 : Fin pG → F2Key P Bh q_U q_D → Finset ℕ)
    (Ssuf Spre Dsuf Dpre : Fin pG → (c' : Fin P.toPoly.K) → Finset ℕ)
    ...
    rowS1Odd hpG
      ...
      = S1 (rowClass hpG rN) ⟨c', ⟨rs, hmem⟩⟩ := by
  rw [rowS1Odd,
    rowS1_mkBridgeRowIndex hpG S1 F2 Ssuf Spre Dsuf Dpre hS1 hF2 hSsuf hSpre hDsuf hDpre
      rN c' rs hmem]
  exact Finset.filter_true_of_mem (fun x hx => hS1odd (rowClass hpG rN) ⟨c', ⟨rs, hmem⟩⟩ x hx)

theorem rowF2Odd_mkBridgeRowIndex {P : WRP.Presentation Step Step} {B Bh M pG q_U q_D Q : ℕ}
    {Ndeep : Fin P.toPoly.K → ℕ} (hpG : 1 ≤ pG)
    (S1 : Fin pG → S1Key P B q_U q_D → Finset ℕ)
    (F2 : Fin pG → F2Key P Bh q_U q_D → Finset ℕ)
    (Ssuf Spre Dsuf Dpre : Fin pG → (c' : Fin P.toPoly.K) → Finset ℕ)
    ...
    rowF2Odd hpG
      ...
      = F2 (rowClass hpG rN) ⟨c', ⟨rs, hmem⟩⟩ := by
  rw [rowF2Odd,
    rowF2_mkBridgeRowIndex hpG S1 F2 Ssuf Spre Dsuf Dpre hS1 hF2 hSsuf hSpre hDsuf hDpre
      rN c' rs hmem]
...
/-- Decode a deep-suffix row stored in the canonical update row index and use
the deep-rank callback to prove the guarded competitor has rank `d*` at the
actual slice. -/
theorem rank_deep_suffix_of_rowDsuf_rowRep_floor
    {P : WRP.Presentation Step Step} {hV : P.Valid}
    {B Bh M pG q_U q_D Q : ℕ} {Ndeep : Fin P.toPoly.K → ℕ}
    {p0 pDomN pDpN NDomN NDpN Nfloor : ℕ}
    (hpG : 1 ≤ pG) (hp0dvd : p0 ∣ pG)
    ...
    (hNDom_floor : NDomN ≤ Nfloor) (hNDp_floor : NDpN ≤ Nfloor)
    (hB_floor : B + 1 + B + 1 ≤ Nfloor)
    (n k : ℕ) (xb : Fin (P.toPoly.arity c') → ℕ)
    (hn_floor : Nfloor ≤ n)
    (hG : CopiedAchSetFold.domDp P mS n)
  ...
  exact hSound.2.2.1 c' (j0F c') (B := B) mS n (B + 1) k hk
    (Finset.mem_filter.mp hactive_mem).2 xb hguard

/-- Prefix twin of `rank_deep_suffix_of_rowDsuf_rowRep_floor`. -/
theorem rank_deep_prefix_of_rowDpre_rowRep_floor
    {P : WRP.Presentation Step Step} {hV : P.Valid}
    {B Bh M pG q_U q_D Q : ℕ} {Ndeep : Fin P.toPoly.K → ℕ}
    {p0 pDomN pDpN NDomN NDpN Nfloor : ℕ}
    (hpG : 1 ≤ pG) (hp0dvd : p0 ∣ pG)
    ...
    (hguard : ∀ i, ((copiedSlice mS n)[xb i]? = some U
        ∧ ∀ q, q < (copiedSlice mS n).length → q < xb i →
  ...
  have hactive :
      P.rank c' (copiedSlice mS n)
          (fun _ : Fin (P.toPoly.arity c') =>
            mixedPosAt
              (Sum.inr (Sum.inr (k0 + 3 - 2)) : CopiedCells.RegionSpecF B ⊕ (ℕ ⊕ ℕ))
              mS (B + 1) n)
        = CopiedDstar.dstarRankGA_m P hV mS n := by
    simpa [hshift] using hactive0
  exact hSound.2.2.2 c' (j0F c') (B := B) mS n (B + 1) (k0 + 3)
    hm hn (by omega) hactive xb hguard

/-- Forward bridge hook for a canonical deep-suffix update row. -/
theorem ord_deep_suffix_of_rowDsuf_rowRep_floor
    {P : WRP.Presentation Step Step} {hV : P.Valid}
    {B Bh M pG q_U q_D Q : ℕ} {Ndeep : Fin P.toPoly.K → ℕ}
    {p0 pDomN pDpN NDomN NDpN Nfloor : ℕ}
    (hpG : 1 ≤ pG) (hp0dvd : p0 ∣ pG)
    ...
    P.toPoly.ord c c' (copiedSlice mS n) ī xb := by
  have hrank :=
    rank_deep_suffix_of_rowDsuf_rowRep_floor
      (P := P) (hV := hV) (B := B) (Bh := Bh) (M := M) (pG := pG)
      (q_U := q_U) (q_D := q_D) (Q := Q) (Ndeep := Ndeep)
      (p0 := p0) (pDomN := pDomN) (pDpN := pDpN) (NDomN := NDomN)
      (NDpN := NDpN) (Nfloor := Nfloor)
      ...
      hm hNdeep_add2 hN_floor hNDom_floor hNDp_floor hB_floor n k xb hn_floor hG
      hrmem hguard
  exact ord_of_tie_hall_rank (P := P) (hV := hV) hall hxbval hsel hlabel hrank

/-- Prefix twin of `ord_deep_suffix_of_rowDsuf_rowRep_floor`. -/
theorem ord_deep_prefix_of_rowDpre_rowRep_floor
    {P : WRP.Presentation Step Step} {hV : P.Valid}
    {B Bh M pG q_U q_D Q : ℕ} {Ndeep : Fin P.toPoly.K → ℕ}
    {p0 pDomN pDpN NDomN NDpN Nfloor : ℕ}
    (hpG : 1 ≤ pG) (hp0dvd : p0 ∣ pG)
    ...
    P.toPoly.ord c c' (copiedSlice mS n) ī xb := by
  have hrank :=
    rank_deep_prefix_of_rowDpre_rowRep_floor
      (P := P) (hV := hV) (B := B) (Bh := Bh) (M := M) (pG := pG)
      (q_U := q_U) (q_D := q_D) (Q := Q) (Ndeep := Ndeep)
      (p0 := p0) (pDomN := pDomN) (pDpN := pDpN) (NDomN := NDomN)
      (NDpN := NDpN) (Nfloor := Nfloor)
      ...
      hm hN_floor hNDom_floor hNDp_floor hB_floor n k xb hn hn_floor hG hrmem hguard
  exact ord_of_tie_hall_rank (P := P) (hV := hV) hall hxbval hsel hlabel hrank
...
/-- The semantic condition recognized by the finite row-index bridge gate. -/
def bridgeGateCond (P : WRP.Presentation Step Step) (c : Fin P.toPoly.K)
  ...
  P.toPoly.sel c (copiedSlice mS n) ī
        ...
        ∀ xb : Fin (P.toPoly.arity c') → ℕ, (∀ i, xb i < (copiedSlice mS n).length) →
          ((∀ i, (((copiedSlice mS n)[xb i]? = some D
                ∧ ∀ q, q < (copiedSlice mS n).length → xb i < q →
                    (copiedSlice mS n)[q]? = some D)
              ∧ xb i % Q = r)
              ∧ (∃ y, y < (copiedSlice mS n).length ∧ xb i = y + (q_D + 2)
    ...
    ∧ (∀ (c' : Fin P.toPoly.K) (r : ℕ), r ∈ rowSpre hpG idx rN c' →
        ∀ xb : Fin (P.toPoly.arity c') → ℕ, (∀ i, xb i < (copiedSlice mS n).length) →
          (((∀ i, CopiedBandRunGate.preBandStrictGuard
                (copiedSlice mS n) Q r q_U (xb i))
              ∧ P.toPoly.sel c' (copiedSlice mS n) xb)
            ∧ P.toPoly.label c' (copiedSlice mS n) xb = D) →
          P.toPoly.ord c c' (copiedSlice mS n) ī xb)
...
/-- Update-shaped semantic condition recognized by the finite row-index bridge
gate.  Compared with `bridgeGateCond`, the suffix/prefix run-band clauses only
range over distinguished-coordinate updates of the fixed base tuple. -/
def bridgeUpdateGateCond (P : WRP.Presentation Step Step) (c : Fin P.toPoly.K)
    (B Bh M mthr pG q_U q_D Q : ℕ) (Ndeep : Fin P.toPoly.K → ℕ)
    (hpG : 1 ≤ pG) (idx : BridgeRowIndex P B Bh M pG q_U q_D Q Ndeep)
    (j0F : (c' : Fin P.toPoly.K) → Fin (P.toPoly.arity c'))
    (ī0F : (c' : Fin P.toPoly.K) → Fin (P.toPoly.arity c') → ℕ)
...
theorem bridgeRowIndex_gate_family (P : WRP.Presentation Step Step)
    (B Bh M mthr pG q_U q_D Q : ℕ) (Ndeep : Fin P.toPoly.K → ℕ)
    ...
    ∃ (GdfaF : BridgeRowIndex P B Bh M pG q_U q_D Q Ndeep → ℕ →
      ...
      ∀ (idx : BridgeRowIndex P B Bh M pG q_U q_D Q Ndeep) (rN : ℕ)
        ...
        (bridgeGateCond P c B Bh M mthr pG q_U q_D Q Ndeep hpG idx rN mS n ī
          ↔ (GdfaF idx rN c).accepts (markAtN (P.toPoly.arity c) (copiedSlice mS n) ī)) := by
  classical
  choose GdfaF hGdfa using fun
      (idx : BridgeRowIndex P B Bh M pG q_U q_D Q Ndeep) (rN : ℕ)
      (c : Fin P.toPoly.K) =>
    CopiedBoundedGateBand.fasU_atomOrd_full_gate_bounded_band P c B Bh M mthr q_U q_D
      (q_D + 2) q_U
      (rowS1Odd hpG idx rN) (fun _ _ => ∅) (fun _ _ => ∅)
      (fun _ _ => ∅) (rowF2Odd hpG idx rN) (fun _ _ => ∅)
      (rowSsuf hpG idx rN) (rowSpre hpG idx rN)
      (rowDsuf hpG idx rN) (rowDpre hpG idx rN) Q hQ
  ...
  exact hGdfa idx rN c mS n ī hqU hqD hBh hval
...
/-- Assemble the zero-base update gate condition from the row-representative
order hooks.  This is the forward bridge-body package needed by the direct
`BridgeUpdateSupplierBundle` route: once the update split has supplied the
global `hall` order obligation, every row-index clause can be discharged by
the named update-row soundness lemmas. -/
theorem bridgeUpdateGateCond_of_updateRow_floor
    {P : WRP.Presentation Step Step} {hV : P.Valid}
    {B Bh M mthr pG pSel q_U q_D Q : ℕ} {Ndeep : Fin P.toPoly.K → ℕ}
    {p0 pDomN pDpN NDomN NDpN Nfloor : ℕ}
    (hpG : 1 ≤ pG) (hp0dvd : p0 ∣ pG) (hQdvd : Q ∣ pG)
    ...
    (hagp : ∀ c' l, l < mS - 1 →
      P.rank c' (copiedSlice mS n)
        (Function.update (fun _ : Fin (P.toPoly.arity c') => 0) (j0F c') l)
          = Fp c' l) :
    bridgeUpdateGateCond P c B Bh M mthr pG q_U q_D Q Ndeep hpG
      (mkBridgeUpdateRowIndex (Q := Q) hV pcF Ts Tp j0F mS (rowRep pG Nfloor)
        S1tab F2tab Dsuf Dpre hS1 hF2 hDsuf hDpre)
  ...
  · intro c' k hkmem xb hxbval hcond
    rcases hcond with ⟨hguard, hselD, hlabelD⟩
    exact ord_deep_suffix_of_rowDsuf_rowRep_floor
      (P := P) (hV := hV) (B := B) (Bh := Bh) (M := M) (pG := pG)
      (q_U := q_U) (q_D := q_D) (Q := Q) (Ndeep := Ndeep)
      (p0 := p0) (pDomN := pDomN) (pDpN := pDpN) (NDomN := NDomN)
      (NDpN := NDpN) (Nfloor := Nfloor)
      ...
      n k xb hn_floor hG hall hkmem hxbval hguard hselD hlabelD
  · intro c' k hkmem xb hxbval hcond
    rcases hcond with ⟨hguard, hselD, hlabelD⟩
    exact ord_deep_prefix_of_rowDpre_rowRep_floor
      (P := P) (hV := hV) (B := B) (Bh := Bh) (M := M) (pG := pG)
      (q_U := q_U) (q_D := q_D) (Q := Q) (Ndeep := Ndeep)
      (p0 := p0) (pDomN := pDomN) (pDpN := pDpN) (NDomN := NDomN)
      (NDpN := NDpN) (Nfloor := Nfloor)
      ...
      hm (hNDPre_floor c') hNDom_floor hNDp_floor hB_floor n k xb hn hn_floor hG
      hall hkmem hxbval hguard hselD hlabelD

/-- Fully update-shaped counterpart of
`bridgeUpdateGateCond_of_updateRow_floor`.  The deep rows and deep
periodicity hypotheses are measured on the one-coordinate update tuples. -/
...
core/frozen cfg branch is unchanged from `bridgeUpdateGateCond`, so the same
selector-table replay discharges the atom order obligation. -/
...
/-- **Budgeted row-indexed fibred tie bridge from abstract suppliers.**  For a
fixed selected-atom budget `C`, the machine family is finite before the row is
known, while the row may choose a finite `BridgeRowIndex` after seeing `mS` and
the budget proof.  The remaining arity-sensitive ingredients are explicit
supplier hypotheses. -/
theorem tie_point_bridge_fibred_clause_budgeted_indexed_of_suppliers
    (P : WRP.Presentation Step Step) (hV : P.Valid)
    (C : ℕ)
    (hCoord : CopiedSelUniform.TieSemanticCoordData P)
    (hRunClass : CopiedSelUniform.RunClassUniformCoverage P hV)
    (CbudN : ℕ)
    (hbudCN : ∀ (mS : ℕ), 1 ≤ mS →
      ∀ n, P.toPoly.domain (copiedSlice mS n) →
        ∀ (c : Fin P.toPoly.K) (l : List (Fin (P.toPoly.arity c) → ℕ)), l.Nodup →
          (∀ x ∈ l, P.toPoly.selectedAtom (copiedSlice mS n) ⟨c, x⟩) →
          l.length ≤ CbudN * (mS + n + 1))
    (hRunResidue : RunResidueCoverage P hV)
    (hBandSound : CopiedBoundedGateBand.BandedTupleRankSoundness P hV)
    (hCoreBoundary : CoreBoundaryCoverage P hV) :
                ...
                ((P.toPoly.sel c (copiedSlice mS n) ī
                 ...
                 ↔ ((GdfaF idx (n % pG) c).accepts
                    ...
                    ∧ P.rankOf (copiedSlice mS n) ⟨c, ī⟩
                        = CopiedDstar.dstarRankGA_m P hV mS n)) := by
  classical
  have hSplitData : CopiedSelUniform.TieSemanticSplitData P hV :=
    CopiedSelUniform.tieSemanticSplitData_of_coordData P hV hCoord hRunClass
  obtain ⟨B, Bh, M, mthr, _pstar, pSel, _hpstar, _hpstardvd, hpSel, hBB, hM2, hBh1, hB1,
    hselBudget⟩ := CopiedSelector.eqRankD_cell_selector_fibred P hV
  obtain ⟨Mc, pcF, Ts, Tp, ī0F, j0F, mx, hmx1, hpcF1, hTs_le_mx, hTp_le_mx, hMc,
    hsufcyc, hprefcyc, hsplitData⟩ :=
    hSplitData
  obtain ⟨pN, hpN, hnClassB⟩ :=
    CopiedTie2b.achievesDstar_iff_on_n_class_of_budget P hV CbudN hbudCN hB1
  have hAchLocus := hRunResidue.1
  have hBandActive := hRunResidue.2
  have hBandSuf := hBandSound.1
  have hBandPre := hBandSound.2.1
  have hBandDeepSuf := hBandSound.2.2.1
  have hBandDeepPre := hBandSound.2.2.2
  have hBandActiveSuf := hBandActive.1
  have hBandActivePre := hBandActive.2
  obtain ⟨NDomN, pDomN, hpDomN, hDomN⟩ := CopiedTie2b.domain_EP_n_uniform_floor P
  obtain ⟨NDpN, pDpN, hpDpN, hDpN⟩ := CopiedTie2b.Dpresent_EP_n_uniform_floor P
    ...
    exact Nat.mul_pos
      (Nat.mul_pos
        (Nat.mul_pos
          ...
          (Nat.lt_of_lt_of_le Nat.zero_lt_one hpDomN))
        (Nat.lt_of_lt_of_le Nat.zero_lt_one hpDpN))
      (Nat.lt_of_lt_of_le Nat.zero_lt_one hQ1)
  have hpSel_dvd_pG : pSel ∣ pG := by
    rw [hpGdef]
    exact dvd_mul_of_dvd_left
      (dvd_mul_of_dvd_left
        (dvd_mul_of_dvd_left (dvd_mul_right pSel pN) pDomN) pDpN) Q
  have hpN_dvd_pG : pN ∣ pG := by
    rw [hpGdef]
    exact dvd_mul_of_dvd_left
      (dvd_mul_of_dvd_left
        (dvd_mul_of_dvd_left (dvd_mul_left pN pSel) pDomN) pDpN) Q
  have hpDomN_dvd_pG : pDomN ∣ pG := by
    rw [hpGdef]
    exact dvd_mul_of_dvd_left (dvd_mul_of_dvd_left (dvd_mul_left pDomN (pSel * pN)) pDpN) Q
  have hpDpN_dvd_pG : pDpN ∣ pG := by
    rw [hpGdef]
    exact dvd_mul_of_dvd_left (dvd_mul_left pDpN ((pSel * pN) * pDomN)) Q
  have hQ_dvd_pG : Q ∣ pG := by
    rw [hpGdef]
    exact dvd_mul_left Q (((pSel * pN) * pDomN) * pDpN)
  set qB : ℕ := mx + Q + 1 with hqB_def
  set Ndeep : Fin P.toPoly.K → ℕ := fun c' => mx + pcF c' with hNdeep_def
  obtain ⟨GdfaF, hGdfa⟩ :=
    bridgeRowIndex_gate_family P B Bh M mthr pG qB qB Q Ndeep hpG
      (Nat.lt_of_lt_of_le Nat.zero_lt_one hQ1) hBB hBh1 hM2
  set Mbr : ℕ := max (max (max 1 (qB + 1)) (2 * Bh + 2))
    ((Finset.univ.sup fun c' : Fin P.toPoly.K =>
      max (max (Ts c' + 2 * pcF c' + 3) (Tp c' + 2 * pcF c' + 3)) (Ndeep c' + 2))) with hMbrdef
  ...
  ·
    ...
    choose NSufActN hNSufActN using fun c' : Fin P.toPoly.K =>
      CopiedAchSetFold.tyingSuf2_fold_n_actual_fixed_period P hV hB1 pN hnClassB
        c' (Ts c') (pcF c') mS (hpcF1 c') (hactBandSuf c') (B + 1) (by omega)
    choose NPreActN hNPreActN using fun c' : Fin P.toPoly.K =>
      CopiedAchSetFold.tyingPre2_fold_n_actual_fixed_period P hV hB1 pN hnClassB
        c' (Tp c') (pcF c') mS (hpcF1 c') (hactBandPre c') (B + 1) (by omega)
    choose NDSufActN hNDSufActN using fun c' : Fin P.toPoly.K =>
      CopiedAchSetFold.deepAchSuf_fold_n_actual_fixed_period P hV hB1 pN hnClassB
        c' (Ndeep c') mS hm2 (B + 1) (by omega)
    choose NDPreActN hNDPreActN using fun c' : Fin P.toPoly.K =>
      CopiedAchSetFold.deepAchPre_fold_n_actual_fixed_period P hV hB1 pN hnClassB
        c' (Ndeep c') mS hm2 (B + 1) (by omega)
    set Nact : ℕ := max (max NDomN NDpN) (Finset.univ.sup fun c' : Fin P.toPoly.K =>
      max (max (NSufActN c') (NPreActN c')) (max (NDSufActN c') (NDPreActN c')))
      with hNact_def
    ...
    have hNDomN_floor : NDomN ≤ Nfloor := by
      ...
      exact le_trans (le_trans (le_max_left NDomN NDpN) (le_max_left _ _)) (le_max_right _ _)
    have hNDpN_floor : NDpN ≤ Nfloor := by
      rw [hNfloor_def, hNact_def]
      exact le_trans (le_trans (le_max_right NDomN NDpN) (le_max_left _ _)) (le_max_right _ _)
    have hNSuf_floor : ∀ c' : Fin P.toPoly.K, NSufActN c' ≤ Nfloor := by
      intro c'
      rw [hNfloor_def, hNact_def]
      have hsup :
          max (max (NSufActN c') (NPreActN c')) (max (NDSufActN c') (NDPreActN c'))
        ...
        Finset.le_sup
          ...
          (Finset.mem_univ c')
      exact le_trans
        (le_trans (le_trans (le_max_left (NSufActN c') (NPreActN c')) (le_max_left _ _)) hsup)
        (le_trans (le_max_right _ _) (le_max_right _ _))
    have hNPre_floor : ∀ c' : Fin P.toPoly.K, NPreActN c' ≤ Nfloor := by
      intro c'
      rw [hNfloor_def, hNact_def]
      have hsup :
          max (max (NSufActN c') (NPreActN c')) (max (NDSufActN c') (NDPreActN c'))
        ...
        Finset.le_sup
          ...
          (Finset.mem_univ c')
      exact le_trans
        (le_trans (le_trans (le_max_right (NSufActN c') (NPreActN c')) (le_max_left _ _)) hsup)
        (le_trans (le_max_right _ _) (le_max_right _ _))
    have hNDSuf_floor : ∀ c' : Fin P.toPoly.K, NDSufActN c' ≤ Nfloor := by
      intro c'
      rw [hNfloor_def, hNact_def]
      have hsup :
          max (max (NSufActN c') (NPreActN c')) (max (NDSufActN c') (NDPreActN c'))
        ...
        Finset.le_sup
          ...
          (Finset.mem_univ c')
      exact le_trans
        (le_trans (le_trans (le_max_left (NDSufActN c') (NDPreActN c')) (le_max_right _ _)) hsup)
        (le_trans (le_max_right _ _) (le_max_right _ _))
    have hNDPre_floor : ∀ c' : Fin P.toPoly.K, NDPreActN c' ≤ Nfloor := by
      intro c'
      rw [hNfloor_def, hNact_def]
      have hsup :
          max (max (NSufActN c') (NPreActN c')) (max (NDSufActN c') (NDPreActN c'))
        ...
        Finset.le_sup
          ...
          (Finset.mem_univ c')
      exact le_trans
        (le_trans (le_trans (le_max_right (NDSufActN c') (NDPreActN c')) (le_max_right _ _)) hsup)
        (le_trans (le_max_right _ _) (le_max_right _ _))
    let repN : Fin pG → ℕ := rowRep pG Nfloor
    have hrepN_modEq : ∀ r : Fin pG, Nat.ModEq pG (repN r) r.1 := by
      intro r
      exact rowRep_modEq r
    have hrepN_ge_floor : ∀ r : Fin pG, Nfloor ≤ repN r := by
      intro r
      exact rowRep_ge_floor hpG r
    have hrepN_mod_pSel : ∀ r : Fin pG, repN r % pSel = r.1 % pSel := by
      intro r
      exact Nat.ModEq.of_dvd hpSel_dvd_pG
        (hrepN_modEq r)
    have hrepN_mod_pN : ∀ r : Fin pG, repN r % pN = r.1 % pN := by
      intro r
      exact Nat.ModEq.of_dvd hpN_dvd_pG
        (hrepN_modEq r)
    have hrepN_mod_pDomN : ∀ r : Fin pG, repN r % pDomN = r.1 % pDomN := by
      intro r
      exact Nat.ModEq.of_dvd hpDomN_dvd_pG
        (hrepN_modEq r)
    have hrepN_mod_pDpN : ∀ r : Fin pG, repN r % pDpN = r.1 % pDpN := by
      intro r
      exact Nat.ModEq.of_dvd hpDpN_dvd_pG
        (hrepN_modEq r)
    have hrepN_mod_Q : ∀ r : Fin pG, repN r % Q = r.1 % Q := by
      intro r
      exact Nat.ModEq.of_dvd hQ_dvd_pG
        (hrepN_modEq r)
    ...
    set sufTie : Fin pG → (c' : Fin P.toPoly.K) → Finset ℕ := fun r c' =>
      (Finset.range (pcF c')).filter (fun ρ =>
        P.rank c' (copiedSlice mS (repN r))
            (CopiedDstar.cellTupleF
              (fun _ : Fin (P.toPoly.arity c') =>
                (CopiedCells.RegionSpecF.sufIdx (Ts c' + ρ) : CopiedCells.RegionSpecF B))
              mS (B + 1) (repN r))
          = CopiedDstar.dstarRankGA_m P hV mS (repN r)
        ∧ P.rank c' (copiedSlice mS (repN r))
            (CopiedDstar.cellTupleF
              (fun _ : Fin (P.toPoly.arity c') =>
                (CopiedCells.RegionSpecF.sufIdx (Ts c' + ρ + pcF c') : CopiedCells.RegionSpecF B))
              mS (B + 1) (repN r))
          = CopiedDstar.dstarRankGA_m P hV mS (repN r)) with hsufTie_def
    set preTie : Fin pG → (c' : Fin P.toPoly.K) → Finset ℕ := fun r c' =>
      (Finset.range (pcF c')).filter (fun ρ =>
        P.rank c' (copiedSlice mS (repN r))
            (CopiedDstar.cellTupleF
              (fun _ : Fin (P.toPoly.arity c') =>
                (CopiedCells.RegionSpecF.prefIdx (Tp c' + ρ) : CopiedCells.RegionSpecF B))
              mS (B + 1) (repN r))
          = CopiedDstar.dstarRankGA_m P hV mS (repN r)
        ∧ P.rank c' (copiedSlice mS (repN r))
            (CopiedDstar.cellTupleF
              (fun _ : Fin (P.toPoly.arity c') =>
                (CopiedCells.RegionSpecF.prefIdx (Tp c' + ρ + pcF c') : CopiedCells.RegionSpecF B))
              mS (B + 1) (repN r))
          = CopiedDstar.dstarRankGA_m P hV mS (repN r)) with hpreTie_def
    set Ssuf : Fin pG → (c' : Fin P.toPoly.K) → Finset ℕ := fun r c' =>
      (Finset.range Q).filter (fun ρ =>
        ((ρ + Q - ((mS + 2 * repN r + 1 + Ts c') % Q)) % pcF c') ∈ sufTie r c')
      with hSsuf_def
    set Spre : Fin pG → (c' : Fin P.toPoly.K) → Finset ℕ := fun r c' =>
      (Finset.range Q).filter (fun ρ =>
        ((ρ + Q - ((Tp c') % Q)) % pcF c') ∈ preTie r c')
      with hSpre_def
    set Dsuf : Fin pG → (c' : Fin P.toPoly.K) → Finset ℕ := fun r c' =>
      (Finset.range (Ndeep c')).filter (fun k =>
        P.rank c' (copiedSlice mS (repN r))
            (fun _ : Fin (P.toPoly.arity c') =>
              mixedPosAt
                (Sum.inr (Sum.inl (k + 1)) : CopiedCells.RegionSpecF B ⊕ (ℕ ⊕ ℕ))
                mS (B + 1) (repN r))
          = CopiedDstar.dstarRankGA_m P hV mS (repN r)) with hDsuf_def
    set Dpre : Fin pG → (c' : Fin P.toPoly.K) → Finset ℕ := fun r c' =>
      ((Finset.range (Ndeep c')).filter (fun k =>
        P.rank c' (copiedSlice mS (repN r))
            (fun _ : Fin (P.toPoly.arity c') =>
              mixedPosAt
          ...
          = CopiedDstar.dstarRankGA_m P hV mS (repN r))).image (fun k => k + 3)
      with hDpre_def
    have hSsuf_bound : ∀ r c' x, x ∈ Ssuf r c' → x < Q := by
      intro r c' x hx
      rw [hSsuf_def] at hx
      exact Finset.mem_range.mp (Finset.mem_filter.mp hx).1
    have hSpre_bound : ∀ r c' x, x ∈ Spre r c' → x < Q := by
      intro r c' x hx
      rw [hSpre_def] at hx
      exact Finset.mem_range.mp (Finset.mem_filter.mp hx).1
    have hDsuf_bound : ∀ r c' x, x ∈ Dsuf r c' → x < Ndeep c' + 3 := by
      intro r c' x hx
      rw [hDsuf_def] at hx
      have hxN := Finset.mem_range.mp (Finset.mem_filter.mp hx).1
      omega
    have hDpre_bound : ∀ r c' x, x ∈ Dpre r c' → x < Ndeep c' + 3 := by
      intro r c' x hx
      rw [hDpre_def] at hx
    ...
    let idx : BridgeRowIndex P B Bh M pG qB qB Q Ndeep :=
      mkBridgeRowIndex (M := M) (Q := Q) (Ndeep := Ndeep)
        S1tab F2tab Ssuf Spre Dsuf Dpre
        hS1tab_bound hF2tab_bound hSsuf_bound hSpre_bound hDsuf_bound hDpre_bound
    refine ⟨idx, Nfloor, ?_⟩
    intro n hn hdom hDpres c ī hval
    obtain ⟨FFs, FFp, Nc, hNcmS, hNcmx, hNcs, hNcp, hFs, hFp, hags, hagp, hsplit⟩ :=
    ...
    have hBh_n : Bh ≤ n := by
      ...
      omega
    have hrowClass_mod_pSel :
        (rowClass hpG (n % pG)).1 % pSel = n % pSel := by
      dsimp [rowClass]
      rw [Nat.mod_mod]
      exact Nat.ModEq.of_dvd hpSel_dvd_pG
        (Nat.mod_modEq n pG : Nat.ModEq pG (n % pG) n)
    have hrowClass_mod_pN :
        (rowClass hpG (n % pG)).1 % pN = n % pN := by
      dsimp [rowClass]
      rw [Nat.mod_mod]
      exact Nat.ModEq.of_dvd hpN_dvd_pG
        (Nat.mod_modEq n pG : Nat.ModEq pG (n % pG) n)
    have hrowClass_mod_pDomN :
        (rowClass hpG (n % pG)).1 % pDomN = n % pDomN := by
      dsimp [rowClass]
      rw [Nat.mod_mod]
      exact Nat.ModEq.of_dvd hpDomN_dvd_pG
        (Nat.mod_modEq n pG : Nat.ModEq pG (n % pG) n)
    have hrowClass_mod_pDpN :
        (rowClass hpG (n % pG)).1 % pDpN = n % pDpN := by
      dsimp [rowClass]
      rw [Nat.mod_mod]
      exact Nat.ModEq.of_dvd hpDpN_dvd_pG
        (Nat.mod_modEq n pG : Nat.ModEq pG (n % pG) n)
    have hrowClass_mod_Q :
        (rowClass hpG (n % pG)).1 % Q = n % Q := by
      dsimp [rowClass]
      rw [Nat.mod_mod]
      exact Nat.ModEq.of_dvd hQ_dvd_pG
        (Nat.mod_modEq n pG : Nat.ModEq pG (n % pG) n)
    rw [← hGdfa idx (n % pG) c mS n ī hqB_lt hqB_lt hBh_n hval]
    constructor
    · rintro ⟨hs, hl, hr, hall⟩
      refine ⟨?_, hr⟩
      refine ⟨hs, hl, ?_, ?_, ?_, ?_, ?_⟩
      · intro b hbsel hbD hbcfg
        refine hall b hbsel hbD ?_
        have hbcfg' : CopiedTieGate.cfgCellGAFL B Bh M mthr (S1L (n % pSel))
            (fun _ _ => ∅) (fun _ _ => ∅) (fun _ _ => ∅) (F2L (n % pSel))
            (fun _ _ => ∅) mS n b := by
          rcases hbcfg with hbulk | hfrozen
          · left
            rcases hbulk with ⟨rs, t, hvalid, htn, hcell, hpos⟩
            refine ⟨rs, t, hvalid, htn, hcell, ?_⟩
            rcases hpos with ⟨hwin, hinner⟩
            ...
            · rcases hres with ⟨hlo, hhi, r, hr, hmod⟩
              ...
              by_cases hbnd :
                  rs ∈ CopiedBoundedGate.boundedTuplesF B (P.toPoly.arity b.1) qB qB
              · have hrow :
                    rowS1Odd hpG idx (n % pG) b.1 rs =
                      S1tab (rowClass hpG (n % pG)) ⟨b.1, ⟨rs, hbnd⟩⟩ := by
                  dsimp [idx]
                  exact rowS1Odd_mkBridgeRowIndex hpG S1tab F2tab Ssuf Spre Dsuf Dpre
                    hS1tab_bound hF2tab_bound hSsuf_bound hSpre_bound hDsuf_bound hDpre_bound
                    hS1tab_odd (n % pG) b.1 rs hbnd
                rw [hrow] at hr
                rw [hS1tab_def] at hr
                simpa [hrowClass_mod_pSel] using hr
              · exfalso
                have hrow : rowS1Odd hpG idx (n % pG) b.1 rs = ∅ := by
                  dsimp [idx]
                  rw [rowS1Odd,
                    rowS1_mkBridgeRowIndex_notMem hpG S1tab F2tab Ssuf Spre Dsuf Dpre
                      hS1tab_bound hF2tab_bound hSsuf_bound hSpre_bound hDsuf_bound hDpre_bound
                      (n % pG) b.1 rs hbnd]
                  simp
                rw [hrow] at hr
          ...
          · right
            ...
            · rcases hfront with ⟨f, hf, hcase⟩
              ...
              by_cases hbnd :
                  rs ∈ CopiedBoundedGate.boundedTuplesF Bh (P.toPoly.arity b.1) qB qB
              · have hrow :
                    rowF2Odd hpG idx (n % pG) b.1 rs =
                      F2tab (rowClass hpG (n % pG)) ⟨b.1, ⟨rs, hbnd⟩⟩ := by
                  dsimp [idx]
                  exact rowF2Odd_mkBridgeRowIndex hpG S1tab F2tab Ssuf Spre Dsuf Dpre
                    hS1tab_bound hF2tab_bound hSsuf_bound hSpre_bound hDsuf_bound hDpre_bound
                    hF2tab_odd (n % pG) b.1 rs hbnd
                rw [hrow] at hf
                rw [hF2tab_def] at hf
                simpa [hrowClass_mod_pSel] using hf
              · exfalso
                have hrow : rowF2Odd hpG idx (n % pG) b.1 rs = ∅ := by
                  dsimp [idx]
                  rw [rowF2Odd,
                    rowF2_mkBridgeRowIndex_notMem hpG S1tab F2tab Ssuf Spre Dsuf Dpre
                      hS1tab_bound hF2tab_bound hSsuf_bound hSpre_bound hDsuf_bound hDpre_bound
                      (n % pG) b.1 rs hbnd]
                  simp
                rw [hrow] at hf
            ...
            · exfalso
              rcases hback with ⟨k, hk, _⟩
              exact Finset.notMem_empty k hk
        exact (hsel n hNsel_n hdom hDpres b hbsel hbD).mpr hbcfg'
      ·
        -- suffix activation soundness
        intro c' r hrmem xb hxbval hcond
        rcases hcond with ⟨hrun, hselxb, hlabelxb⟩
        refine hall ⟨c', xb⟩ ⟨hxbval, hselxb⟩ hlabelxb ?_
        let rcls : Fin pG := rowClass hpG (n % pG)
        have hrow : rowSsuf hpG idx (n % pG) c' = Ssuf rcls c' := by
          dsimp [idx, rcls]
          exact rowSsuf_mkBridgeRowIndex hpG S1tab F2tab Ssuf Spre Dsuf Dpre
            hS1tab_bound hF2tab_bound hSsuf_bound hSpre_bound hDsuf_bound hDpre_bound
            (n % pG) c'
        rw [hrow, hSsuf_def] at hrmem
        have hrQ : r < Q := Finset.mem_range.mp (Finset.mem_filter.mp hrmem).1
        have hρrep_mem := (Finset.mem_filter.mp hrmem).2
        have hrep_pN : repN rcls % pN = n % pN := by
          rw [hrepN_mod_pN rcls]
          dsimp [rcls]
          exact hrowClass_mod_pN
        have hrep_pDomN : repN rcls % pDomN = n % pDomN := by
          rw [hrepN_mod_pDomN rcls]
          dsimp [rcls]
          exact hrowClass_mod_pDomN
        have hrep_pDpN : repN rcls % pDpN = n % pDpN := by
          rw [hrepN_mod_pDpN rcls]
          dsimp [rcls]
          exact hrowClass_mod_pDpN
        have hdom_rep : P.toPoly.domain (copiedSlice mS (repN rcls)) :=
          (SliceFasSelector.iff_on_class (Pr := fun x => P.toPoly.domain (copiedSlice mS x))
            hpDomN (hDomN mS hm) (le_trans hNDomN_floor hn)
            (le_trans hNDomN_floor (hrepN_ge_floor rcls)) hrep_pDomN.symm).mp hdom
        have hDpres_rep :
            ∃ a, P.toPoly.selectedAtom (copiedSlice mS (repN rcls)) a ∧
              P.toPoly.labelOf (copiedSlice mS (repN rcls)) a = D :=
          (SliceFasSelector.iff_on_class
            (Pr := fun x => ∃ a, P.toPoly.selectedAtom (copiedSlice mS x) a ∧
              P.toPoly.labelOf (copiedSlice mS x) a = D)
            hpDpN (hDpN mS hm) (le_trans hNDpN_floor hn)
            (le_trans hNDpN_floor (hrepN_ge_floor rcls)) hrep_pDpN.symm).mp hDpres
        have hB_n : B + 1 + B + 1 ≤ n := by
          have := le_trans hBh_floor hn
          omega
        have hB_rep : B + 1 + B + 1 ≤ repN rcls := by
          have := le_trans hBh_floor (hrepN_ge_floor rcls)
          omega
        have hsets := hNSufActN c' n (repN rcls)
          (le_trans (hNSuf_floor c') hn)
          (le_trans (hNSuf_floor c') (hrepN_ge_floor rcls))
          hB_n hB_rep hrep_pN.symm ⟨hdom, hDpres⟩ ⟨hdom_rep, hDpres_rep⟩
        have hrep_mod_Q : repN rcls ≡ n [MOD Q] := by
          rw [Nat.ModEq, hrepN_mod_Q rcls]
          dsimp [rcls]
          exact hrowClass_mod_Q
        have hshift_mod_Q :
            (mS + 2 * repN rcls + 1 + Ts c') % Q =
              (mS + 2 * n + 1 + Ts c') % Q := by
          have htwo : 2 * repN rcls ≡ 2 * n [MOD Q] := Nat.ModEq.mul_left 2 hrep_mod_Q
          exact Nat.ModEq.add_right (Ts c') (Nat.ModEq.add_right 1 (Nat.ModEq.add_left mS htwo))
        have hρeq :
            ((r + Q - ((mS + 2 * repN rcls + 1 + Ts c') % Q)) % pcF c') =
              ((r + Q - ((mS + 2 * n + 1 + Ts c') % Q)) % pcF c') := by
          rw [hshift_mod_Q]
        have hρact_mem :
            ((r + Q - ((mS + 2 * n + 1 + Ts c') % Q)) % pcF c') ∈
              {ρ ∈ Finset.range (pcF c') |
                P.rank c' (copiedSlice mS n)
                      (CopiedDstar.cellTupleF
                        (fun _ : Fin (P.toPoly.arity c') =>
                          (CopiedCells.RegionSpecF.sufIdx (Ts c' + ρ) :
                            CopiedCells.RegionSpecF B)) mS (B + 1) n) =
                    CopiedDstar.dstarRankGA_m P hV mS n ∧
                  P.rank c' (copiedSlice mS n)
                      (CopiedDstar.cellTupleF
                        (fun _ : Fin (P.toPoly.arity c') =>
                          (CopiedCells.RegionSpecF.sufIdx (Ts c' + ρ + pcF c') :
                            CopiedCells.RegionSpecF B)) mS
                          (B + 1) n) =
                    CopiedDstar.dstarRankGA_m P hV mS n} := by
          rw [hsets]
          rwa [← hρeq]
        set ρact : ℕ := (r + Q - ((mS + 2 * n + 1 + Ts c') % Q)) % pcF c' with hρact_def
        have hρact_lt : ρact < pcF c' := by
          exact Finset.mem_range.mp (Finset.mem_filter.mp hρact_mem).1
        have hact := (Finset.mem_filter.mp hρact_mem).2
        show P.rankOf (copiedSlice mS n) (⟨c', xb⟩ : P.toPoly.Atom) =
          CopiedDstar.dstarRankGA_m P hV mS n
        show P.rank c' (copiedSlice mS n) xb = CopiedDstar.dstarRankGA_m P hV mS n
        exact hBandSuf c' (j0F c') (ī0F c') (B := B) mS n
          (pcF c') (Ts c') Q (qB + 2) (FFs c')
          hm (by omega) (hpcF1 c') (Nat.lt_of_lt_of_le Nat.zero_lt_one hQ1) (hpcF_dvd_Q c')
          (by omega) (by have := hTs_le_mx c'; rw [hqB_def]; omega) (hactBandSuf c')
          (hFs c') (hags c') hrQ hρact_def hact.1 hact.2 xb
          hxbval hrun
      ·
          -- prefix activation soundness
          intro c' r hrmem xb hxbval hcond
          rcases hcond with ⟨⟨hpreGuard, hselxb⟩, hlabelxb⟩
          refine hall ⟨c', xb⟩ ⟨hxbval, hselxb⟩ hlabelxb ?_
          let rcls : Fin pG := rowClass hpG (n % pG)
          have hrow : rowSpre hpG idx (n % pG) c' = Spre rcls c' := by
            dsimp [idx, rcls]
            exact rowSpre_mkBridgeRowIndex hpG S1tab F2tab Ssuf Spre Dsuf Dpre
              hS1tab_bound hF2tab_bound hSsuf_bound hSpre_bound hDsuf_bound hDpre_bound
              (n % pG) c'
          rw [hrow, hSpre_def] at hrmem
          have hrQ : r < Q := Finset.mem_range.mp (Finset.mem_filter.mp hrmem).1
          have hρrep_mem := (Finset.mem_filter.mp hrmem).2
          have hrep_pN : repN rcls % pN = n % pN := by
            rw [hrepN_mod_pN rcls]
          ...
          have hB_rep : B + 1 + B + 1 ≤ repN rcls := by
            have := le_trans hBh_floor (hrepN_ge_floor rcls)
            omega
          have hsets := hNPreActN c' n (repN rcls)
            (le_trans (hNPre_floor c') hn)
            (le_trans (hNPre_floor c') (hrepN_ge_floor rcls))
            hB_n hB_rep hrep_pN.symm ⟨hdom, hDpres⟩ ⟨hdom_rep, hDpres_rep⟩
          have hρact_mem :
              ((r + Q - ((Tp c') % Q)) % pcF c') ∈
                {ρ ∈ Finset.range (pcF c') |
                  P.rank c' (copiedSlice mS n)
                        (CopiedDstar.cellTupleF
                          (fun _ : Fin (P.toPoly.arity c') =>
                            (CopiedCells.RegionSpecF.prefIdx (Tp c' + ρ) :
                              CopiedCells.RegionSpecF B)) mS (B + 1) n) =
                      CopiedDstar.dstarRankGA_m P hV mS n ∧
                    P.rank c' (copiedSlice mS n)
                        (CopiedDstar.cellTupleF
                          (fun _ : Fin (P.toPoly.arity c') =>
                            (CopiedCells.RegionSpecF.prefIdx (Tp c' + ρ + pcF c') :
                              CopiedCells.RegionSpecF B)) mS
                            (B + 1) n) =
                      CopiedDstar.dstarRankGA_m P hV mS n} := by
            rw [hsets]
            exact hρrep_mem
          set ρact : ℕ := (r + Q - ((Tp c') % Q)) % pcF c' with hρact_def
          have hρact_lt : ρact < pcF c' := by
            exact Finset.mem_range.mp (Finset.mem_filter.mp hρact_mem).1
          have hact := (Finset.mem_filter.mp hρact_mem).2
          show P.rankOf (copiedSlice mS n) (⟨c', xb⟩ : P.toPoly.Atom) =
            CopiedDstar.dstarRankGA_m P hV mS n
          show P.rank c' (copiedSlice mS n) xb = CopiedDstar.dstarRankGA_m P hV mS n
          exact hBandPre c' (j0F c') (ī0F c') (B := B) mS n
            (pcF c') (Tp c') Q qB (FFp c')
            hm (by omega) (hpcF1 c') (Nat.lt_of_lt_of_le Nat.zero_lt_one hQ1) (hpcF_dvd_Q c')
            (by have := hTp_le_mx c'; rw [hqB_def]; omega) (hactBandPre c')
            (hFp c') (hagp c') hrQ hρact_def hact.1 hact.2 xb hxbval hpreGuard
      ·
        -- deep suffix activation soundness
        intro c' k hkmem xb hxbval hcond
        rcases hcond with ⟨hguard, hselxb, hlabelxb⟩
        refine hall ⟨c', xb⟩ ⟨hxbval, hselxb⟩ hlabelxb ?_
        let rcls : Fin pG := rowClass hpG (n % pG)
        have hrow : rowDsuf hpG idx (n % pG) c' = Dsuf rcls c' := by
          dsimp [idx, rcls]
          exact rowDsuf_mkBridgeRowIndex hpG S1tab F2tab Ssuf Spre Dsuf Dpre
            hS1tab_bound hF2tab_bound hSsuf_bound hSpre_bound hDsuf_bound hDpre_bound
            (n % pG) c'
        rw [hrow, hDsuf_def] at hkmem
        have hrep_pN : repN rcls % pN = n % pN := by
          rw [hrepN_mod_pN rcls]
          dsimp [rcls]
          exact hrowClass_mod_pN
        ...
        have hB_rep : B + 1 + B + 1 ≤ repN rcls := by
          have := le_trans hBh_floor (hrepN_ge_floor rcls)
          omega
        have hsets := hNDSufActN c' n (repN rcls)
          (le_trans (hNDSuf_floor c') hn)
          (le_trans (hNDSuf_floor c') (hrepN_ge_floor rcls))
          hB_n hB_rep hrep_pN.symm ⟨hdom, hDpres⟩ ⟨hdom_rep, hDpres_rep⟩
        have hactive_mem :
            k ∈ {i_off ∈ Finset.range (Ndeep c') |
              P.rank c' (copiedSlice mS n)
                  (fun _ : Fin (P.toPoly.arity c') =>
                    mixedPosAt
                      (Sum.inr (Sum.inl (i_off + 1)) : CopiedCells.RegionSpecF B ⊕ (ℕ ⊕ ℕ))
                      mS (B + 1) n)
                = CopiedDstar.dstarRankGA_m P hV mS n} := by
          rw [hsets]
          exact hkmem
        have hactive := (Finset.mem_filter.mp hactive_mem).2
        show P.rankOf (copiedSlice mS n) (⟨c', xb⟩ : P.toPoly.Atom) =
          CopiedDstar.dstarRankGA_m P hV mS n
        show P.rank c' (copiedSlice mS n) xb = CopiedDstar.dstarRankGA_m P hV mS n
        exact hBandDeepSuf c' (j0F c') (B := B) mS n (B + 1) k
          (by
            have hkN : k < Ndeep c' := Finset.mem_range.mp (Finset.mem_filter.mp hkmem).1
            have hNle := hNdeep_add2_le_mS c'
            omega)
          hactive xb hguard
      ·
        -- deep prefix activation soundness
        intro c' k hkmem xb hxbval hcond
        rcases hcond with ⟨hguard, hselxb, hlabelxb⟩
        refine hall ⟨c', xb⟩ ⟨hxbval, hselxb⟩ hlabelxb ?_
        let rcls : Fin pG := rowClass hpG (n % pG)
        have hrow : rowDpre hpG idx (n % pG) c' = Dpre rcls c' := by
          dsimp [idx, rcls]
          exact rowDpre_mkBridgeRowIndex hpG S1tab F2tab Ssuf Spre Dsuf Dpre
            hS1tab_bound hF2tab_bound hSsuf_bound hSpre_bound hDsuf_bound hDpre_bound
            (n % pG) c'
        rw [hrow, hDpre_def] at hkmem
        obtain ⟨k0, hk0mem, hk_eq⟩ := Finset.mem_image.mp hkmem
        have hrep_pN : repN rcls % pN = n % pN := by
          rw [hrepN_mod_pN rcls]
          dsimp [rcls]
          exact hrowClass_mod_pN
        ...
        have hB_rep : B + 1 + B + 1 ≤ repN rcls := by
          have := le_trans hBh_floor (hrepN_ge_floor rcls)
          omega
        have hsets := hNDPreActN c' n (repN rcls)
          (le_trans (hNDPre_floor c') hn)
          (le_trans (hNDPre_floor c') (hrepN_ge_floor rcls))
          hB_n hB_rep hrep_pN.symm ⟨hdom, hDpres⟩ ⟨hdom_rep, hDpres_rep⟩
        have hactive_mem :
            k0 ∈ {i_off ∈ Finset.range (Ndeep c') |
              P.rank c' (copiedSlice mS n)
                  (fun _ : Fin (P.toPoly.arity c') =>
                    mixedPosAt
                      (Sum.inr (Sum.inr (i_off + 1)) : CopiedCells.RegionSpecF B ⊕ (ℕ ⊕ ℕ))
                      mS (B + 1) n)
                = CopiedDstar.dstarRankGA_m P hV mS n} := by
          rw [hsets]
          exact hk0mem
        have hactive0 := (Finset.mem_filter.mp hactive_mem).2
        subst k
        have hshift : k0 + 3 - 2 = k0 + 1 := by omega
        have hactive :
            P.rank c' (copiedSlice mS n)
        ...
        show P.rankOf (copiedSlice mS n) (⟨c', xb⟩ : P.toPoly.Atom) =
          CopiedDstar.dstarRankGA_m P hV mS n
        show P.rank c' (copiedSlice mS n) xb = CopiedDstar.dstarRankGA_m P hV mS n
        exact hBandDeepPre c' (j0F c') (B := B) mS n (B + 1) (k0 + 3)
          hm (by omega) (by omega) hactive xb hguard
    ·
      -- backward semantic coverage via the canonical split and active row tables
      rintro ⟨hgate, hr⟩
      have hcore_from_cfg :
          ∀ (b : P.toPoly.Atom),
            P.toPoly.selectedAtom (copiedSlice mS n) b →
            ...
            (∀ (rs : Fin (P.toPoly.arity b.1) → CopiedCells.RegionSpecF Bh) (t : ℕ),
              ...
              rs ∈ CopiedBoundedGate.boundedTuplesF Bh (P.toPoly.arity b.1) qB qB) →
            P.toPoly.atomOrd (copiedSlice mS n) ⟨c, ī⟩ b := by
        intro b hbsel hbD hbcfg hbndB hbndH
        refine hgate.2.2.1 b hbsel hbD ?_
        rcases hbcfg with hbulk | hfrozen
        · left
          rcases hbulk with ⟨rs, t, hvalid, htn, hcell, hpos⟩
          have hbnd : rs ∈ CopiedBoundedGate.boundedTuplesF B (P.toPoly.arity b.1) qB qB :=
            hbndB rs t hvalid htn hcell hpos
          refine ⟨rs, t, hvalid, htn, hcell, ?_⟩
          rcases hpos with ⟨hwin, hinner⟩
          refine ⟨hwin, ?_⟩
        ...
        · right
          rcases hfrozen with ⟨rs, t, hvalid, htn, hcell, hpos⟩
          have hbnd : rs ∈ CopiedBoundedGate.boundedTuplesF Bh (P.toPoly.arity b.1) qB qB :=
            hbndH rs t hvalid htn hcell hpos
          refine ⟨rs, t, hvalid, htn, hcell, ?_⟩
          rcases hpos with ⟨hwin, hinner⟩
          refine ⟨hwin, ?_⟩
      ...
      have hdeep_suf_order :
          ∀ (c' : Fin P.toPoly.K) (l : ℕ),
            ...
            P.toPoly.atomOrd (copiedSlice mS n) ⟨c, ī⟩
              (⟨c', fun _ : Fin (P.toPoly.arity c') => mS + 2 * n + 1 + l⟩ :
                P.toPoly.Atom) := by
        intro c' l hlm hqBl hnotHi hselRun hachRun
        have hgeNc : Nc ≤ l + pcF c' := by
          by_contra hlt
          exact hnotHi (by omega)
        set k : ℕ := mS - 2 - l with hk_def
        have hk_add : l + k = mS - 2 := by
          rw [hk_def]
          omega
        have htailOff : mS - 1 - (k + 1) = l := by
          rw [hk_def]
        ...
        have hactive_mem :
            k ∈ {i_off ∈ Finset.range (Ndeep c') |
              P.rank c' (copiedSlice mS n)
                  (fun _ : Fin (P.toPoly.arity c') =>
                    mixedPosAt
                      (Sum.inr (Sum.inl (i_off + 1)) :
                        CopiedCells.RegionSpecF B ⊕ (ℕ ⊕ ℕ))
                      mS (B + 1) n)
                = CopiedDstar.dstarRankGA_m P hV mS n} := by
          refine Finset.mem_filter.mpr ⟨Finset.mem_range.mpr hkN, ?_⟩
          rw [← htupleDeep]
          exact hachRun
        have hactive_rep :
            k ∈ {i_off ∈ Finset.range (Ndeep c') |
              P.rank c' (copiedSlice mS (repN rcls))
                  (fun _ : Fin (P.toPoly.arity c') =>
                    mixedPosAt
                      (Sum.inr (Sum.inl (i_off + 1)) :
                        CopiedCells.RegionSpecF B ⊕ (ℕ ⊕ ℕ))
                      mS (B + 1) (repN rcls))
                = CopiedDstar.dstarRankGA_m P hV mS (repN rcls)} := by
          rw [← hsets]
          exact hactive_mem
        have hrmem : k ∈ rowDsuf hpG idx (n % pG) c' := by
          rw [hrow, hDsuf_def]
          exact hactive_rep
        have hvalSuf :
            ∀ i, (fun _ : Fin (P.toPoly.arity c') => mS + 2 * n + 1 + l) i
              < (copiedSlice mS n).length := by
          intro i
          change mS + 2 * n + 1 + l < (copiedSlice mS n).length
          rw [length_copiedSlice]
          omega
        exact hgate.2.2.2.2.2.1 c' k hrmem
          (fun _ : Fin (P.toPoly.arity c') => mS + 2 * n + 1 + l)
          hvalSuf
          (by
            have hrunTail :=
              (CopiedBandRunGate.suffixBandGuard_copiedSlice mS n 0 l hm (by omega)
                hlm (Nat.zero_le l)).1
            refine ⟨?_, hselRun.1, hselRun.2⟩
            intro i
            refine ⟨hrunTail, ?_⟩
            rw [length_copiedSlice]
            omega)
      have hdeep_pre_order :
          ∀ (c' : Fin P.toPoly.K) (l : ℕ),
            l < mS - 1 → qB ≤ l → ¬ l + pcF c' < Nc →
            (P.toPoly.sel c' (copiedSlice mS n)
                (fun _ : Fin (P.toPoly.arity c') => l)
            ...
            P.rank c' (copiedSlice mS n)
              ...
              = CopiedDstar.dstarRankGA_m P hV mS n →
            P.toPoly.atomOrd (copiedSlice mS n) ⟨c, ī⟩
              (⟨c', fun _ : Fin (P.toPoly.arity c') => l⟩ : P.toPoly.Atom) := by
        intro c' l hlm hqBl hnotHi hselRun hachRun
        have hgeNc : Nc ≤ l + pcF c' := by
          by_contra hlt
          exact hnotHi (by omega)
        set k0 : ℕ := mS - 2 - l with hk0_def
        have hk0_add : l + k0 = mS - 2 := by
          rw [hk0_def]
          omega
        have hpreOff : mS - 1 - (k0 + 1) = l := by
          rw [hk0_def]
        ...
        have hactive_mem :
            k0 ∈ {i_off ∈ Finset.range (Ndeep c') |
              P.rank c' (copiedSlice mS n)
                  (fun _ : Fin (P.toPoly.arity c') =>
                    mixedPosAt
                      (Sum.inr (Sum.inr (i_off + 1)) :
                        CopiedCells.RegionSpecF B ⊕ (ℕ ⊕ ℕ))
                      mS (B + 1) n)
                = CopiedDstar.dstarRankGA_m P hV mS n} := by
          refine Finset.mem_filter.mpr ⟨Finset.mem_range.mpr hk0N, ?_⟩
          rw [← htupleDeep]
          exact hachRun
        have hactive_rep :
            k0 ∈ {i_off ∈ Finset.range (Ndeep c') |
          ...
          rw [← hsets]
          exact hactive_mem
        have hrmem : k0 + 3 ∈ rowDpre hpG idx (n % pG) c' := by
          rw [hrow, hDpre_def]
          exact Finset.mem_image.mpr ⟨k0, hactive_rep, rfl⟩
        have hvalPre :
            ∀ i, (fun _ : Fin (P.toPoly.arity c') => l) i
              < (copiedSlice mS n).length := by
          intro i
          change l < (copiedSlice mS n).length
          rw [length_copiedSlice]
          omega
        exact hgate.2.2.2.2.2.2 c' (k0 + 3) hrmem
          (fun _ : Fin (P.toPoly.arity c') => l)
          hvalPre
          (by
            have hpreGuard :=
              CopiedBandRunGate.preBandStrictGuard_copiedSlice mS n Q qB l hm (by omega)
                hlm hqBl
            have hfirst := CopiedBandRunGate.firstDGuard_copiedSlice mS n hm (by omega)
            refine ⟨?_, hselRun.1, hselRun.2⟩
            intro i
            refine ⟨hpreGuard.1.1, ?_⟩
            have hpin : l + (k0 + 3) = mS + 1 := by omega
            constructor
            · rw [hpin]
              exact hfirst.1
            · intro q hq
              rw [hpin] at hq
              exact hfirst.2 q hq)
      refine ⟨hgate.1, hgate.2.1, hr, ?_⟩
      intro b hbsel hbD hbach
      have hwhole :
          ∀ b : P.toPoly.Atom,
            (P.toPoly.selectedAtom (copiedSlice mS n) b ∧
              P.toPoly.labelOf (copiedSlice mS n) b = D) →
            P.rankOf (copiedSlice mS n) b = CopiedDstar.dstarRankGA_m P hV mS n →
            P.toPoly.atomOrd (copiedSlice mS n) ⟨c, ī⟩ b := by
        apply (hsplit c ī).mpr
        refine ⟨?_, ?_, ?_⟩
        ·
          -- core / shallow / deep-boundary coverage
          exact hCoreBoundary (B := B) (Bh := Bh) (M := M) (mthr := mthr)
            (qB := qB) (Q := Q) (mS := mS) (n := n) (Nc := Nc) (mx := mx)
            (pcF := pcF) (Ts := Ts) (Tp := Tp) j0F
            (S1 := S1L (n % pSel)) (F2 := F2L (n % pSel)) (c := c) (ī := ī)
            hm hqB_lt hQ1 hqB_def hpcF_dvd_Q hTs_le_mx hTp_le_mx hNcmx
            hcore_from_cfg hdeep_suf_order hdeep_pre_order
            (by
              intro b hbsel hbD hbrank
              exact (hsel n hNsel_n hdom hDpres b hbsel hbD).mp hbrank)
        ·
          -- suffix-run residue coverage
          intro r hclass b hsuf hcls hselD
          rcases b with ⟨c', xb⟩
          rcases hsuf with ⟨l, hlo, hhi, hxbeq⟩
          change xb = (fun _ : Fin (P.toPoly.arity c') => mS + 2 * n + 1 + l) at hxbeq
          subst xb
          have hbachS :
          ...
          · have hrawcfg :
                CopiedTieGate.cfgCellGAFL B Bh M mthr (S1L (n % pSel))
                  ...
                  (fun _ _ => ∅) mS n
                    (⟨c', fun _ : Fin (P.toPoly.arity c') => mS + 2 * n + 1 + l⟩ :
                P.toPoly.Atom) :=
              (hsel n hNsel_n hdom hDpres
                (⟨c', fun _ : Fin (P.toPoly.arity c') => mS + 2 * n + 1 + l⟩ :
                  P.toPoly.Atom) hselD.1 hselD.2).mp hbachS
            exact hcore_from_cfg
              (⟨c', fun _ : Fin (P.toPoly.arity c') => mS + 2 * n + 1 + l⟩ :
                P.toPoly.Atom) hselD.1 hselD.2 hrawcfg
              (by
                intro rs t hvalid htn hcell _hpos
                rw [CopiedBoundedGate.mem_boundedTuplesF]
                intro i
              ...
              (by
                ...
                | sufIdx l' =>
                    ...
                    omega)
          · have hlocus :=
              hAchLocus.1 c' (j0F c') (ī0F c') mS n (pcF c') (Ts c') Nc qB mx
                (FFs c') (hpcF1 c') hTqS (hFs c') (hags c')
                (CopiedSelUniform.selconst_suf P c' (Mc c') (hMc c') mS n
                  (pcF c') (Ts c') Nc hm (hNcs c') (hsufcyc c'))
                hNcmS hNcmx l hlm hselRun hachRun
            rcases hlocus with hsh' | hslope | hdeep
            · exact False.elim (hsh hsh')
            ·
              set rAbs : ℕ := (mS + 2 * n + 1 + l) % Q with hrAbs_def
              have hrAbsQ : rAbs < Q := by
                rw [hrAbs_def]
                exact Nat.mod_lt _ (Nat.lt_of_lt_of_le Nat.zero_lt_one hQ1)
              let rcls : Fin pG := rowClass hpG (n % pG)
              have hrow : rowSsuf hpG idx (n % pG) c' = Ssuf rcls c' := by
                dsimp [idx, rcls]
                exact rowSsuf_mkBridgeRowIndex hpG S1tab F2tab Ssuf Spre Dsuf Dpre
              ...
              have hsets := hNSufActN c' n (repN rcls)
                ...
                (le_trans (hNSuf_floor c') (hrepN_ge_floor rcls))
                hB_n hB_rep hrep_pN.symm ⟨hdom, hDpres⟩ ⟨hdom_rep, hDpres_rep⟩
              set ρact : ℕ := (rAbs + Q - ((mS + 2 * n + 1 + Ts c') % Q)) % pcF c'
                with hρact_def
              have hρact_lt : ρact < pcF c' := by
                rw [hρact_def]
                exact Nat.mod_lt _ (Nat.lt_of_lt_of_le Nat.zero_lt_one (hpcF1 c'))
              have hresloc : ρact = (l - Ts c') % pcF c' := by
                have hres := CopiedSelUniform.runLocalResidue_of_abs_mod
                  (Q := Q) (pc := pcF c') (shift := mS + 2 * n + 1)
                  (T := Ts c') (l := l) (ρ := rAbs)
                  (Nat.lt_of_lt_of_le Nat.zero_lt_one hQ1) (hpcF_dvd_Q c') hrAbsQ
                  (by rw [hrAbs_def]) (by omega)
                simpa [hρact_def] using hres
              have h0F : FFs c' (Ts c' + ρact) =
                  CopiedDstar.dstarRankGA_m P hV mS n := by
                apply hslope
                · omega
                · have := hactBandSuf c'
                ...
                · rw [Nat.add_sub_cancel_left, Nat.mod_eq_of_lt hρact_lt]
                  exact hresloc
              have h1F : FFs c' (Ts c' + ρact + pcF c') =
                  CopiedDstar.dstarRankGA_m P hV mS n := by
                apply hslope
                · omega
                · have := hactBandSuf c'
                ...
                · have hmod :
                      (Ts c' + ρact + pcF c' - Ts c') % pcF c' = ρact := by
                    calc
                      (Ts c' + ρact + pcF c' - Ts c') % pcF c'
                          = (ρact + pcF c') % pcF c' := by
                            rw [show Ts c' + ρact + pcF c' - Ts c' = ρact + pcF c' by omega]
                      _ = ρact % pcF c' := by
                            rw [Nat.add_mod, Nat.mod_self, add_zero, Nat.mod_mod]
                      _ = ρact := Nat.mod_eq_of_lt hρact_lt
                  rw [hmod]
                  exact hresloc
              have hactiveRanks := hBandActiveSuf c' (j0F c') (ī0F c') (B := B)
                mS n (pcF c') (Ts c') ρact (FFs c') (hpcF1 c') hρact_lt
                (hactBandSuf c') (hags c') h0F h1F
              have hactive_mem :
                  ρact ∈ {ρ ∈ Finset.range (pcF c') |
                    P.rank c' (copiedSlice mS n)
                          (CopiedDstar.cellTupleF
                            (fun _ : Fin (P.toPoly.arity c') =>
                              (CopiedCells.RegionSpecF.sufIdx (Ts c' + ρ) :
                      ...
                      P.rank c' (copiedSlice mS n)
                          (CopiedDstar.cellTupleF
                            (fun _ : Fin (P.toPoly.arity c') =>
                              ...
                              (B + 1) n) =
                        CopiedDstar.dstarRankGA_m P hV mS n} := by
                refine Finset.mem_filter.mpr ⟨Finset.mem_range.mpr hρact_lt, ?_⟩
                constructor
                · exact hactiveRanks.1
                · exact hactiveRanks.2
              have hrep_mod_Q : repN rcls ≡ n [MOD Q] := by
                rw [Nat.ModEq, hrepN_mod_Q rcls]
                dsimp [rcls]
                exact hrowClass_mod_Q
              have hshift_mod_Q :
                ...
                exact Nat.ModEq.add_right (Ts c') (Nat.ModEq.add_right 1 (Nat.ModEq.add_left mS htwo))
              have hρeq :
                  ((rAbs + Q - ((mS + 2 * repN rcls + 1 + Ts c') % Q)) % pcF c') =
                    ρact := by
                rw [hshift_mod_Q, hρact_def]
              have hρrep_mem :
                  ((rAbs + Q - ((mS + 2 * repN rcls + 1 + Ts c') % Q)) % pcF c') ∈
                    sufTie rcls c' := by
                rw [hρeq]
                have hactive_rep :
                    ρact ∈ {ρ ∈ Finset.range (pcF c') |
                      P.rank c' (copiedSlice mS (repN rcls))
                            (CopiedDstar.cellTupleF
                              (fun _ : Fin (P.toPoly.arity c') =>
                                (CopiedCells.RegionSpecF.sufIdx (Ts c' + ρ) :
                                  CopiedCells.RegionSpecF B)) mS (B + 1) (repN rcls)) =
                          CopiedDstar.dstarRankGA_m P hV mS (repN rcls) ∧
                        P.rank c' (copiedSlice mS (repN rcls))
                            (CopiedDstar.cellTupleF
                              (fun _ : Fin (P.toPoly.arity c') =>
                                (CopiedCells.RegionSpecF.sufIdx (Ts c' + ρ + pcF c') :
                  ...
                  rw [← hsets]
                  exact hactive_mem
                simpa [hsufTie_def] using hactive_rep
              have hrmem : rAbs ∈ rowSsuf hpG idx (n % pG) c' := by
                rw [hrow, hSsuf_def]
                exact Finset.mem_filter.mpr ⟨Finset.mem_range.mpr hrAbsQ, hρrep_mem⟩
              exact hgate.2.2.2.1 c' rAbs hrmem
                (fun _ : Fin (P.toPoly.arity c') => mS + 2 * n + 1 + l)
                hselD.1.1
                (by
                  have hqBl : qB ≤ l := by omega
                  have hrunBand :=
                    CopiedBandRunGate.suffixBandGuard_copiedSlice mS n qB l hm (by omega) hlm hqBl
                  have hpmod : (mS + 2 * n + 1 + l) % Q = rAbs := by rw [hrAbs_def]
                  refine ⟨?_, hselRun.1, hselRun.2⟩
                  intro i
                  exact ⟨⟨hrunBand.1, hpmod⟩, hrunBand.2⟩)
            ·
              ...
              have hkN : k < Ndeep c' := by
                rw [hNdeep_def]
                exact lt_of_le_of_lt hk_le_from_locus hdeep.2
              let rcls : Fin pG := rowClass hpG (n % pG)
              have hrow : rowDsuf hpG idx (n % pG) c' = Dsuf rcls c' := by
                dsimp [idx, rcls]
                exact rowDsuf_mkBridgeRowIndex hpG S1tab F2tab Ssuf Spre Dsuf Dpre
              ...
              exact hgate.2.2.2.2.2.1 c' k hrmem
                ...
                (by
                  ...
                  omega)
        ·
          -- prefix-run residue coverage
          intro r hclass b hpre hcls hselD
          rcases b with ⟨c', xb⟩
          rcases hpre with ⟨l, hlo, hhi, hxbeq⟩
          change xb = (fun _ : Fin (P.toPoly.arity c') => l) at hxbeq
          subst xb
          have hbachP :
              P.rankOf (copiedSlice mS n)
                  (⟨c', fun _ : Fin (P.toPoly.arity c') => l⟩ : P.toPoly.Atom) =
          ...
          · have hrawcfg :
              ...
              (hsel n hNsel_n hdom hDpres
                (⟨c', fun _ : Fin (P.toPoly.arity c') => l⟩ : P.toPoly.Atom)
                hselD.1 hselD.2).mp hbachP
            exact hcore_from_cfg
              (⟨c', fun _ : Fin (P.toPoly.arity c') => l⟩ : P.toPoly.Atom)
              hselD.1 hselD.2 hrawcfg
              (by
                intro rs t hvalid htn hcell _hpos
                rw [CopiedBoundedGate.mem_boundedTuplesF]
                intro i
              ...
              (by
                ...
                | sufIdx l' =>
                    ...
                    omega)
          · have hlocus :=
              hAchLocus.2 c' (j0F c') (ī0F c') mS n (pcF c') (Tp c') Nc qB mx
                (FFp c') (hpcF1 c') hTqP (hFp c') (hagp c')
                (CopiedSelUniform.selconst_pre P c' (Mc c') (hMc c') mS n
                  (pcF c') (Tp c') Nc hm (hNcp c') (hprefcyc c'))
                hNcmS hNcmx l hlm hselRun hachRun
            rcases hlocus with hsh' | hslope | hdeep
            · exact False.elim (hsh hsh')
            ·
              ...
              have hresloc : ρact = (l - Tp c') % pcF c' := by
                have hres := CopiedSelUniform.runLocalResidue_of_abs_mod
                  ...
                  (T := Tp c') (l := l) (ρ := rAbs)
                  (Nat.lt_of_lt_of_le Nat.zero_lt_one hQ1) (hpcF_dvd_Q c') hrAbsQ
                  (by simp [hrAbs_def]) (by omega)
                simpa [hρact_def] using hres
              have h0F : FFp c' (Tp c' + ρact) =
                  CopiedDstar.dstarRankGA_m P hV mS n := by
                apply hslope
                · omega
                · have := hactBandPre c'
                ...
                · rw [Nat.add_sub_cancel_left, Nat.mod_eq_of_lt hρact_lt]
                  exact hresloc
              have h1F : FFp c' (Tp c' + ρact + pcF c') =
                  CopiedDstar.dstarRankGA_m P hV mS n := by
                apply hslope
                · omega
                · have := hactBandPre c'
                ...
                · have hmod :
                  ...
                  rw [hmod]
                  exact hresloc
              have hactiveRanks := hBandActivePre c' (j0F c') (ī0F c') (B := B)
                mS n (pcF c') (Tp c') ρact (FFp c') (hpcF1 c') hρact_lt
                (hactBandPre c') (hagp c') h0F h1F
              have hactive_mem :
                  ρact ∈ {ρ ∈ Finset.range (pcF c') |
                    P.rank c' (copiedSlice mS n)
                          (CopiedDstar.cellTupleF
                ...
                · exact hactiveRanks.2
              have hρrep_mem :
                  ((rAbs + Q - ((Tp c') % Q)) % pcF c') ∈ preTie rcls c' := by
                have hactive_rep :
                    ρact ∈ {ρ ∈ Finset.range (pcF c') |
                      P.rank c' (copiedSlice mS (repN rcls))
                            (CopiedDstar.cellTupleF
                  ...
                  rw [← hsets]
                  exact hactive_mem
                simpa [hpreTie_def, hρact_def] using hactive_rep
              have hrmem : rAbs ∈ rowSpre hpG idx (n % pG) c' := by
                rw [hrow, hSpre_def]
                exact Finset.mem_filter.mpr ⟨Finset.mem_range.mpr hrAbsQ, hρrep_mem⟩
              exact hgate.2.2.2.2.1 c' rAbs hrmem
                (fun _ : Fin (P.toPoly.arity c') => l)
                hselD.1.1
                (by
                  have hqBl : qB ≤ l := by omega
                  ...
                  refine ⟨⟨?_, hselRun.1⟩, hselRun.2⟩
                  intro i
                  simpa [hrAbs_def] using hpreGuard)
            ·
              set k0 : ℕ := mS - 2 - l with hk0_def
              have hk0_add : l + k0 = mS - 2 := by
                rw [hk0_def]
              ...
              have hk0N : k0 < Ndeep c' := by
                rw [hNdeep_def]
                exact lt_of_le_of_lt hk0_le_from_locus hdeep.2
              let rcls : Fin pG := rowClass hpG (n % pG)
              have hrow : rowDpre hpG idx (n % pG) c' = Dpre rcls c' := by
                dsimp [idx, rcls]
                exact rowDpre_mkBridgeRowIndex hpG S1tab F2tab Ssuf Spre Dsuf Dpre
              ...
              exact hgate.2.2.2.2.2.2 c' (k0 + 3) hrmem
                ...
                (by
                  ...
                  · intro q hq
                    rw [hpin] at hq
                    exact hfirst.2 q hq)
      exact hwhole b ⟨hbsel, hbD⟩ hbach

/-- **Budgeted row-indexed fibred tie bridge** (arity-1 supplier wrapper).  This
keeps the historical theorem name while the proof body now lives in
`tie_point_bridge_fibred_clause_budgeted_indexed_of_suppliers`. -/
theorem tie_point_bridge_fibred_clause_budgeted_indexed
    (P : WRP.Presentation Step Step) (hV : P.Valid)
    (harity1 : ∀ c, P.toPoly.arity c = 1) (C : ℕ) :
    ∃ (B Bh M _mthr pG q_U q_D Q : ℕ) (Ndeep : Fin P.toPoly.K → ℕ) (Mbr : ℕ),
      1 ≤ pG ∧ 1 ≤ Mbr ∧
      ∃ (GdfaF : BridgeRowIndex P B Bh M pG q_U q_D Q Ndeep → ℕ →
          (c : Fin P.toPoly.K) → SliceMSO.DetAuto (MarkedN (P.toPoly.arity c))),
              ...
              ∀ (c : Fin P.toPoly.K) (ī : Fin (P.toPoly.arity c) → ℕ),
                ...
                ((P.toPoly.sel c (copiedSlice mS n) ī
                 ...
                 ↔ ((GdfaF idx (n % pG) c).accepts
                    ...
                    ∧ P.rankOf (copiedSlice mS n) ⟨c, ī⟩
                        = CopiedDstar.dstarRankGA_m P hV mS n)) := by
  rcases bridgeSupplierBundle_of_arity_one P hV harity1 with
    ⟨hCoord, hRunClass, ⟨CbudN, hbudCN⟩, hRunResidue, hBandSound, hCoreBoundary⟩
  exact tie_point_bridge_fibred_clause_budgeted_indexed_of_suppliers P hV C
    hCoord hRunClass CbudN hbudCN hRunResidue hBandSound hCoreBoundary
...
/-- Fully generic finite-index bridge package, with the index type chosen
inside the bridge data.  This is the right shape for product indices that
depend on the bridge parameters, such as `ChoiceBridgeRowIndex`. -/
def TiePointBridgeBudgetedIndexedFinite
    (P : WRP.Presentation Step Step) (hV : P.Valid) (C : ℕ) : Prop :=
    ∃ (_B _Bh _M _mthr pG _q_U _q_D _Q : ℕ) (_Ndeep : Fin P.toPoly.K → ℕ) (Mbr : ℕ),
      1 ≤ pG ∧ 1 ≤ Mbr ∧
