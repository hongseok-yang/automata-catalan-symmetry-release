...
Paper references below cite the STABLE LaTeX labels (the slice analysis is
§7, `sec:slice-semilinearity`; the wrapped-flat/no-swap section is §8,
`sec:wrapped-flat`).

This is a *genuine* theorem about the concrete class `WRP.IsWRP` defined in
`WRP.lean`: it concludes `False` from the hypotheses that a WRP transduction
realises an area↔dinv swap.

Every ingredient is **proved**: `wrp_slice_profile_affine`
(the `W_n`-instance of the paper's §7 slice analysis, the `lem:one-loop-*`
lemmas) is a THEOREM, discharged for GENERAL arity by
...
(`thm:wrp-slice-semilinearity`) is `wrp_slice_profile_semilinear`,
and the combinatorial results `forced_triangular_profile`
(`cor:forced-triangular-pairs`) and `S_tri_not_semilinear`
(`lem:triangular-not-semilinear`)
are likewise proved.  Run `#print axioms wrp_no_area_dinv_swap` for the full trust
base: `[propext, Classical.choice, Quot.sound, SliceMSO.buchi]` — the textbook
Büchi–Elgot–Trakhtenbrot equivalence is the only axiom.
-/
...
/-- **Paper §7 slice analysis (the `lem:one-loop-*` lemmas), a
...
⚠ **The hypothesis `hne` is required.**  The
hypothesis `hne` (some slice is in `T`'s domain) cannot be dropped: without it the
...
no-swap theorem below); the explicit domain hypothesis `W_n ∈ dom(T)` of the
paper's `thm:wrp-slice-semilinearity` records the same point.

The conclusion also carries the **pointwise
identification**: on every in-domain slice the profile
...
the first-ascent profile set is semilinear.  Proved from the
slice-analysis theorem `wrp_slice_profile_affine` together with the
...
/-- **`cor:model-free-obstruction` (paper.tex).**  The combinatorial half of
the no-swap argument, stated without reference to any computational model: if
`F` is a length-preserving Dyck-path map swapping `area` and `dinv`, then its
first-ascent/tail profile on the family `W_n` is not semilinear.  The profile
is identified with `S_tri` by `forced_triangular_profile`
(`cor:forced-triangular-pairs`), and `S_tri` is not semilinear
(`S_tri_not_semilinear`, `lem:triangular-not-semilinear`).  Axiom-clean. -/
theorem model_free_obstruction
    ...
    ¬ IsSemilinear2 {p : ℕ × ℕ | ∃ n : ℕ, n ≥ 1 ∧
      p = (firstAscent (F (wrappedFlat n)), tailU (F (wrappedFlat n)))} := by
  rw [forced_triangular_profile F hDyck hlen harea hdinv]
  exact S_tri_not_semilinear
...
the concrete class `WRP.IsWRP`.  Bijectivity of `F` is nowhere assumed,
matching the paper's map form.  One intentional strengthening against the
paper: `Presentation.Valid` asks totality only of the combined order `≺`, so
`IsWRP` quantifies over a superset of the paper's class (the verbatim class is
covered by `wrp_no_area_dinv_swap_paper` in `WRPPaperTheorems.lean`).
Proof: by `Realises`, `T`
agrees with `F` on the Dyck slice `W_n`, so its profile set equals `F`'s;
length preservation gives the `O(n)` growth, so the slice analysis
(`wrp_slice_profile_semilinear`, `thm:wrp-slice-semilinearity`) makes that set
semilinear — contradicting the model-free obstruction
(`model_free_obstruction`, `cor:model-free-obstruction`). -/
theorem wrp_no_area_dinv_swap
  ...
  -- The slice analysis makes the profile semilinear, but the model-free
  -- obstruction says it is not.  Contradiction.
  have hsemi := wrp_slice_profile_semilinear T hWRP hgrow hne
  rw [hprof] at hsemi
  exact model_free_obstruction F hDyck hlen harea hdinv hsemi
