...
on `ε`.  This is why the revision's concatenation closure
(`thm:wrp-closures` (iii), lines 1490–1493) is stated on *nonempty* inputs.

This file defines the revision's classes and the transfer principles:

* `Polyreg.Presentation.ArityPos`, `Polyreg.IsPolyregularPos`, `WRP.IsWRPPos` —
  the revision's `def:polyregular` / `def:wrp` classes;
...
* `WRP.isWRPPos_of_isPolyregularPos` — the conservativity inclusion
  `prop:conservative` within the revision's convention (rank dimension `0`,
...
/-- The revision's class is contained in the Lean class (which also admits
...
/-- The value of a revision-convention polyregular map at `ε` is `ε` (or
undefined). -/
...
/-- The revision's class is contained in the Lean class; every negative
theorem over `IsWRP` yields the revision's statement a fortiori. -/
...
/-- The value of a revision-convention WRP map at `ε` is `ε` (or undefined). -/
...
/-- **`prop:conservative` within the revision's convention**: an arity-positive
...
/-! ## The revision's positive memberships land in its own class

The zeta sweep (`thm:zeta-wrp`), the Narayana height sweep (Section 6), and
every additive level sort (`prop:alw-sweep-swr`) are `sRR₁`, hence arity-1,
hence in the revision's `IsWRPPos` class. -/

/-- **`thm:zeta-wrp` under the revision's convention**: the zeta sweep is an
arity-positive WRP map. -/
theorem zetaSweep_isWRPPos :
    WRP.IsWRPPos (fun w : List Step => some (zetaSweep w)) :=
  zetaSweep_isSRR1.isWRPPos

/-- **Section 6 membership under the revision's convention**: the Narayana
height sweep is an arity-positive WRP map. -/
theorem heightSweep_isWRPPos :
    WRP.IsWRPPos (fun w : List Step => some (heightSweep w)) :=
  heightSweep_isSRR1.isWRPPos

/-- **`prop:alw-sweep-swr` under the revision's convention**: every additive
