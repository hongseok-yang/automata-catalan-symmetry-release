...
80% — `tie_count_fibred_of_gate_budgeted_indexed` — which takes a folded, row-indexed
gate hypothesis and produces the affine tie count.  Structure = `strict_count_fibred` with
...
budgeted capstone consumes `CopiedTieSlice.TiePointBridgeBudgetedIndexed`, whose finite row
index is chosen after the row and budget proof are known.
-/
...
open WRP Step SliceFamilyCell CopiedCells CopiedDstar CopiedRegionF CopiedRecount CopiedGateEP
  CopiedKernels CopiedSetup CopiedAffineAt MSOMarkN SliceMarkN SliceFasCountGA
open scoped Classical

/-- **Budgeted finite-index fibred tie count.**  The row-indexed form of the fibred tie
count: for a fixed budget `C`, the bridge may choose a finite row index `idx : ι` after
seeing the row `mS` and its budget proof.  The output
