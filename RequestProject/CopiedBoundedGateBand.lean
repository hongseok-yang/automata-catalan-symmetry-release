...
The bounded-cell full gate over the `mS`-free descriptor set `CopiedBoundedGate.boundedTuplesF`, with
the suffix/prefix RUN clauses replaced by their DEPTH-BANDED variants
(`CopiedBandRunGate.updateSufOrdClauseAtBandTot` / `updatePreOrdClauseAtBandTot`): the run clause
now fires only on competitors at least `kSuf`/`kPre` deep into the boundary run.  This is the sound
gate the bridge of `CopiedTieSlice` instantiates (with `kSuf = q_D+2`, `kPre = q_U`, and `Ssuf`/`Spre`
the TWO-rep tying residue sets): a two-rep-tying class is uniformly `d*` on the affine régime
`[q_D, …)`, so every banded competitor is a `d*`-achiever and the forward `TIE ⇒ accepts` direction
discharges.  The exported gate is `fasU_atomOrd_full_gate_bounded_band_updateDeep`.
...
open WRP Step MSOMarkN CopiedCells CopiedBoundedGate CopiedBandRunGate CopiedDstarCMS
  CopiedSelUniform CopiedSetupMS
...
non-scalarised form a descriptor-level bridge can supply directly. -/
...
tables: the stored representatives are the distinguished-coordinate update
tuples themselves, so completeness is just the affine table equation at the two
representatives. -/
