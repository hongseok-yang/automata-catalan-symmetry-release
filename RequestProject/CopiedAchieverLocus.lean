...
`d*`-achiever is covered by SOME gate.  This file proves the suffix and prefix cases: a selected-`D`
achiever at final-`D`-run depth `l` is SHALLOW (`l < q_D`, hence a CORE cell), or in a SLOPE-0 residue
class (hence its whole class ties `d*`, and a run clause of `CopiedSufRunGate` covers it), or a
FROM-END cell (`mS-1-l < mx+pc`, `mx = max q_U q_D`, covered by the deep gate of
`CopiedDeepRunGate`).

The three cases are clean once the run's `[Ts, Nc)`-segment of each `pc`-class is uniformly selected:
an interior achiever (`Ts+pc ≤ l ∧ l+pc < Nc`) forces its whole class to tie `d*` — this is exactly
the per-class uniformity core of `CopiedSelUniform` (the slope-sign / endpoint-domination collapse),
whose conclusion is the slope-0 disjunct.  The shallow and deep ends are pure arithmetic.  The deep
band must reach offsets `[1, mx+pc)` (not just `[1,q_D]`) because a slope<0 class's deepest SELECTED
member can sit at `l ∈ [Nc-pc, Nc)` (unreachable by the selection-constancy window, so deeper
same-class members may be unselected — no rank<d* contradiction); its from-end offset is
`mS-1-l ∈ [mx, mx+pc)`.
-/import RequestProject.CopiedSelUniform

namespace CopiedAchieverLocus

open WRP Step CopiedDstar CopiedDstarCMS CopiedSelUniform

/-- Arity-free suffix achiever trichotomy for an updated-coordinate tuple: the
selected-D constancy and witness use `Function.update ī0 j0`, so no tuple-fibre
scalarisation is needed. -/
...
/-- Descriptor-facing suffix/prefix d*-achiever coverage package: selected-D
constancy and the witness are stated directly for the distinguished-coordinate
update tuple. -/
